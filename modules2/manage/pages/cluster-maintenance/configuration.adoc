...
:page-aliases: cluster-administration:configuration.adoc
...
Cluster properties are stored internally and automatically replicated across all nodes, which ensures that each node is in sync. Examples of cluster properties include Kafka front-end settings, authentication settings, and settings for enabling features such as xref::tiered-storage.adoc[Tiered Storage] and xref:manage:rack-awareness.adoc[rack awareness]. Most Redpanda configuration properties are cluster properties.
...
* xref:./cluster-property-configuration.adoc[Configure cluster properties]
