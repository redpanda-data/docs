...
<head>
    <meta name="title" content="Maintenance Mode | Redpanda Docs"/>
    <meta name="description" content="Enable maintenance mode to temporarily take a broker offline, for example during a rolling upgrade."/>
    <link rel="canonical" href="https://docs.redpanda.com/docs/manage/node-management/" />
</head>

Maintenance mode lets you take a Redpanda broker offline temporarily while minimizing disruption to client operations. When a broker is in maintenance mode, you can safely perform operations that require the Redpanda process to be temporarily stopped; for example, system maintenance or a [rolling upgrade](../cluster-maintenance/rolling-upgrade).
