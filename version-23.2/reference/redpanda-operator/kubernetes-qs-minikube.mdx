---
title: Install Redpanda for Kubernetes with minikube
linkRoot: ../../../
---
...
<head>
    ...
    <link rel="canonical" href="https://docs.redpanda.com/docs/reference/redpanda-operator/kubernetes-qs-minikube/" />
</head>
...
Do not use mnikube for production workloads. Instead, [Install Redpanda for Kubernetes on Cloud](../kubernetes-qs-cloud).
:::
...
```bash
kubectl exec -it -n panda-ns one-node-cluster-0 -- rpk cluster metadata -X brokers='localhost:9092'
```
...
```bash
kubectl exec -it -n panda-ns one-node-cluster-0 -- rpk topic create panda-chat -p 5 -X brokers='localhost:9092'
```

Produce messages to the topic:

```bash
kubectl exec -it -n panda-ns one-node-cluster-0 -- rpk topic produce panda-chat -X brokers='localhost:9092'
```
...
```bash
kubectl exec -it -n panda-ns one-node-cluster-0 -- rpk topic consume panda-chat -X brokers='localhost:9092'
```
...
```bash
kubectl exec -it -n panda-ns one-node-cluster-0 -- rpk topic list -X brokers='localhost:9092'
```
