...
<head>
    <meta name="title" content="Transactions | Redpanda Docs"/>
    <meta name="description" content="The Transactions API gurantees both atomicity and exactly-once semantics. "/>
    <link rel="canonical" href="https://docs.redpanda.com/docs/develop/transactions/" />
</head>
...
- EOS helps developers avoid the anomalies of at-most-once processing (with potential lost events) and at-least-once processing (with potential duplicated events). Redpanda supports EOS when transactions are used in combination with [idempotent producers](../../develop/produce-data/idempotent-producers). 
...
:::note
Atomicity of transactions is not guaranteed when [remote recovery](../../manage/tiered-storage/#remote-recovery) is used.
:::
...
- When upgrading, make sure to use maintenance mode with [rolling upgrades](../../manage/cluster-maintenance/rolling-upgrade).
:::

The two primary use cases for transactions are:

- [Atomic (all or nothing) publishing of multiple messages](../../develop/transactions/#atomic-publishing-of-multiple-messages)
- [Exactly-once stream processing](../../develop/transactions/#exactly-once-stream-processing)
...
Transactions are supported on topics with compaction configured. When [`log_cleanup_policy`](../../reference/cluster-properties/#log_cleanup_policy) or [`cleanup.policy`](../../reference/topic-properties#cleanuppolicy) are set to either `compact` or `compact,delete`, the compaction process removes the aborted transaction's data and all transactional control markers from the log. The resulting compacted segment contains only committed data batches (and potentially harmless gaps in the offsets due to skipped batches). 
