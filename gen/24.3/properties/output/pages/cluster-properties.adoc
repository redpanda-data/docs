...
*Default:* `std::chrono::seconds(15min)`
...
*Default:* `std::vector<ss::sstring>{"basic"}`
...
A list of supported SASL mechanisms. Accepted values: `SCRAM`, `GSSAPI`, `OAUTHBEARER`.
...
*Default:* `std::vector<ss::sstring>{"scram"}`
