...
Data transforms support reading schemas and writing schemas to the schema registry within Redpanda. 

Once a schema is obtained, a record maybe decoded using the appropriate library. Using the TinyGo compiler and runtime, the following libraries for Apache Avro and Protocol Buffers have been known to work.
...
`SchemaReference` is a way for a one schema to reference another. The details for how referencing is done are type specific; for example, JSON objects that use the key "$ref" can refer to another schema via URL. For more details on references, see the following: xref:manage:schema-reg/schema-reg-api.adoc#reference-a-schema[Reference a schema]
...
`Decode` decodes b into v. If <<decodefn, `DecodeFn`>> option was not used, this returns `ErrNotRegistered`.
...
`MustAppendEncode` returns the value of <<serdet-append-encode, `AppendEncode`>>, panicking on error. This is a shortcut for if your encode function cannot error.
...
`MustEncode` returns the value of <<serdet-encode, `Encode`>>, panicking on error. This is a shortcut for if your encode function cannot error.
