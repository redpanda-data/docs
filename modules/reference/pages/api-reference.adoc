...
* Admin API Reference
+
Manage components of a Redpanda cluster, such as individual brokers and partition leadership. The Redpanda Admin API also allows you to perform operations that are specific to Redpanda Streaming and cannot be done using the standard Kafka API. See also: xref:manage:use-admin-api.adoc[].
+
--
