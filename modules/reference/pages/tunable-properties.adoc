...
The maximum limit per partition for the number of segments pending deletion from the cloud. Segments can be deleted due to retention or compaction. If this limit is breached and deletion fails, then segments are orphaned in the cloud and must be removed manually.
...
* link:./cluster-properties.mdx[Cluster Configurable Properties]
* link:./node-properties.mdx[Node Configuration Properties]
+
== Suggested reading
