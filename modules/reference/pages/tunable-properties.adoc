...
This threshold is evaluated with each request for data. When the amount of tokens to replenish exceeds this threshold, then we add tokens to the token bucket. This ensures we're not updating the atomic for the token count with each request. The range for this threshold is automatically clamped to the corresponding throughput limit for ingress and egress.
