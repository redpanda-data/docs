...
Indicates the number of active GET requests to the cloud storage.
...
Measures the utilization of the cloud storage client pool as a percentage (0 means unused, 100 means fully utilized).
...
Counts how many times a shard has borrowed a cloud storage client from another shard.
...
Counts the total number of NoSuchKey errors returned by the cloud storage provider.
...
Counts the total number of REST API errors encountered from the cloud storage provider.
...
Counts the total number of SlowDown errors received from the cloud storage provider.
...
Counts the total number of transport errors (including TCP and TLS errors) from the cloud storage provider.
...
Counts the total number of completed `GET` requests to cloud storage.
...
Reports the total number of bytes received from cloud storage.
...
Reports the total number of bytes sent to cloud storage.
...
Counts the total number of completed PUT requests to cloud storage.
...
Indicates the current number of files stored in the cloud storage cache.
...
Counts the total number of get requests directed at the cloud storage cache.
...
Reports the current size of the cloud storage cache in bytes.
...
Provides a histogram of the latency experienced when acquiring a cloud storage client.
...
Reports the size (in bytes) of a chunk downloaded from cloud storage for a given partition.
