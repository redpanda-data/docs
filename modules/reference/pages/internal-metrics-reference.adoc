...
Counts how many times a shard has borrowed a object storage client from another shard.
...
Reports the total number of offsets that are pending a commit to the iceberg catalog. Lag is reported only on the leader while followers report 0. -1 is reported if iceberg is disabled while -2 indicates the lag is not yet computed.
...
Reports the total number of offsets that are pending translation to iceberg. Lag is reported only on leader replicas while followers report 0. -1 is reported if iceberg is disabled while -2 indicates the lag is not yet computed.
