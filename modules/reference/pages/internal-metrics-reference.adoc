...
Current effective quota, in bytes per second.
...
Sequence of throttle times, in seconds.
...
Total amount of Kafka traffic (in bytes) taken in from clients for processing.
...
Total amount of Kafka traffic (in bytes) published to clients.
