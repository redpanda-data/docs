...
There are three entity types supported by Redpanda: user, client ID, and client ID prefix. Use the `--default` flag to assign quotas to default entity types.
...
|--dry |- |Perform a dry run. Validate the request without altering the quotas. Show what would be done, but do not execute the command.
