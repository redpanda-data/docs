...
```bash
rpk acl delete --allow-principal bar --operation all --topic foo --group g
```
...
```bash
rpk acl create --topic foo --operation all --allow-role red-role
rpk acl create --topic bar --operation all --deny-role red-role
```

It's possible to delete one of the roles:

```bash
rpk acl delete --topic foo --operation all --allow-role red-role
```
