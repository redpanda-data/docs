...
Creating an ACL with user pass:q[`*`] grants or denies the permission for all users.
...
the default host is the wildcard pass:q[`*`] which allows or denies the principal from
...
matches any name of the given resource type (--topic pass:q[`*`] matches all topics).
