...
Environment variables can be specified for the transform using the `--var` flag, these are separated by an equals for example: `--var=KEY=VALUE`. The `--var` flag can be repeated to specify multiple variables.
