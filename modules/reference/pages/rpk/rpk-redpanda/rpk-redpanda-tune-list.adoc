...
Most OSes configure THP use to `madvise` by default, meaning a process must explicitly request them. Redpanda is designed to make the required `madvise` call meaning THPs are in use under typical operation scenarios. Other possible values for THP use are `disabled` or `always`.
