...
The `COORDINATOR-PARTITION` column indicates the partition in the `__consumer_offsets` topic responsible for the group, if topic details are available; run with `--verbose` for more info if it is missing.
