...
:::danger
See xref:manage:kubernetes:security:kubernetes-tls.adoc[Configuring Redpanda TLS on Kubernetes] for the most up-to-date instructions using the Redpanda Helm chart. This page is based on the `redpanda-operator` Helm chart and is preserved only for backward compatibility.
:::
