...
A size-based retention limit for Tiered Storage that configures the maximum size that a topic partition in local storage can grow before becoming eligible for cleanup. It is equivalent to <<retentionlocaltargetbytes, `retention.local.target.bytes`>> without Tiered Storage.
