...
If `retention.ms` is set to a positive value, it overrides the cluster property xref:./cluster-properties.adoc#delete_retention_ms[`delete_retention_ms`] for the topic.
...
- xref:./node-configuration-sample.adoc[Node Configuration Template]
