...
| xref:./cluster-properties.adoc#_log_retention_ms[`log_retention_ms`]
...
If `retention.ms` is set to a positive value, it overrides the cluster property xref:./cluster-properties.adoc#_log_retention_ms[`log_retention_ms`] for the topic.
