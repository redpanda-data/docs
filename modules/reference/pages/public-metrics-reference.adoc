...
*Usage*: On a healthy system, this should not exceed the value set for prop:cloud_storage_cluster_metadata_upload_interval_ms[]. You may consider setting an alert if this remains `0` for longer than 1.5 * prop:cloud_storage_cluster_metadata_upload_interval_ms[] as that may indicate a configuration issue.
