...
The expiration time, represented as a unix-style timestamp, of the shortest-lived CA in the installed CA chain.
...
*Usage*: This metric is recorded for all resources that may have a TLS certificate installed. The `area` and `detail` labels provide a hierarchical way of identifying the resource you need information on.

---

=== redpanda_tls_certificate_expires_at_timestamp_seconds

The expiration time, represented as a unix-style timestamp, for the shortest-lived installed certificate. While up to five certificates may be installed for a resource, this provides only the timestamp of the next to expire.
...
The serial number of the shortest-lived installed certificate. As these serial numbers are technically unbounded, this will return the least significant 4B characters of the certificate's serial number. While up to five certificates may be installed for a resource, this provides only the serial number of the next to expire.
...
The last time, represented as a unix-style timestamp, that at least one TLS certificate was loaded for this resource.
...
Indicates whether the identified resource has at least one valid installed certificate.
