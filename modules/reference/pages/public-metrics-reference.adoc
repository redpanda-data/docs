...
*Usage*: Create an alert when this gauge dips below a steady-state threshold as a broker becomes unresponsive.
...
*Usage*: When this counter increases, the controller is dropping requests.
...
Number of partition replicas in the cluster currently being removed from a broker.
...
Alert for low cache storage: `0-OK`, `1-low space`, `2-degraded`.
...
Number of status RPCs received by a broker.
...
Number of status RPCs sent by a broker.
...
Number of timed-out status RPCs from a broker.
...
Memory usage ratio of in-flight requests in Schema Registry.
...
Usage ratio of in-flight requests in Schema Registry.
...
Number of requests queued in Schema Registry, due to memory limitations.
...
Number of won leader elections across all partitions in a given topic.
...
Total number of bytes that must be delivered to learners.
...
The sum of offsets that partitions on this broker need to recover.
...
Bandwidth consumed for partition movement, in bytes per sec.
...
Number of partition replicas are currently recovering on this broker.
...
Number of partition replicas that have to be recovered for this broker.
...
Running count of errors in creating and publishing audit event log entries.
