...
Number of partition replicas currently being added or moved to a broker.
...
Count of partitions that do not meet the required rack awareness constraint.

*Type*: gauge

*Usage*: Elevated values could suggest misconfigurations in rack placement, which can affect fault tolerance.
...
*Usage*: A non-zero value indicates that some partitions do not have an active leader. Consider increasing the number of brokers or replication factor if this persists.
...
The lowest recorded available memory (in bytes) per CPU shard since the process start.
...
Total number of cloud storage requests that experienced backoff delays.
...
Current utilization (as a percentage, 0–100) of the cloud storage client pool.
...
Total number of cloud storage download requests that experienced backoff delays.
...
Total number of successful download requests from cloud storage.
...
Histogram representing the lease duration for cloud storage clients.
...
Total number of cloud storage requests that resulted in a “not found” error.
...
Count of instances where a shard borrowed a cloud storage client from another shard.
...
Total number of cloud storage upload requests that experienced backoff delays.
...
Total number of successful upload requests to cloud storage.
...
[NOTE]
====
Cloud storage metrics are available only if you have:
...
Count of log segments that have been deleted from cloud storage due to retention policies or compaction processes.
...
Cumulative count of errors encountered during cloud storage operations, segmented by direction.
...
Count of times the cloud storage upload housekeeping queue was fully drained.
...
Total number of successfully executed cloud storage housekeeping jobs.
...
Total number of cloud storage housekeeping jobs that failed.
...
Count of cloud storage housekeeping jobs that were skipped during execution.
...
Count of times cloud storage upload housekeeping was paused.
...
Average rate (per shard) of requests that were throttled during cloud storage operations.
...
Count of instances when cloud storage upload housekeeping resumed after a pause.
...
Total number of rounds executed by the cloud storage upload housekeeping process.
...
Count of log segments reuploaded from cloud storage sources (either from the cache or via direct download).
...
Total number of successful partition manifest uploads to cloud storage.
...
Total number of successful segment index uploads to cloud storage.
...
Count of segment reader operations delayed due to reaching the reader limit.
...
Total number of successful data segment uploads to cloud storage.
...
Total number of log segments accounted for in cloud storage for the topic.
...
Count of log segments pending deletion from cloud storage for the topic.
...
Total number of successful spillover manifest uploads to cloud storage.
...
Total number of bytes uploaded for the topic to cloud storage.
