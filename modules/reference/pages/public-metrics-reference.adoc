...
The expiration time, represented as a unix-style timestamp, of the shortest-lived CA in the installed CA chain. This lists all resources that have at least one CA configured.
...
The expiration time, represented as a unix-style timestamp, for the shortest-lived installed certificate. While up to five certificates may be installed for a resource, this provides only the timestamp of the next to expire. This lists all resources that have at least one certificate installed.
...
Indicates whether a resource has at least one valid installed certificate. This lists all resources that have at least one certificate installed.
