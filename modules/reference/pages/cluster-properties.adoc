...
Whether or not Azure hierarchical namespaces (HNS) is enabled on <<cloud_storage_azure_storage_account>>. If this property is not set, then <<cloud_storage_azure_shared_key>> must be set, and each broker tries to determine at startup if HNS is enabled. Setting this property to `true` disables the check and assumes HNS is active. Setting this to `false` disables the check and assumes HNS is not active.
...
The managed identity ID to use for access to the Azure storage account. To use Azure managed identities, you must set <<cloud_storage_credentials_source>> to `azure_vm_instance_metadata`. See xref:manage:secuurity/iam-roles.adoc[IAM Roles] for more information on managed identities.
...
*Valid values*: config_file, aws_instance_metadata, sts, gcp_instance_metadata, azure_vm_instance_metadata
