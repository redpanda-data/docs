...
The managed identity ID to use for access to the Azure storage account. To use Azure managed identities, you must set <<cloud_storage_credentials_source>> to `azure_vm_instance_metadata`, or `azure_aks_oidc_federation` for Azure Kubernetes Service (AKS) deployments. See xref:manage:security/iam-roles.adoc[IAM Roles] for more information on managed identities.
