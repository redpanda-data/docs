...
Enable topic-aware partition balancing, which balances a topic's partition count on each broker before balancing the cluster's overall partition count. This is helpful because different topics in a cluster can have vastly different load profiles.
