...
Specifies the maximum number of active transaction sessions (producers). When the threshold
is exceeded, Redpanda terminates old sessions. When an idle producer corresponding to the terminated
session wakes up and produces, its batches are rejected with the message `invalid producer epoch` or
`invalid_producer_id_mapping`, depending on where it is in the transaction execution phase.
