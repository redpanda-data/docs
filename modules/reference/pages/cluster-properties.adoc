...
Validation mode performed before recovering a topic from cloud storage. In case of failure, the reason for failure will appear as ERROR lines in Redpanda log. For each topic, reports errors for all partitions, but for each partition, only the first error is reported.
