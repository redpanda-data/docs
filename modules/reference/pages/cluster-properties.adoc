...
IMPORTANT: Starting in version 23.3, `delete_retention_ms` is deprecated. Use xref:upgrade:deprecated/cluster-resource.adoc[log_retention_ms] instead.
...
The amount of time to keep a log file before deleting it (in milliseconds). If not set, the value in log.retention.minutes is used. If set to -1, no time limit is applied. This is a cluster-wide default when a topic does not set or disable xref:./topic-properties.adoc#retentionms[`retention.ms`].
