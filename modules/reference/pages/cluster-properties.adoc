...
List of throughput control groups that define exclusions from node-wide throughput limits.
...
* `[{'name': 'first_group','client_id': 'client1'}, {'name': 'catch all'}]`
* `[{'client_id': 'consumer-\d+'}]`

A connection is assigned the first matching group and is then excluded from throughput control.
