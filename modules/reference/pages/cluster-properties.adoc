...
Validation mode performed before recovering a topic from cloud storage. In case of failure, the reason for failure will appear as ERROR lines in Redpanda application log. For each topic, reports errors for all partitions, but for each partition, only the first error is reported.

This property accept the following parameters:

- `no_check`. Skips the checks for topic recovery.
- `check_manifest_existence`.  Run an existence check of each `partition_manifest`. Fails if there are connection issues to the cloud storage.
- `check_manifest_and_segment_metadata`. Download the manifest and run a consistency check, comparing the metadata with the cloud storage objects. The process will fail if any metadata references cloud storage objects that are missing.
