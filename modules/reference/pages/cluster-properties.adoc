...
Whether or not Azure hierarchical namespaces (HNS) is enabled on <<cloud_storage_azure_storage_account>>. If this property is not set, then each broker tries to determine at startup if HNS is enabled. Setting this property to `true` disables the check and assumes HNS is active. Setting this to `false` disables the check and assumes HNS is not active.
