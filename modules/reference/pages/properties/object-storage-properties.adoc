...
Maximum object storage cache size in bytes.

This property interacts with <<cloud_storage_cache_size_percent,`cloud_storage_cache_size_percent`>>:

- When they're both set Redpanda uses the smaller calculated value in bytes of the two.
...
Maximum size of the cache as a percentage minus the space that Redpanda avoids using defined by the xref:reference:cluster-properties.adoc#disk_reservation_percent[`disk_reservation_percent`] cluster property. Calculated at startup and dynamically updated if this property or  or  <<cloud_storage_cache_size,`cloud_storage_cache_size`>> changes.
