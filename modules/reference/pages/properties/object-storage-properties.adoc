...
Maximum size of the cloud cache as a percentage of unreserved disk space (see <<disk_reservation_percent>>). The default value for this option is tuned for a shared disk configuration. Consider increasing the value if using a dedicated cache disk.
