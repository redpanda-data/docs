...
Force Redpanda to use or not use an "Azure Data Lake Storage Gen2 hierarchical namespace" compliant client in <<cloud_storage_azure_storage_account,`cloud_storage_azure_storage_account`>>. 

When this property is not set, <<cloud_storage_azure_shared_key,`cloud_storage_azure_shared_key`>> must be set, and each broker infers at startup if a hierarchical namespace is enabled. 
