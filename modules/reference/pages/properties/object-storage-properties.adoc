...
Optional unique name to disambiguate this cluster's metadata in object storage (for example, for Whole Cluster Restore) when multiple clusters share a bucket. Must be unique within the bucket, 1-64 chars, `[A-Za-z0-9_-]`. Do not change once set.
