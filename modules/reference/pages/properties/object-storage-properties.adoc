...
Cluster provider access key. This access key is part of the credentials that Redpanda requires to authenticate with object storage services for Tiered Storage. This access key is used with the <<cloud_storage_secret_key>> to form the complete credentials required for authentication.
...
Cloud provider bucket or container that should be used to store data.
...
Required for cluster provider authentication with IAM roles.
...
Whether or not an Azure hierarchical namespace is enabled on the <<cloud_storage_azure_storage_account,`cloud_storage_azure_storage_account`>>. If this property is not set, <<cloud_storage_azure_shared_key,`cloud_storage_azure_shared_key`>> must be set, and each node tries to determine at startup if a hierarchical namespace is enabled. Setting this property to `true` disables the check and treats a hierarchical namespace as active. Setting to `false` disables the check and treats a hierarchical namespace as not active.
