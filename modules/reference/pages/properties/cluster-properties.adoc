...
The level of validation Redpanda performs on batches produced to topics. When set to `legacy`, minimal validation is performed. When set to `relaxed`, full validation is performed on uncompressed batches and compressed batches with the `max_timestamp` value left unset. When set to `strict`, full validation of all batches is performed. Use `strict` in environments where producers are not trusted.
...
*Accepted values:* `legacy`, `relaxed`, `strict`
