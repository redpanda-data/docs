...
TLS client-initiated renegotiation is considered unsafe and is disabled by default. Only re-enable it if you experience issues with your TLS-enabled client. This property has no effect on TLSv1.3 connections as client-initiated renegotiation was removed.
...
Target fetch size quota byte rate in bytes per second. This property is disabled by default.
