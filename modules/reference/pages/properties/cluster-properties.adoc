...
* `non_polling_with_debounce`: Allows for fetches to be delayed before they begin to work. This results in fewer/larger fetch responses, reducing overall reactor utilization, and allowing for other requests such as produces to increase. This option will add a fixed delay as specified by <<fetch_reads_debounce_timeout,`fetch_reads_debounce_timeout`>> to the start of every fetch.
