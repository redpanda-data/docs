...
* `polling`: Repeatedly polls every partition in the request for new data. The polling interval is set by <<fetch_reads_debounce_timeout,`fetch_reads_debounce_timeout`>>.

* `non_polling`: The backend is signaled when a partition has new data, so it doesn’t need to repeatedly read from every partition in the fetch. This should improve fetch latency and CPU utilization for most workloads.

* `non_polling_with_debounce`: This option behaves like `non_polling` but includes a debounce mechanism with a fixed delay specified by <<fetch_reads_debounce_timeout,fetch_reads_debounce_timeout>> at the start of each fetch. This delay results in fewer, larger fetch responses, reducing overall reactor utilization and allowing other requests, such as produces, to increase. 
...
NOTE: Memory per shard is computed after `data_transforms_per_core_memory_reservation`, and only applies when `log_compaction_use_sliding_window` is true.
