...
Once a broker has been restarted and the cluster is up and healthy with this sectio in place, then this section is not needed for subsequent restarts and can be removed from the config if desired (for example, if you need to have identical config files across each broker within a cluster).
