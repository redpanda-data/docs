...
List of node ID and UUID overrides applied at broker startup. Each entry includes the current UUID, desired new ID and UUID, and an ignore flag. An entry applies only if 'current_uuid' matches the broker's actual UUID.
