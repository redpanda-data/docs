...
Broker properties are organized under different top-level sections (`redpanda`, `pandaproxy`, `pandaproxy_client`, `schema_registry`, `schema_registry_client`, and `audit_log_client`) in your configuration file. Simple properties use a single key-value pair, while complex properties (such as listeners and TLS configurations) include examples.
...
Address of Kafka API published to the clients. If not set, the <<kafka_api, `kafka_api`>> broker property is used. When behind a load balancer or in containerized environments, this should be the externally accessible address that clients use to connect.
...
Directory for archival cache. Should be present when xref:reference:properties/cluster-properties.adoc#cloud_storage_enabled[`cloud_storage_enabled`] cluster property is present. If not specified, Redpanda uses a default path within the data directory.
...
* `<kafka-port>`: The port number for the Kafka API endpoint
* `<internal-listener-name>`: Name for internal network connections (e.g., `internal`)
* `<external-listener-name>`: Name for external network connections (e.g., `external`)
* `<mtls-listener-name>`: Name for mTLS connections (e.g., `mtls`)
...
The `authentication_method` property configures authentication for Schema Registry API listeners.
...
The `authentication_method` property configures authentication for HTTP Proxy API listeners.
