...
* prop:http_authentication[] (cluster property): Controls which authentication methods are available globally (`["BASIC"]`, `["OIDC"]`, or `["BASIC", "OIDC"]`)

When `authentication_method: http_basic` is set on a listener, clients can use any authentication method that is enabled in the prop:http_authentication[] cluster property.
...
* <<api_doc_dir, prop:api_doc_dir[]>> - API documentation directory (independent from HTTP Proxy's same-named property)
* <<schema_registry_api, prop:schema_registry_api[]>> - API listener configuration (similar to HTTP Proxy's <<pandaproxy_api, prop:pandaproxy_api[]>>)
* <<schema_registry_api_tls, prop:schema_registry_api_tls[]>> - TLS configuration (similar to HTTP Proxy's <<pandaproxy_api_tls, prop:pandaproxy_api_tls[]>>)
