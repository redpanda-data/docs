...
ifdev::env-cloud[]
Many topic-level properties have corresponding xref:manage:cluster-maintenance/config-cluster.adoc[cluster properties] that set a default value for all topics of a cluster. To customize the value for a topic, you can set a topic-level property that overrides the value of the corresponding cluster property.
endif::[]
