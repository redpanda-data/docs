Starting in Redpanda v25.3, several topic properties support enhanced tristate behavior. Properties like prop:retention.ms[], prop:retention.bytes[], prop:segment.ms[], and others now distinguish between zero values (immediate eligibility for cleanup/compaction) and negative values (disable the feature entirely). Previously, zero and negative values were treated the same way.
ifndef::env-cloud[]
