...
:page-aliases: deployment:performance-storage-tuning.adoc
...
* Use xref:cluster-administration:continuous-data-balancing.adoc[Continuous Data Balancing] to automatically distribute partitions across nodes for more balanced disk usage.
...
CAUTION: Do not set `delete_retention_ms` to `-1` unless you're using xref:data-management:tiered-storage.adoc#remote-write[remote write with Tiered Storage] to upload segments to remote storage. Setting it to `-1` configures indefinite retention, which can fill disk space.
...
For details about how to modify cluster configuration properties, see xref:cluster-administration:cluster-property-configuration.adoc[Cluster configuration].
