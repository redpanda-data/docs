...
* When upgrading, make sure to use maintenance mode with a glossterm:rolling upgrade[].
* When running transactional workloads from clients, it is recommended that you tune xref:reference:cluster-properties#max_transactions_per_coordinator[`max_transactions_per_coordinator`] to the expected number of active transactions that your clients are running at any given time (if your client transaction IDs are not reused). The total number of transactions in the cluster at any one time is `max_transactions_per_coordinator * transaction_coordinator_partitions` (default is 50). When the threshold is exceeded, Redpanda terminates old sessions. When an idle producer corresponding to the terminated session wakes up and produces, its batches are rejected with the message `invalid producer epoch` or `invalid_producer_id_mapping`, depending on where it is in the transaction execution phase.
Be aware that if you use the default and your clients create a new ID for every transaction, this will continue to accumulate and never remove, which will bloat memory.
