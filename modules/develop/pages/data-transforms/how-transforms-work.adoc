...
- *Single message transforms*: Only single record transforms are supported, but multiple output records from a single input record are supported. For aggregations, joins, or complex transformations, consider using xref:connect:streaming:about.adoc[Redpanda Connect] or https://flink.apache.org/[Apache Flink^].
