...
You can use Wasm transforms to fan out data from one input topic to multiple output topics, with each output topic having its own registered schema. This pattern is useful for:

* Processing batched updates where each sub-element routes to a different destination
* Distributing data to multiple Iceberg-enabled topics based on routing fields
* Schema-based partitioning where different message types go to different topics

See also: <<connect-to-schema-registry, Connect to the Schema Registry>>

Consider a data pipeline that processes batched updates. The following example demonstrates a common data pipeline pattern:

. *Input*: Batched updates in a single message (array of objects)
. *Extraction*: Each sub-element contains a routing field (for example, `"table": "orders"`)
. *Routing*: Route to output topic based on the routing field value
. *Validation*: Each output topic has its own schema registered in Schema Registry

Each message contains an array of table updates, where each update object has a `table` field indicating its destination:
...
The transform extracts each update and routes it to the appropriate topic based on the `table` field. Each output topic has its own registered schema in Schema Registry for validation.
...
// Schema definitions for each output topic (JSON Schema format)
// Define `required` fields to enforce data quality
const (
    orderSchema = `{
        "type": "object",
        "properties": {
...
var (
    srClient sr.Client
    schemaIDs map[string]int  // Map of topic name to schema ID
)

func main() {
    srClient = sr.NewClient()

    // Register schemas for each output topic using {topic-name}-value naming convention
    schemaIDs = make(map[string]int)
    schemaIDs["orders"] = ensureSchema("orders-value", orderSchema)
    schemaIDs["inventory"] = ensureSchema("inventory-value", inventorySchema)
    schemaIDs["customers"] = ensureSchema("customers-value", customerSchema)

    log.Printf("Registered schemas: %v", schemaIDs)
    transform.OnRecordWritten(routeUpdates)
}

func ensureSchema(subject string, schemaJSON string) int {
    subjectSchema, err := srClient.CreateSchema(subject, sr.Schema{
        Schema: schemaJSON,
        Type:   sr.TypeJson,
    })
    if err != nil {
        log.Fatalf("Failed to register schema for %s: %v", subject, err)
    }
    log.Printf("Schema registered: %s (ID: %d)", subject, subjectSchema.ID)
    return subjectSchema.ID
}
...
----
use redpanda_transform_sdk::*;
use redpanda_transform_sdk_sr as sr;
...
fn main() {
    env_logger::init();
    let sr_client = sr::Client::new();

    let mut schema_ids = HashMap::new();
    schema_ids.insert("orders".to_string(), ensure_schema(&sr_client, "orders-value", ORDER_SCHEMA));
    schema_ids.insert("inventory".to_string(), ensure_schema(&sr_client, "inventory-value", INVENTORY_SCHEMA));
    schema_ids.insert("customers".to_string(), ensure_schema(&sr_client, "customers-value", CUSTOMER_SCHEMA));
...
fn ensure_schema(client: &sr::Client, subject: &str, schema_json: &str) -> i32 {
    let schema = sr::Schema {
        schema: schema_json.to_string(),
        schema_type: sr::SchemaType::Json,
        references: vec![],
    };

    match client.create_schema(subject, &schema) {
        Ok(subject_schema) => {
            info!("Schema registered: {} (ID: {})", subject, subject_schema.id);
            subject_schema.id
        }
        Err(e) => panic!("Failed to register schema for {}: {}", subject, e),
    }
}

fn route_updates(event: WriteEvent, writer: &mut RecordWriter) -> Result<()> {
    ...
    for (i, update) in batch.updates.iter().enumerate() {
        if let Some(&schema_id) = schema_ids.get(&update.table) {
            ...
            let mut record = Record::new(data_bytes);
            record = record.key(event.record.key().unwrap_or_default().to_vec());

            writer.write_with_options(record, WriteOptions::to_topic(&update.table))?;
        } else {
            log::warn!("Unknown table in update {}: {}", i, update.table);
        }
    }
