...
await esbuild.build({
  plugins: [
    polyfillNode({
      ...
      polyfills: {
        console: true, // Enable console polyfill
...
// Check environment variables before registering the transform function.
const inputTopic = process.env.REDPANDA_INPUT_TOPIC;
if (inputTopic) {
  console.log(`Input topic: ${inputTopic}`);
} else {
  console.log("An input topic is not set");
}

const outputTopic0 = process.env.REDPANDA_OUTPUT_TOPIC_0;
if (outputTopic0) {
  console.log(`Output topic 0: ${outputTopic0}`);
} else {
  console.log("An output topic is not set");
}
...
- xref:reference:data-transforms/js/index.adoc[JavaScript Schema Registry client reference]
