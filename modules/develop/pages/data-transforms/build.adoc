...
The transform extracts each update and routes it to the appropriate topic based on the `table` field. In this example, it is assumed that you have created each output topic and xref:manage:schema-reg/schema-reg-api.adoc[registered the corresponding schemas] in Schema Registry.

NOTE: Replace the hardcoded schema IDs (1, 2, 3) in the code examples with the IDs returned by Schema Registry during registration. Use the `{topic-name}-value` naming convention for schema subjects (for example, `orders-value`, `inventory-value`).
