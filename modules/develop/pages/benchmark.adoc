...
=== Ensure consistent leader distribution

For reproducible benchmark results, configure Redpanda to use the `greedy` leader balancer mode. Unlike the default `calibrated` mode, `greedy` mode pre-calculates the target leader distribution and produces deterministic results across runs, eliminating noise from non-deterministic leader placement.

[,bash]
----
rpk cluster config set leader_balancer_mode greedy
----

See xref:manage:cluster-maintenance/cluster-balancing.adoc#leadership-balancing-modes[Leadership balancing modes] for details.
