...
ifndef::env-cloud[]
To disable idempotence (and risk duplicate messages as a result of retries), set prop:enable_idempotence[] to `false`. For instructions on how to edit any cluster property, see xref:manage:cluster-maintenance/cluster-property-configuration.adoc[Configure cluster properties].
endif::[]
