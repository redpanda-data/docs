...
Redpanda employs a unique strategy to help ensure accuracy in retention operations. Every time a producer posts a message to a topic, Redpanda records the broker's system date and time in the message's `broker_timestamp` property. This helps ensure retention functions as intended in cases where a producer submits a message with a created timestamp skewed from the broker's time. Closed segments are only eligible for deletion under Redpanda's retention policy when the age of all messages in the segment exceed a set threshold. Using broker timestamps ensures that skewed messages are cleaned up appropriately where they may otherwise appear to never exceed the retention policy threshold.

==== Configure broker timestamp alerting

Redpanda increments the xref:reference:internal-metrics-reference.adoc#vectorized_kafka_rpc_produce_bad_create_time[`vectorized_kafka_rpc_produce_bad_create_time`] metric each time a broker receives a message with a skewed created timestamp that is outside a specified range. Two cluster properties control the threshold for this metric. The minimum accepted values for both of these properties is 5 minutes. Any attempt to set a value lower than that is rejected by Redpanda. The properties are:
