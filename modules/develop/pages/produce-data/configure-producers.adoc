...
Any time the created timestamp is outside a specified range, Redpanda logs an alert to the xref:reference:internal-metrics-reference.adoc#vectorized_kafka_rpc_produce_bad_create_time[`vectorized_kafka_rpc_produce_bad_create_time`] metric.Two cluster properties control the alert threshold.The minimum accepted values for both of these properties is 5 minutes.Any attempt to set a value lower than that is rejected by Redpanda. The properties are:

* `log_message_timestamp_alert_before_ms`: Defines the allowed skew before the broker's time.Setting this to millis::max() effectively disables this check.Default: Not set.
* `log_message_timestamp_alert_after_ms`: Defines the allowed skew after the broker's time.There is no way to disable this check.Default: 3600000.

==== Disable broker timestamp retention

While not advised for typical use, Redpanda does make it possible to override the use of broker timestamps for retention policy. You accomplish this through use of the Admin API. Use the xref:api:ROOT:admin-api.adoc#Licenses-and-Features/operation/put_feature[`activate feature`] API to disable the `broker_time_based_retention` property.
