...
Redpanda employs a unique strategy to help optimize retention. Every time a producer posts a message to a topic, Redpanda records the broker's system date and time in the message's `broker_timestamp` property. This helps ensure retention functions as intended in cases where a producer submits a message with a created timestamp skewed from the broker's time.

Any time the created timestamp is outside a specified range, Redpanda logs an alert to the `vectorized_kafka_rpc_produce_bad_create_time` metric. Two properties control the alert threshold:

* `log_message_timestamp_alert_before_ms`: Defines the allowed skew before the broker's time. Setting this to millis::max() effectively disables this check. Default:
* `log_message_timestamp_alert_after_ms`: Defines the allowed skew after the broker's time. Default: 3600000

It is possible to override this behavior and use the message's created timestamp or append timestamp. You accomplish this through use of the Admin API. Use the `activate feature` API and specify the `broker_time_based_retention` property. If you do disable this feature, make sure to specify the desired retention policy by setting the timestamp policy appropriately. It defaults to `CreateTime` but may be changed to `AppendTime` if desired.
