...
While not advised for typical use, Redpanda allows you to override the use of broker timestamps for retention policy. You accomplish this through use of the Admin API. Use the link:/api/doc/admin/operation/operation-put_feature[`activate feature`] API to disable the `broker_time_based_retention` property.
