...
Leader Pinning controls which replica is elected as leader, and does not move replicas to different brokers. If all of a topic's replicas are on brokers in non-preferred racks, there is no replica in the preferred racks to elect as leader, and it is possible to have a non-preferred leader indefinitely.

For example, consider a cluster deployed across four racks (A, B, C, D) with Leader Pinning configured as `ordered_racks:A,B,C,D`. With a replication factor of 3, rack awareness can only place replicas in three of the four racks. If the highest-priority rack (A) does not receive a replica, there is no replica there to elect as leader, and it is possible to have a non-preferred leader indefinitely.
...
* Ensure the topic's replication factor is no greater than the number of preferred racks.

endif::[]
ifdef::env-cloud[]
To prevent this scenario:

* Configure `redpanda.leaders.preference` before creating topics to help ensure replicas land in preferred racks from the start.
* Ensure the topic's replication factor is no greater than the number of preferred racks.
