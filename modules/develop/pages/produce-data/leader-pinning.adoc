...
Before you can enable Leader Pinning, you must xref:manage:rack-awareness.adoc#configure-rack-awareness[configure rack awareness] on the cluster. If the prop:enable_rack_awareness[link=true,helm-path=auto] cluster configuration property is set to `false`, Leader Pinning is disabled across the cluster.
...
You can configure Leader Pinning at the topic level, the cluster level, or both. Set the topic configuration property to configure individual topics, or set the cluster configuration property to apply a default for all topics. You can also combine both: apply a cluster-wide default, then override specific topics with the topic property.

This configuration is based on the following scenario: you have Redpanda deployed in a multi-AZ or multi-region cluster, and you have configured each broker so that the prop:rack[link=true,helm-path=auto] configuration property contains racks corresponding to the AZs:
...
The xref:reference:properties/topic-properties.adoc#redpanda-leaders-preference[`redpanda.leaders.preference`] property inherits the default value from the cluster property prop:default_leaders_preference[].
...
* Set the cluster configuration property prop:default_leaders_preference[link=true,helm-path=auto], which specifies the default Leader Pinning configuration for all topics that don’t have prop:redpanda.leaders.preference[] explicitly set. It accepts values in the same format as prop:redpanda.leaders.preference[], where the default is `none`.
+
...
Use the topic configuration property prop:redpanda.leaders.preference[] to configure Leader Pinning for individual topics. The property accepts the following string values:
...
* Enable prop:enable_rack_awareness[link=true,helm-path=auto] to distribute replicas across racks automatically.
