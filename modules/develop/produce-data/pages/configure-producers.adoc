= Configure Producers
:description: Learn how to configure producers with Redpanda.
...
In cases when you configure the producer to be xref:produce-data:idempotent-producers.adoc[idempotent].
...
and xref:consume-data:consumer-offsets.adoc[Consumer Offsets].
