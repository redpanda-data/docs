...
- For Golang development, at least version 1.20 of https://go.dev/doc/install[Go^] installed on your host machine. For Rust development, you need the latest stable version of Rust installed on the host machine via https://rustup.rs/[rustup].
...
+
```bash
rpk transform init --language=<language>
```
+
...
+
TIP: When creating a custom data transform, initialization steps can be done in `main.rs` because it's only run once at the start of the package. Although state can be cached in global variables, Redpanda may restart a Wasm module at any point, which causes the state to be lost.
====
