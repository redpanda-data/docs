...
- The latest stable version of Rust installed on the host machine via [rustup](https://rustup.rs/).
...
```
+
You may specify a language using the `--language=<language>` flag. If you do not include the flag, the `transform init` command will prompt you for the language.
+
...
====
+
TIP: When creating a custom data transform, initialization steps can be done in `main` because they are only run once at the start of the package. Although state can be cached in global variables, Redpanda may restart a Wasm module at any point, which causes the state to be lost.
