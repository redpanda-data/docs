...
. Ensure that your Redpanda cluster has xref:manage:kubernetes/networking/external/index.adoc[external access] enabled and is accessible from your local machine using the advertised addresses.
+
...
. Ensure that you have configured `rpk` to connect to your cluster. You can use a xref:manage:kubernetes/networking/connect-to-redpanda.adoc#rpk-profile[pre-configured `rpk` profile]:
+
[source,bash]
----
