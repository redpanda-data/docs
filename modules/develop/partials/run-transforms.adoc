...
ifndef::env-cloud[]
- xref:deploy:redpanda/index.adoc[A Redpanda cluster] running at least version {page-component-version}.
...
+
TIP: For a tutorial on setting up a Redpanda cluster with external access, see xref:deploy:redpanda/kubernetes/get-started-dev.adoc[].
endif::[]
...
. To enable data transforms, set the prop:data_transforms_enabled[] cluster property to `true`:
+
...
You should see only the valid JSON from the input topic. Invalid JSON messages are written to the `invalid-json` topic.
