...
```js
import { onRecordWritten, Record } from "@redpanda-data/transform-sdk";
...
function csvToJsonTransform(event, writer) {
  // The input data is a CSV (without a header row) that is structured as:
  // key, item, quantity
  const input = event.record.value.toString('utf-8');
  ...
  for (const row of rows) {
    ...
    const value = JSON.stringify(itemQuantity);

    const record = new Record({
      key: Buffer.from(columns[0], 'utf-8'),
...
function filterValidJson(event, writer) {
  const recordValue = event.record.value;

  if (isValidJson(recordValue)) {
    writer.write(event.record);
  } else {
    writer.write(event.record, ??toTopic("invalid-json"));
  }
}
...
+
```bash
rpk topic create input-topic invalid-json
```
...
+
```bash
rpk transform deploy --input-topic=input-topic --output-topic=invalid-json
```
...
You should see only the invalid JSON from the input topic.
