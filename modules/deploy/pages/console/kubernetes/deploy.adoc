...
When the Console server is configured with TLS (`config.server.tls.enabled: true`), the ServiceMonitor uses HTTPS and references the TLS certificate and key paths for scraping.
