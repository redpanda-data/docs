...
:page-aliases: deploy:deployment-option/self-hosted/manual/production/production-readiness.adoc

Before running a production workload on Redpanda, follow this readiness checklist. Redpanda Data recommends using the xref:deploy:redpanda/manual/production/production-deployment-automation.adoc[automated deployment instructions] with Ansible. If you cannot deploy with Ansible, use the xref:deploy:redpanda/manual/production/production-deployment.adoc[manual deployment instructions].

NOTE: For Kubernetes deployments, see the xref:deploy:redpanda/kubernetes/k-production-readiness.adoc[Production Readiness Checklist for Kubernetes].

== Critical requirements

The Critical requirements checklist helps you to confirm that:
...
Check that the xref:get-started:licensing/index.adoc[Redpanda License] has been loaded into the cluster configuration. This is required to enable xref:get-started:licensing/index.adoc#redpanda-enterprise-edition[Enterprise] features. 
...
Check that you have the  xref:deploy:redpanda/manual/production/requirements.adoc#cpu-and-memory[correct number of CPUs and sufficient memory] to run Redpanda.
...
.Output for mdadm RAID mount point
[source,bash,role=no-copy]
----
Filesystem     Type  Size  Used Avail Use% Mounted on
...
Check that the xref:deploy:redpanda/manual/production/production-deployment.adoc#secure-the-admin-api[Admin API is secured], and any users defined in the superusers configuration are appropriately protected with strong credentials.

See also:
xref:manage:security/authentication.adoc#create-superusers
...
Check that xref:reference:rpk/rpk-iotune.adoc[`rpk iotune`] has been run on all hosts. Ensure that the mountpoint listed in this configuration file matches the mountpoint for Redpanda's data directory, usually `/var/lib/redpanda`. See xref:deploy:redpanda/manual/production/production-deployment.adoc#generate-optimal-io-configuration-settings[Generate optimal I/O configuration settings].

See also:

* xref:deploy:redpanda/manual/production/production-deployment.adoc#tune-the-linux-kernel-for-production[Tune the Linux kernel for production]
* xref:deploy:redpanda/kubernetes/k-tune-workers.adoc[Tune Kubernetes Worker Nodes for Production]
...
Redpanda Data recommends that you set prop:minimum_topic_replications[] and prop:default_topic_replications[] to at least 3.
...
Check that you have a xref:deploy:redpanda/manual/production/dev-deployment.adoc[development environment] or test environment configured to evaluate upgrades and new versions before rolling them straight to production.
...
Check that you have an upgrade policy defined and implemented. Redpanda Enterprise Edition supports xref:upgrade:rolling-upgrade.adoc#perform-a-rolling-upgrade[rolling upgrades], so upgrades do not require downtime. However, make sure that upgrades are scheduled on a regular basis, ideally using automation such as xref:deploy:redpanda/manual/production/production-deployment-automation.adoc#use-ansible-to-install-redpanda[Ansible] or xref:manage:kubernetes/k-configure-helm-chart.adoc[Helm].

=== High availability 

If you have xref:deploy:redpanda/manual/high-availability.adoc[high availability] requirements, check that the cluster is configured across multiple availability zones or fault domains.
...
* xref:deploy:redpanda/manual/high-availability.adoc#multi-az-deployments[Multi-AZ deployments]
* xref:manage:kubernetes/k-rack-awareness.adoc#configure-rack-awareness[Configure rack awareness in Kubernetes]

== Advanced requirements

The Advanced requirements checklist ensures full enterprise readiness, indicates that your system is operating at the highest level of availability, and can prevent or recover from the most serious incidents. The advanced requirements confirm the following:
...
Review your deployment automation. Specifically, if you need to reprovision a cluster, ensure that cluster installation is managed using automation such as xref:deploy:redpanda/manual/production/production-deployment-automation.adoc#use-terraform-to-set-up-infrastructure[Terraform], xref:deploy:redpanda/manual/production/production-deployment-automation.adoc#use-ansible-to-install-redpanda[Ansible], or xref:manage:kubernetes/k-configure-helm-chart.adoc[Helm], and that the configuration is saved in source control.
