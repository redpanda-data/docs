...
:page-categories: Deployment, Upgrades
:env-linux: true
...
If you install or upgrade Redpanda through the Linux package manager, you must re-point your package manager to `linux.pkg.redpanda.com` and import the new signing key. Existing and historical package versions remain available on the new repository, so no Redpanda version is dropped.

[IMPORTANT]
====
The legacy `dl.redpanda.com` repository is scheduled to shut down on *September 28, 2026*. Until then, both repositories serve identical packages, so you can migrate on your own schedule. After that date, hosts still pointed at `dl.redpanda.com` cannot install new packages or upgrades from it, and `apt update` or `yum makecache` against the legacy repository fails.
====
