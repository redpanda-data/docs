...
This guide covers the procedural steps for executing failover of shadow topics or an entire shadow cluster from read-only replicas to fully writable resources. For conceptual information about how failover works, see xref:./failover.adoc[].
...
For emergency situations, see xref:./disaster-response.adoc[].