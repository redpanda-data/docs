...
**Replicated by default (unless you set `exclude_default` to `true`):**
* `compression.type`
* `retention.bytes`
* `retention.ms`
* `delete.retention.ms`
...
To replicate additional topic properties beyond these defaults, explicitly list them in `synced_shadow_topic_properties`.
