...
Failover is the process of failing shadow topics or an entire shadow cluster from read-only replicas to fully writable resources. You can fail over individual topics for selective workload migration or fail over the entire cluster for comprehensive disaster recovery. This critical operation transforms your shadow resources into operational production assets, allowing you to redirect application traffic when the source cluster becomes unavailable.
...
2. **Failedover topics**: Converts read-only shadow topics into regular, writable topics
3. **Updates topic state**: Changes topic status from `ACTIVE` to `FAILING_OVER`, then `FAILED_OVER`

The failover process is irreversible. After topics are failedover, they cannot return to shadow mode, and automatic fallback to the original source cluster is not supported.
...
This failsover only the specified shadow topic while leaving other topics in the shadow link still replicating. Use this approach when you need to selectively fail over specific workloads or when testing failover procedures.
...
This failsover all shadow topics associated with the shadow link simultaneously, effectively failing over the entire cluster's replicated data. Use this approach during a complete regional disaster when you need to activate the entire shadow cluster as your new production environment.
...
Force deleting a shadow link immediately failsover all topics in the link, bypassing the normal failover state transitions. Use this approach only when topics are stuck in transitional states and you need immediate access to all replicated data.

[WARNING]
====
Force deleting a shadow link is irreversible and immediately failsover all topics. This should only be used as a last resort when normal failover procedures fail.
====
