...
Topic filters select which topics Shadowing automatically creates as shadow topics when they appear on the source cluster. Once Shadowing creates a shadow topic, it continues replicating until you fail over the topic, delete it, or delete the entire shadow link.
...
[,bash]
----
rpk shadow delete <my-disaster-recovery-link> --force
----

Force deleting a shadow link immediately promotes all topics in the link, bypassing the normal failover state transitions. Use this approach only when topics are stuck in transitional states and you need immediate access to all replicated data.

[WARNING]
====
Force deleting a shadow link is irreversible and immediately promotes all topics. This should only be used as a last resort when normal failover procedures fail.
====
