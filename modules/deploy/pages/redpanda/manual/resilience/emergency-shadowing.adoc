= Emergency Shadowing Failover
:description: Step-by-step emergency runbook for failing over Redpanda Shadow Links during disasters.
:env-linux: true
...
This runbook provides step-by-step procedures for emergency failover when your primary Redpanda cluster becomes unavailable. Follow these procedures only during active disasters when immediate failover is required.

[IMPORTANT]
====
This is an emergency procedure. For planned failover testing or regular shadow link management, see xref:deploy:redpanda/manual/resilience/shadowing.adoc[].
====
...
* [ ] Shadow link names and configuration details documented
...
=== Step 2: Verify shadow cluster status
...
**Critical**: Note the replication lag to estimate potential data loss.
...
**Wait for**: All critical topics to reach `PROMOTED` state before proceeding.

=== Step 6: Update application configuration

Redirect your applications to the shadow cluster:

1. **Update connection strings** in your applications to point to shadow cluster brokers
2. **Update DNS records** if using DNS-based service discovery
3. **Restart applications** to pick up new connection settings
4. **Verify connectivity** from application hosts to shadow cluster
...
* Time of failover initiation and completion
* Applications affected and recovery times
* Data loss estimates based on replication lag
* Issues encountered during failover
