...
The `CHART` column shows the Helm Chart version (e.g., `redpanda-5.2.4`), which should be compatible with the `APP VERSION` (Redpanda version).
--
...
The Operator version is shown in the image tag (e.g., `v25.2.4`), which should be compatible with your Redpanda broker version.
...
* Helm Chart or Operator version must be within ±1 minor version of Redpanda version
...
Check that the default replication factor is set appropriately for production.
