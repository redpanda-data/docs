...
:tags: ["Kubernetes"]
:page-aliases: deploy:deployment-option/self-hosted/kubernetes/kubernetes-production-deployment.adoc, deploy:deployment-option/self-hosted/kubernetes/k-deployment-overview.adoc
:env-kubernetes: true
...
IMPORTANT: Deploy Kubernetes clusters with *unmanaged (manual) node updates*. Managed (automatic) updates during cluster deployment can lead to service downtime, data loss, or quorum instability. Transitioning from managed updates to unmanaged updates after deployment may require downtime. To avoid these disruptions, plan for unmanaged node updates from the start. See xref:deploy:redpanda/kubernetes/k-requirements.adoc#node-updates[Kubernetes Cluster Requirements and Recommendations].
...
- Get started
** xref:deploy:redpanda/kubernetes/local-guide.adoc[Local Deployment Guide] (kind and minikube)
** xref:deploy:redpanda/kubernetes/aks-guide.adoc[Azure Kubernetes Service Guide] (AKS)
** xref:deploy:redpanda/kubernetes/eks-guide.adoc[Elastic Kubernetes Service Guide] (EKS)
** xref:deploy:redpanda/kubernetes/gke-guide.adoc[Google Kubernetes Engine Guide] (GKE)

- xref:deploy:redpanda/kubernetes/k-requirements.adoc[Kubernetes Cluster Requirements and Recommendations]

- xref:deploy:redpanda/kubernetes/k-production-workflow.adoc[Production deployment workflow]
