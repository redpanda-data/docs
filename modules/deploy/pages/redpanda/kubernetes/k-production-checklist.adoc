...
=== 1. Check connectivity from `rpk` to brokers with/without SASL
...
=== 4. All brokers active membership
...
=== 5. No brokers in maintenance mode
...
=== 8. Default topic replication factor (≥3)
...
See xref:develop:config-topics.adoc#choose-the-replication-factor[Choose the Replication Factor] for guidance on selecting appropriate replication factors.

=== 9. Existing topics replication factor (≥3)
...
=== 11. RAID/LVM stripe configuration (multiple disks only)
...
=== 12. Storage performance requirements
...
=== 13. CPU and memory resource limits
...
=== 14. CPU to memory ratio (1:2 minimum)
...
=== 22. Pod Disruption Budget configured
...
=== 23. Rack awareness and topology spread
...
=== 24. Redpanda license verification
...
=== 25. Operator CRDs validation (Operator deployments only)
...
=== 26. Deployment method detection
...
=== 27. XFS filesystem for data directory
...
=== 29. Node isolation configuration
...
=== 30. Continuous data balancing enabled
...
=== 32. System requirements validation
...
=== 34. Tiered Storage configuration
