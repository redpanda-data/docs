...
// The documented verification: every Redpanda-related resource must be gone before uninstalling
// the operator, or leftover finalizers block CRD deletion. Poll with a bounded loop.
// (step {"runShell": {"command": "timeout 300 bash -c 'until [ -z \"$(kubectl get users,topics,schemas,redpandas,consoles --namespace redpanda -o name 2>/dev/null)\" ]; do sleep 5; done'; kubectl get users,topics,schemas,redpandas,consoles --namespace redpanda", "timeout": 330000, "exitCodes": [0]}})
