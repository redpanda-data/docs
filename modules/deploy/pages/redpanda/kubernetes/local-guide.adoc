...
// CRD deletion takes ~40 seconds per CRD on CI runners (~6 minutes for all of them).
// The inner timeout kills the whole process tree on overrun so no orphaned kubectl
// process outlives the step and hangs the job.
// (step {"runShell": {"command": "timeout --kill-after=10 540 bash -c 'kubectl get crds -o name | grep cluster.redpanda.com | xargs kubectl delete'", "timeout": 600000, "exitCodes": [0]}})
