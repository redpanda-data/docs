...
// CRD deletion can exceed 2 minutes on CI runners. The inner timeout kills the whole
// process tree on overrun so no orphaned kubectl process outlives the step and hangs the job.
// (step {"runShell": {"command": "timeout --kill-after=10 240 bash -c 'kubectl get crds -o name | grep cluster.redpanda.com | xargs kubectl delete'", "timeout": 300000, "exitCodes": [0]}})
