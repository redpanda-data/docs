...
* **Average batch size:**
  Reflects the average size of each batch. Large or inconsistent batch sizes may indicate the need to adjust producer settings or verify storage capacity.

* **Topic and partition summaries:**
  Provides details on resource usage by individual topics. For example, if a single topic (such as `_redpanda.transform_logs` in this example) is responsible for most throughput, it may need optimization or additional resources.
