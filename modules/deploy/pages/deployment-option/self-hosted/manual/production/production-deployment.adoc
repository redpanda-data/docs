...
`RP_BOOTSTRAP_USER=username:password[:mechanism]`. The only supported value for `mechanism` is `SCRAM-SHA-512`; if it is omitted, Redpanda defaults to `SCRAM-SHA-256`. 
