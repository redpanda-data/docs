...
`RP_BOOTSTRAP_USER=username:password[:mechanism]`. The only supported values for `mechanism` are `SCRAM-SHA-256` or `SCRAM-SHA-512`; if it is omitted, Redpanda defaults to `SCRAM-SHA-256`.
