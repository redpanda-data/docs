...
For a full and up to date list of configuration options please see the (https://registry.terraform.io/modules/redpanda-data/redpanda-cluster/aws/latest[Terraform module^]):
...
. Our module will generate a properly configured GCP network within your GCP project by default. You can disable this functionality by commenting out that section of the main.tf and providing your own subnet to var.subnet. 
...
```bash
terraform apply --var subnet=$GCP_SUBNET --var="public_key_path=~/.ssh/id_rsa.pub" --var "ssh_user=ubuntu" --var="project_name=$GCP_PROJECT_NAME" --var="hosts_file=$HOSTS_FILE_DIR"
```
