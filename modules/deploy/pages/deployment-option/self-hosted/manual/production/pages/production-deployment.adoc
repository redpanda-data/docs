...
* See xref:production:production-deployment-automation.adoc[Automate Deployment for Production] to use Terraform and Ansible to deploy Redpanda.
* See xref:get-started:quick-start.adoc[Redpanda Quickstart] to try out Redpanda in Docker or xref:production:dev-deployment.adoc[Deploy for Development].
:::
...
* Networking
 ** Minimum 10 GigE
* Follow guidelines to ensure the production stability of the cluster. See xref:manage:cluster-maintenance:disk-utilization.adoc[Manage Disk Space].
* Confirm that your client system clocks are aligned with broker time. See xref:reference:cluster-properties:.adoc#storage_ignore_timestamps_in_future_sec[storage_ignore_timestamps_in_future_sec].
...
:::tip
Redpanda strongly recommends using one of these supported deployment tools. See xref:production:production-deployment-automation.adoc[Automate Deploying for Production].
:::
