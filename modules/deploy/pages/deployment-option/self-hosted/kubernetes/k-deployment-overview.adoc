...
You can customize Redpanda configurations with Helm by overriding default values using YAML files (via `--values`) or directly on the command line (via `--set`).
...
When `useFlux` is set to `true`, the Redpanda Operator delegates reconciliation to https://fluxcd.io/flux/concepts/[Flux^] controllers. The workflow is as follows:
+
...
spec:
  clusterSpec:
    useFlux: false  # or true, depending on your desired deployment mode
----
...
Bare-metal Kubernetes environments give you complete control over both the control plane and the worker nodes, which can be advantageous when:
