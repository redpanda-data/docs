...
The recommended option for deploying Redpanda in Kubernetes is a combination of Helm and the Redpanda Operator. The Redpanda Operator handles the deployment and management of the Redpanda Helm chart for you using https://fluxcd.io/flux/concepts/[Flux^].
...
You can run Redpanda on managed Kubernetes services as well as in bare-metal environments. Managed Kubernetes services offer simpler deployment and maintenance, bare-metal environments provide complete control and potential cost efficiencies.
