...
:description: Learn how to deploy and configure Kafka Connect using the standalone Redpanda Connectors Helm chart.
...
This topic describes how to deploy Kafka Connect in Kubernetes using the standalone Redpanda Connectors Helm chart.

include::shared:partial$community-supported-connectors.adoc[]

The Redpanda Connectors Helm chart is a standalone chart that deploys an instance of https://redpanda.com/guides/kafka-tutorial/what-is-kafka-connect[Kafka Connect]. The underlying Docker image contains _only_ the MirrorMaker2 connector but you can build a custom image to install additional connectors.
...
The Redpanda Connectors Helm chart is a standalone chart that you deploy separately from your Redpanda cluster.
...
This section provides advice for configuring the standalone Redpanda Connectors Helm chart. For all available settings, see xref:reference:k-connector-helm-spec.adoc[].
...
- The Kafka Connect cluster URL and status
- A list of available connector types (MirrorMaker2 connectors)
- The ability to create new connector instances
...
Before deploying Kafka Connect in production, verify these configuration items:

==== Security

- [ ] **TLS enabled**: Set `brokerTLS.enabled=true` if your Redpanda cluster uses TLS
- [ ] **SASL configured**: Configure `auth.sasl` settings if using authentication
- [ ] **Network policies**: Ensure proper network segmentation between components
- [ ] **Service accounts**: Use dedicated service accounts with minimal required permissions

==== Scalability and reliability

- [ ] **Multiple replicas**: Set `deployment.replicas` to at least 2 for high availability
- [ ] **Resource limits**: Configure appropriate CPU/memory limits based on your workload
- [ ] **Anti-affinity rules**: Use pod anti-affinity to spread replicas across nodes
- [ ] **Node selection**: Use node selectors or taints to avoid scheduling on Redpanda nodes

==== Monitoring and observability

- [ ] **Prometheus monitoring**: Enable `monitoring.enabled=true` if using Prometheus Operator
- [ ] **Log aggregation**: Configure log collection for centralized logging
- [ ] **Health checks**: Verify liveness and readiness probes are working
- [ ] **Alerting**: Set up alerts for connector failures and resource usage

==== Data management

- [ ] **Internal topics**: Configure replication factors for internal topics appropriately
- [ ] **Backup strategy**: Plan for connector configuration backup and recovery
- [ ] **Topic naming**: Establish naming conventions for connector-created topics
