...
:page-aliases: platform:getting-started/kubernetes-qs-local-access.adoc, platform:getting-started/kubernetes-qs-minikube.adoc, platform:quickstart/kubernetes-qs-local-access.adoc, platform:quickstart/kubernetes-qs-minikube.adoc
...
```
+
You need cluster-level permissions to install glossterm:cert-manager[] and Redpanda Operator CRDs in the next steps.
...
+
```bash
alias rpk-topic="kubectl --namespace <namespace> exec -i -t redpanda-0 -c redpanda -- rpk topic -X brokers=redpanda-0.redpanda.redpanda.svc.cluster.local.:9093,redpanda-1.redpanda.redpanda.svc.cluster.local.:9093,redpanda-2.redpanda.redpanda.svc.cluster.local.:9093 --tls-truststore /etc/tls/certs/default/ca.crt --tls-enabled"
```
