...
:tags: ["Kubernetes"]
:page-aliases: deploy:deployment-option/self-hosted/kubernetes/kubernetes-best-practices.adoc, deploy:deployment-option/self-hosted/kubernetes/redpanda-cluster-recommendations.adoc
:page-toclevels: 1
...
This section provides advice for configuring the Redpanda Helm chart. For all available settings, see xref:reference:redpanda-helm-spec.adoc[]. To learn how to customize the Redpanda Helm chart, see xref:manage:kubernetes/configure-helm-chart.adoc[Customize the Helm Chart].
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#nameoverride[Helm specification].
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#commonlabels[Helm specification].
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#tolerations[Helm specification].

=== Docker image

Specify the image tag to deploy a known version of the Docker image. By default, the image tag is set in `Chart.appVersion`. Avoid using the `latest` tag, which can lead to unexpected changes.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#statefulset-replicas[Helm specification].
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#tls[Helm specification].

See also: xref:manage:kubernetes/security/kubernetes-tls.adoc[]

=== Authentication

Configure SASL authentication for secure communication with your Redpanda cluster.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#auth[Helm specification].

See also: xref:manage:kubernetes/security/sasl-kubernetes.adoc[]

=== Resources

By default, the resource values are for a development environment. In a production cluster, the resources you allocate to Redpanda should be proportionate to your machine type. You should determine and set these values before deploying the cluster.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#resources[Helm specification].

See also:

- xef:manage:kubernetes/manage-resources.adoc[]
- xref:deploy:deployment-option/self-hosted/kubernetes/kubernetes-cluster-requirements.adoc[]

=== Storage

By default, the Redpanda Helm chart creates PersistentVolumeClaims with 20Gi (PVCs) for each Pod (Redpanda broker) in the StatefulSet. The PVC uses the default StorageClass in your cluster.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#storage[Helm specification].

See also:

- xref:manage:kubernetes/storage/volume-types.adoc[]
- xref:manage:kubernetes/storage/configure-storage.adoc[]

=== External access

By default, the Redpanda Helm chart deploys a NodePort Service for external access to the Redpanda listeners.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#external[Helm specification].
...
- xref:manage:kubernetes/networking/configure-external-access-nodeport.adoc#externaldns[ExternalDNS with a NodePort Service]
- xref:manage:kubernetes/networking/configure-external-access-loadbalancer.adoc#externaldns[ExternalDNS with LoadBalancer Services]
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#logging[Helm specification].
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#monitoring[Helm specification].

See also: xref:manage:kubernetes/monitoring/monitor-redpanda.adoc[]
...
Probes determine the health and readiness of your Pods. Configure them based on the startup behavior of your Redpanda brokers.
...
RBAC is a method to provide permissions based on roles. You can enable or disable features that need extra privileges using RBAC.

NOTE: Rack awareness requires both ServiceAccount and RBAC.
...
For all available settings, see the xref:reference:redpanda-helm-spec.adoc#rbac[Helm specification].
