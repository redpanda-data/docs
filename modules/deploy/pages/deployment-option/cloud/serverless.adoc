...
After you click to start a trial, Redpanda instantly prepares an account for you. Your account includes a `welcome` cluster and a `hello-world` demo topic in AWS you can explore. The `hello-world` topic is free as a read-only topic. It becomes to billable with your credits on the first write to it. 
