...
You can manage secure user access to Redpanda Cloud with single sign-on (SSO). Redpanda uses OIDC to delegate the authentication process to an external identity provider (IdP), such as Okta.

On the *Users* page, you can integrate Redpanda Cloud with up to two IdPs. Enter the client ID, client secret, and OIDC discovery URL supplied by your IdP. (Refer to your IdP documentation for these values.) You can also select to automatically assign a role to users who log in with the IdP: read, write, or admin.  

Security set up is different for every IdP. For Okta:

. Follow the https://help.okta.com/en-us/Content/Topics/Apps/Apps_App_Integration_Wizard_OIDC.htm[Okta documentation^] to create an application within Okta for Redpanda. The Redpanda callback location (that is, the redirect location where Okta sends the user) is `https://auth.prd.cloud.redpanda.com/login/callback`.  

. Okta provides the `clientId`, `clientSecret`, and `discoveryUrl`. The discovery URL for Okta generally looks like `https://<orgname>.okta.com/oauth2/<anId>/.well-known/openid-configuration`, where `anId` could be “default”. These fields are required for SSO configuration on the Redpanda Cloud *Users* page. 
