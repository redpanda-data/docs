...
Redpanda Cloud uses a control plane and data plane architecture, with clusters interchangeably called data planes. The control plane is a separate entity that enforces rules in the data plane. For example, with BYOC, you deploy the Redpanda data plane in your own VPC. Redpanda manages it on the control plane with an agent. 

image::shared:d_c_plane.png[Data plane and control plane]

=== Control plane

The control plane is where a good deal of cluster management, operations,
...
The data plane is where your Redpanda cluster resides. It is self-contained and managed by an agent that receives cluster specifications from the control plane. All network connections into the data plane take place through
