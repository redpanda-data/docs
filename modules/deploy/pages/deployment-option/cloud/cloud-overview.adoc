...
* <<Serverless>>: Clusters are hosted in Redpanda Cloud. This is the fastest and easiest way to start data streaming in the cloud. A cluster is available instantly with no base cost. With pay-as-you-go billing after the free trial, you only pay for what you consume. 
* <<Dedicated Cloud>>: Single-tenant clusters are hosted in Redpanda Cloud. This provides more control over your deployment. 
* <<Bring Your Own Cloud (BYOC)>>: Clusters are hosted in your private cloud. This provides more control over your deployment and offers full data sovereignty. 

=== Serverless

With Serverless clusters, you host your data in Redpanda's VPC, and Redpanda handles automatic scaling, provisioning, operations, and maintenance. 

Applications communicate with Serverless clusters over the public internet using encrypted connections. Serverless does not support private networking (that is, VPC peering to talk over private IPs). Use Dedicated clusters for private networking.

Available on AWS.

NOTE: Serverless is currently in a limited availability release with limited capacity, usage limits, and no public SLA.

==== Sign up for Serverless

Serverless supports self-service signup and pay-as-you-go billing with a credit card. You can view invoices and edit payment methods on the Redpanda Cloud *Billing* page. 
...
Contact https://redpanda.com/try-redpanda?section=enterprise-trial[Redpanda sales^] to request a private offer, and you are billed directly. 
...
With Serverless, you only pay for what you consume, without any commitment. A cluster is created instantly, so you can surface it in your applications (for example, for tenant isolation). As your workload increases, you can migrate a Serverless cluster to a Dedicated or BYOC cluster.
...
* Managed connectors
* Higher limits and quotas (see xref:deploy:deployment-option/cloud/serverless.adoc#limitations[Serverless limits])
