...
You can manage secure user access to Redpanda Cloud with single sign-on (SSO). Redpanda integrates with any OIDC-compliant identity provider (IdP) that supports discovery, including Okta, Auth0, Microsoft Entra, and Active Directory Federation Services (AD-FS). 
