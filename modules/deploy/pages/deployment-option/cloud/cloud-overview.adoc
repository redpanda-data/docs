...
When you create a cluster, you select your throughput tier. The following table lists current AWS and GCP tier limits. Legacy tiers may have different limits. For more information, contact https://support.redpanda.com/hc/en-us/requests/new[support^]. 
...
The Redpanda Cloud shared responsibility model outlines the security responsibilities between Redpanda and customers. Responsibilities depend on the type of deployment. 
...
| Redpanda upgrades and hotfixes | X |  
| Cost management and attribution | X |   
| Software vulnerability remediation | X |    
| Infrastructure vulnerability remediation | X |   
| IAM (roles, service accounts, access segmentation) | X |  
| Compute | X |   
| VPC (subnets, routing, firewall) | X |   
| VPC peering | X |  
| VPC private links (service endpoint) | X |    
| VPC private links (consumer endpoint) |  | X 
| Local storage | X |   
| Tiered Storage | X |  
| Control plane | X |  
| Access controls and audit | X |  
| Managed disaster recovery |  | X 
| Observability and monitoring (SLOs, SLIs, tracing, alerting, runbooks) | X |   
| Availability SLA | X |  
| Proactive threat detection | X |  
| Static secret rotation | X |  
| Incident response | X |   
| Resilience verification  | X |  
|===
...
| Redpanda upgrades and hotfixes | X |   
| Cost management and attribution | X | X 
| Software vulnerability remediation | X |  
| Infrastructure vulnerability remediation | X |   
| IAM (roles, service accounts, access segmentation) | X | X  
| Compute | X |    
| VPC (subnets, routing, firewall) | X | X 
| VPC peering |  | X  
| VPC private links (service endpoint) | X |  
| VPC private links (consumer endpoint) |  | X  
| Local storage | X |  
...
| Cost management and attribution | X | X  
| Software vulnerability remediation | X |   
| Infrastructure vulnerability remediation | X | X  
| IAM (roles, service accounts, access segmentation) |  | X  
| Compute | X |   
| VPC (subnets, routing, firewall) |  | X  
| VPC peering |  | X  
...
| Access controls and audit | X | X  
| Managed disaster recovery |  |  X 
| Observability and monitoring (SLOs, SLIs, tracing, alerting, runbooks) | X | X  
| Availability SLA | X | X  
| Proactive threat detection | X | X  
| Static secret rotation | X | X  
| Incident response | X |   
| Resilience verification | X | 
|===
