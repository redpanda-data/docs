...
You can manage secure user access to Redpanda Cloud with single sign-on (SSO). Redpanda integrates with any OIDC-compliant identity provider (IdP) that supports discovery, including Okta, Autho0, Microsoft Entra, and Active Directory Federation Services (AD-FS). 

=== Integrate IdP

On the *Users* page, you can integrate Redpanda Cloud with up to two IdPs. Enter the client ID, client secret, and discovery URL for the IdP. (Refer to your IdP documentation for these values.) You can also select to automatically assign a role to users who log in with the IdP: read, write, or admin.
...
Okta provides the following fields required for SSO configuration on the *Users* page: `clientId`, `clientSecret`, and `discoveryUrl`. The discovery URL for Okta generally looks like the following (where `anId` could be “default”):

[pass]
```bash
https://<orgname>.okta.com/oauth2/<anId>/.well-known/openid-configuration
``` 
