...
On the *Users* page, you can integrate Redpanda Cloud with up to two IdPs. Enter the client ID, client secret, and discovery URL for the IdP. (See your IdP documentation for these values.) You can also select to automatically assign a read, write, or admin role to users who log in with the IdP.
