...
Redpanda Cloud uses a control plane and data plane architecture. The control plane enforces rules in the data plane, and the data plane is where your Redpanda cluster resides. The term _data plane_ is used interchangeably with cluster. For example, with BYOC, you deploy the Redpanda data plane in your own VPC. Redpanda manages it on the control plane with an agent. 
