...
You can manage control plane authentication (organization to Redpanda Console) with single sign-on (SSO). Redpanda integrates with any OIDC-compliant identity provider (IdP) that supports discovery, including Okta, Auth0, Microsoft Entra, and Active Directory Federation Services (AD-FS). 

=== Integrate IdP

On the *Users* page, you can add connections for up to two different IdPs. Enter the client ID, client secret, and discovery URI for the IdP. (See your IdP documentation for these values. Discovery URI may be named the well known URL or the `issuer_url`.) By default, the connection is added in a disabled state. Edit the connection to enable it and optionally assign a read, write, or admin role to users who log in with the IdP. Deleting an SSO connection also deletes all users attached to it. 
