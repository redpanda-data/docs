...
On the *Users* page, you can integrate Redpanda Cloud with up to two IdPs. Enter the client ID, client secret, and OIDC discovery URL supplied by your IdP. (You may need to refer to your IdP documentation to find these values.) You can also select to automatically assign a role to users who log in with the IdP: read, write, or admin.  
