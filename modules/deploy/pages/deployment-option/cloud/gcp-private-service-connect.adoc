...
[,bash]
----
REDPANDA_BROKERS='<PSC-Kafka-API-seed-URL>'
...
curl -s \
  -X POST \
  <PSC-HTTP-Proxy-seed-URL>/topics/psc-test \
...
curl -s \
  -X POST \
  "<PSC-HTTP-Proxy-seed-URL>/consumers/test_group"\
...
curl -s -o /dev/null -w "%{http_code}" \
  -X POST \
  "<PSC-HTTP-Proxy-seed-URL>/test_group/instances/<instance>/subscription"\
  -H "Content-Type: application/vnd.kafka.v2+json" \
  -d '{
  "topics": [
     "psc-test"
...
----
curl -s \
  "<PSC-HTTP-Proxy-seed-URL>/records?timeout=1000&max_bytes=100000"\
...
The service attachment exposes a set of seed ports. The seed ports load balance requests among brokers. Use the following ports for initiating a connection from a consumer:
