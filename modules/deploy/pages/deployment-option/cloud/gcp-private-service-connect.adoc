...
The PSC bootstrap server hostname is unique to each cluster. The service attachment exposes a set of bootstrap ports for access to Redpanda services. These ports load balance requests among brokers. Make sure you use the following ports for initiating a connection from a consumer:
...
[,bash]
----
REDPANDA_BROKERS='<PSC-Kafka-API-bootstrap-server-URL>'
...
curl -s \
  -X POST \
  "<PSC-HTTP-Proxy-bootstrap-server-URL>/topics/psc-test" \
...
----
curl -s \
  "<PSC-HTTP-Proxy-bootstrap-server-URL>/topics/psc-test/partitions/0/records?offset=0&timeout=1000&max_bytes=100000"\
