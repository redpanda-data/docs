...
NOTE: This guide is for configuring GCP Private Service Connect using the Redpanda Cloud API. See xref:deploy:deployment-option/cloud/configure-private-service-connect-in-cloud-ui.adoc[Configure Private Service Connect in the Cloud UI] if you want to set up the endpoint service using the UI.
