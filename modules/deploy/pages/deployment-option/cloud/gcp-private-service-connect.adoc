...
. Call the https://docs.api.redpanda.com/#post-/v1beta1/networks[`POST /v1beta1/networks`] endpoint to create a network.
+
...
----
+
Store the network ID (`metadata.network_id`) returned in the response to the Create Network call.
+
[,bash]
----
...
+
Set the following variables. Variables with a `byovpc_` prefix represent xref:./vpc-byo-gcp.adoc[customer-managed resources] that should have been created previously.
+
--
...
export CLUSTER_POST_BODY=`cat << EOF
{
    ...
    "customer_managed_resources": {
        "gcp": {
            ...
            "console_service_account": { "email": "<byovpc_console_service_acc_email>L" },
