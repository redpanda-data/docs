...
The PSC bootstrap server hostname is unique to each cluster. The service attachment exposes a set of seed ports for access to Redpanda services. The seed ports load balance requests among brokers. Make sure you use the following ports for initiating a connection from a consumer:
