...
After <<get-a-cloud-api-access-token,getting an access token>>, you can <<,enable PSC when creating a new BYOC cluster>>, or you can <<,enable PSC for existing BYOC clusters>>.
...
You must use one additional customer-managed resource to use PSC on a BYOVPC Redpanda Cloud cluster. You need a NAT subnet with the *Purpose* set to `PRIVATE_SERVICE_CONNECT`. You can create the subnet using the `1_subnet` Terraform module. Review the module, then run the commands below.

Make sure to provide your own values for the following:
- `region`: The region of the Redpanda Cloud cluster.
- `project_id`: The *host* GCP project ID.
- `vpc_id`: The Redpanda Cloud cluster VPC ID.
- `psc_nat_subnet_name`: The name of the NAT subnet for Private Service Connect.
- `psc_nat_subnet_cidr`: The CIDR of the PSC NAT subnet. The mask should be at least `/29`. Each PSC connection takes up one IP address from the NAT subnet, so the CIDR must be able to accommodate all projects from which connections to the service attachment will be issued.
...
[,bash]
----
cd terraform/1_subnet
cat << EOF > input.tfvars
region = "<region>"
project_id = "<host-GCP-project-ID>"
vpc_id = "<host-VPC-ID>"
psc_nat_subnet_name = "<PSC-NAT-subnet-name>"
psc_nat_subnet_cidr = "<PSC-NAT-subnet-CIDR>"
EOF
terraform init
...
- `ZONES`: List of GCP zones where the brokers will be deployed. Format: `["<zone 1>", "<zone 2>", "<zone N>"]`
- `THROUGHPUT_TIER`: The Redpanda Cloud cluster tier. E.g. `tier-1-gcp-v2-x86`.
- `REDPANDA_VERSION`: The Redpanda Cloud cluster version.
- `CONSUMER_ACCEPT_LIST`: The list of IDs of GCP projects from which PSC connection requests are accepted. Format: `[{"source": "<GCP project ID 1>"}, {"source": "<GCP project ID 2>"}, {"source": "<GCP project ID N>"}]`
...
- `BYOVPC_SUBNET_MASTER_RANGE_NAME`: The name of the master IPv4 range.
    ...
    "customer_managed_resources": {
        "gcp": {
            "subnet": {
                ...
                "k8s_master_ipv4_range": "$BYOVPC_SUBNET_MASTER_RANGE_NAME"
            },
...
== Enable PSC on an existing BYOVPC Redpanda Cloud cluster
...
. Make a https://docs.api.redpanda.com/#patch-/v1beta1/clusters/-cluster.id-[`PATCH /v1beta1/clusters/{cluster.id}`] request to update the cluster to include the newly-created PSC NAT subnet and enable PSC.
+
Set the following environment variables:
