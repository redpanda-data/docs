...
The Redpanda AWS PrivateLink endpoint service to allows secure access to Redpanda Cloud from your own VPC. Consider using the endpoint service if you have multiple VPCs and could benefit from a more simplified network management:

* PrivateLink allows you to have overlapping CIDR ranges in VPC networks.
...
After <<get-an-access-token,getting an access token>>, you can <<enable-privatelink-endpoint-service-fo- new-clusters,enable PrivateLink when creating a new cluster>>, or you can <<enable-privatelink-endpoint-service-for-existing-clusters,enable PrivateLink for existing clusters>>.

== Requirements

* Ensure you have `rpk` installed.
* Your Redpanda cluster and <<create-client-vpc,VPC>> must be in the same region.
* You need to use the Redpanda Cloud API to enable the Redpanda endpoint service for your clusters. Follow the steps below to <<get-an-access-token,get an access token>>.  

== Get an access token 

. You need the base URL of the Redpanda Cloud API:
+
[,bash]
----
...
----
+
**BYOC clusters:** Check that the cluster operation is completed by calling https://docs.api.redpanda.com/#get-/v1beta1/operations/-id-[GET /v1beta1/operations/{id}], passing the operation ID returned from the Create Cluster call.
+
...
. Make a `PATCH /v1beta1/clusters/{cluster.id}` request to update the cluster with the Redpanda Private Link Endpoint Service enabled.
+
...
. Before proceeding, check the state of the Update Cluster operation by calling https://docs.api.redpanda.com/#get-/v1beta1/operations/-id-[GET /v1beta1/operations/{id}], passing the operation ID returned from Update Cluster call. When the state is `STATE_READY`, proceed to the next step.

. Check the service state by calling https://docs.api.redpanda.com/#get-/v1beta1/clusters/-id-[`GET /v1beta1/clusters/{id}`]. The `service_state` in the `private_link.status` response object needs to be `Available` for you to <<connect-to-privatelink-endpoint-service,connect to the service>>.
+
[,bash]
----
curl -X GET \
...
You need the service name to <<create-vpc-endpoint,create VPC private endpoints>>. Run the following command to get the service name:
...
[,bash]
----
PROFILE=<specific-profile-from-your-credential-file>

aws ec2 create-vpc --region $REGION --profile $PROFILE --cidr-block 10.0.0.0/20
CLIENT_VPC_ID=<client_vpc_id>
----

You can also use an existing VPC. You need the VPC ID to <<modify-vpc-attributes,modify its attributes>>.
...
This assumes that the Redpanda broker count is three. If you are not using three brokers, modify the example below:
...
You can access Redpanda services such as Schema Registry and HTTP proxy from the client VPC, for example, from an EC2 instance in the VPC.
...
=== Access Redpanda Proxy seed service
...
=== Access Redpanda Proxy on a specific broker
