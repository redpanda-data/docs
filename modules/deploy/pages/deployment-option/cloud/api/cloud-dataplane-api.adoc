...
The xref:deploy:deployment-option/cloud/api/cloud-api-overview.adoc#cloud-api-architecture[data plane] contains the actual Redpanda clusters. Every cluster is its own data plane, and so it has its own distinct Data Plane API URL.
...
+
--
To retrieve the Data Plane API URL of a cluster, make a request to `GET /v1beta2/serverless/clusters/\{id}`.
--
======
