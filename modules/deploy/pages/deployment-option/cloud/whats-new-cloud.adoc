...
Redpanda Cloud now supports OpenID Connect (OIDC) integration to leverage existing identity providers for xref:deploy:deployment-option/cloud/cloud-overview.adoc#configure-single-sign-on[single sign-on]. Redpanda uses OIDC to delegate the authentication process to an external identity provider (IdP), such as Okta. 
