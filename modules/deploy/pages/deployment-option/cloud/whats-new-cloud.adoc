...
Redpanda now supports xref:deploy:deployment-option/cloud/create-byoc-cluster-azure.adoc[BYOC clusters on Azure]. This is a limited availability release for BYOC clusters. Additional functionality is expected when this is generally available.

=== Serverless API

The xref:deploy:deployment-option/cloud/api/cloud-serverless-controlplane-api.adoc[Redpanda Cloud API] now includes support for xref:deploy:deployment-option/cloud/serverless.adoc[Serverless]. This lets you start event streaming in the cloud even faster through automating the creation and deletion of Serverless clusters. 

=== Self service sign up for Dedicated on AWS Marketplace

To start using Dedicated, you can now sign up for a xref:deploy:deployment-option/cloud/cloud-overview.adoc#sign-up-for-dedicated[free trial on the AWS Marketplace]. New trials receive $300 (USD) in free credits to spend in the first 30 days. AWS Marketplace charges for anything beyond $300 unless you cancel the subscription. After 30 days, you can continue using your Dedicated cluster without any commitment, only paying for what you consume.
