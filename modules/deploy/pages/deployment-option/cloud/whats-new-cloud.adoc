...
The xref:deploy:deployment-option/cloud/.adoc[Redpanda Cloud API] now includes support for xref:deploy:deployment-option/cloud/serverless.adoc[Serverless]. This lets you start event streaming in the cloud even faster through automating the creation and deletion of Serverless clusters. 
