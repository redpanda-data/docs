...
=== Control plane authentication with SSO

Redpanda Cloud now supports OpenID Connect (OIDC) integration to leverage existing identity providers for control plane authentication with xref:deploy:deployment-option/cloud/cloud-overview.adoc#single-sign-on[single sign-on] (SSO). Redpanda uses OIDC to delegate the authentication process to an external IdP, such as Okta. 
