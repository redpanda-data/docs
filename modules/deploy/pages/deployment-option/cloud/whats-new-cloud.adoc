...
Redpanda Cloud now supports OpenID Connect (OIDC) integration, so you can leverage existing identity providers for user authentication to your Redpanda organization with xref:deploy:deployment-option/cloud/cloud-overview.adoc#single-sign-on[single sign-on] (SSO). Redpanda uses OIDC to delegate the authentication process to an external IdP, such as Okta. 
