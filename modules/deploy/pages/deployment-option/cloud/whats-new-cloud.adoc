...
xref:deploy:deployment-option/cloud/serverless.adoc[Redpanda Serverless] moved out of beta and into limited availability. This means that it has limited capacity, usage limits, and no public SLA, but it is a production-ready deployment option with automatically-scaling clusters available instantly. There is no base cost to create a cluster and pay-as-you-go billing after the free trial. 
