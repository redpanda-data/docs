...
. Enter a cluster name, then select the namespace, provider (AWS), xref:deploy:deployment-option/cloud/byoc-tiers.adoc[region, availability zone type (single AZ or multi AZ), zones], and Redpanda version.
+
[NOTE]
==== 
