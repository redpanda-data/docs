...
+
Enter a cluster name, and select the provider (AWS), xref:deploy:deployment-option/cloud/regions.adoc[region], availability zone type (single AZ or multi AZ) and zones, xref:deploy:deployment-option/cloud/cloud-overview.adoc#cluster-tiers[tier], and Redpanda version.
+
[NOTE]
====
