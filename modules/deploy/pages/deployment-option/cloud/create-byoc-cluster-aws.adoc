...
. Enter a cluster name, then select the namespace, the provider (AWS), region, availability zone type (single AZ or multi AZ) and zones, throughput tier, and Redpanda version.
+
[NOTE]
==== 
