...
* Each Iceberg sink connector has to have its own control topic. The control topic should be created before creating the connector.
...
Data stored in Iceberg format can be read by different analytical systems like AWS Athena, Snowflake or Apache Spark.

The following example uses:

* Iceberg REST catalog
* AWS S3 bucket as a storage for Iceberg files
* Apache Spark reading the Iceberg data from S3 bucket
