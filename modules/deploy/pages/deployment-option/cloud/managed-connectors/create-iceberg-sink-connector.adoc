...
The primary use-case for Iceberg is for single storage of analytical data. It is cheap to read from different tools like AWS Athena, Snowflake or Apache Spark.
The traditional way of importing data is to push the data to every single tool, which means a higher cost of data transfer and storage.
