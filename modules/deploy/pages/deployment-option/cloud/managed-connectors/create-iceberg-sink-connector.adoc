...
== Using Iceberg connectors with AWS Glue catalog and AWS Lake Formation
The connector can be used with AWS Glue catalog and AWS Lake Formation service.
AWS Lake Formation allows to use assume role form of authentication only.
Connectors UI does not support Lake Formation specific properties. Please, use JSON config editor instead.
Sample config:
...
```
{
    ...
    "iceberg.catalog.client.assume-role.tags.LakeFormationAuthorizedCaller": "iceberg-connector-1",
