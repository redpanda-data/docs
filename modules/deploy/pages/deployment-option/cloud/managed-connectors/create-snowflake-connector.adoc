...
Snowflake tables have metadata containing offset information. If a Redpanda topic has been dropped and recreated since the initial sync to Snowflake it can cause issues due to mismatch of offsets which can mean new data is not sent to Snowflake. In the case of deleting/recreating Redpanda topics that have already been sent data to Snowflake via the connector, you must delete the corresponding Snowflake table as well.
...
|* Ignoring invalid task provided offset mytopic-12/OffsetAndMetadata{offset=7182, leaderEpoch=null, metadata=''} -- not yet consumed, taskOffset=7182 currentOffset=6804"}*
|Snowflake tables store metadata containing offset information. It is likely that this topic has been dropped and recreated since the initial sync to Snowflake, and this may cause a mismatch of offsets. When deleting and/or recreating Redpanda topics that have already sent data to Snowflake, you must delete the corresponding Snowflake table as well.
|===
