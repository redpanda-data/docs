...
|Snowflake tables store metadata that contains offset information. It is likely that this topic was dropped and recreated since the initial sync to Snowflake, which caused a mismatch of offsets. When deleting or recreating Redpanda topics that have already sent data to Snowflake, you must delete the corresponding Snowflake table as well.
|===
