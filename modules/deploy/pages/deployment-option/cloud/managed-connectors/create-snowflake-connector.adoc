...
|Snowflake tables store metadata containing offset info. Likely this topic has been dropped and recreated since the initial sync to Snowflake causing issue with mismatch of offsets. When deleting/recreating Redpanda topics that have already been sent data to Snowflake,you need to delete the corresponding Snowflake table as well.
|===
