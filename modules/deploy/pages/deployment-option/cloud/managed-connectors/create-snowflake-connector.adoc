...
Snowflake tables contain metadata with offset information. If a Redpanda topic is dropped and recreated after the initial sync to Snowflake, it can cause issues due to a mismatch of offsets. This can result in new data not being sent to Snowflake. If you delete or recreate Redpanda topics that have already sent data to Snowflake using this connector, you must delete the corresponding Snowflake table as well.
