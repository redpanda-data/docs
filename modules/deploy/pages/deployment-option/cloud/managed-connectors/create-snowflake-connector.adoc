...
Snowflake tables contain metadata with offset information. If a Redpanda topic is dropped and recreated after the initial sync to Snowflake it can cause issues due to mismatch of offsets, which can result in new data not being sent to Snowflake. When deleting and/or recreating Redpanda topics that have already sent data to Snowflake using the connector, you must delete the corresponding Snowflake table as well.
