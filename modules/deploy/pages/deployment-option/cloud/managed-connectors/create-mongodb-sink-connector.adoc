...
== Prerequisites
* Valid credentials with `readWrite` role to access the MongoDB database. For more granular access, you need to allow
`insert`, `remove` and `update` actions to specific databases/collections.

== Limitations

If you want to use the MongoDB sink connector with the `MongoDB` CDC handler for data sourced from MongoDB (using the
MongoDB source connector), you need to select `STRING` or `BYTES` as the value converter for both the source and sink
connectors.
