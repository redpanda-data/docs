...
| `File name template`
| The template for file names on S3. Supports `{{ variable }}` placeholders for substituting variables. Supported placeholders are: <ul><li>``topic``</li><li>``partition``</li><li>``start_offset`` (the offset of the first record in the file)</li><li>``timestamp``</li><li>``key`` (when used, other placeholders are not substituted)</li></ul>
