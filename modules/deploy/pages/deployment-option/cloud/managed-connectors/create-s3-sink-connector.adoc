...
There are two sets of placeholders supported, (`topic`, `partition`, `start_offset`, `timestamp`) and (`key`), which  are exclusive.
For the former set, the `topic`, `partition` and `start_offset` placeholders are required.
