...
When using Connectors API, instead of `connection.url`, `connection.username` and `connection.password` configuration fields
you can specify a value for the `connection.uri` field, of the form `mongodb+srv://username:password@cluster0.xxx.mongodb.net`.
