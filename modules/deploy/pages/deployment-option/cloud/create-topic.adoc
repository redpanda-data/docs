...
After creating a cluster, you can create a topic for that cluster on the *Clusters* page, where you select the cluster in which you want to create the topic. For Dedicated and BYOC clusters, you can optionally overwrite the default settings. Defaults are populated from information stored in the broker.
