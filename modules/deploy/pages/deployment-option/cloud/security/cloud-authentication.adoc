...
The following features use xref:./authorization/cloud-iam-policies.adoc[IAM policies] to generate dynamic and short-lived credentials to interact with cloud provider APIs:
...
IAM policies have constrained permissions so that each service can only
