...
-d "$CLUSTER_PATCH_BODY" https://api.redpanda.com/v1beta1/clusters/<cluster-id>`
----
...
When you consume, produce to, or manage topics using xref:reference:rpk/rpk-topic/rpk-topic.adoc[`rpk`], you must provide a client certificate and private key. You may use the use the `--tls-cert` and `--tls-key` options, or xref:reference:rpk/rpk-x-options.adoc[environment variables] with `rpk`.
