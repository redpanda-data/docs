...
+
Enter a cluster name, then select the resource group (optional), cloud provider (AWS or GCP), xref:deploy:deployment-option/cloud/regions.adoc[region], availability zone type (single AZ or multi AZ) and zones, xref:deploy:deployment-option/cloud/cloud-overview.adoc#cluster-tiers[tier], and Redpanda version.
+
[NOTE]
====
