...
+
Enter a cluster name, and select the provider (AWS or GCP), xref:deploy:deployment-option/cloud/regions.adoc#?tab=tabs-1-amazon-web-services-aws-regions[region], availability zone type (single AZ or multi AZ) and zones, xref:deploy:deployment-option/cloud/cloud-overview.adoc#cluster-tiers[tier], and Redpanda version.
+
[NOTE]
====
