...
+
Enter a cluster name, then select the resource group, cloud provider (AWS, GCP, or Azure), xref:deploy:deployment-option/cloud/dedicated/dedicated-tiers.adoc[region, availability zones, tier], and Redpanda version.
+
[NOTE]
====
...
+
NOTE: Private networks require either a VPC peering connection or a private connectivity service, such as AWS PrivateLink, GCP Private Service Connect, or Azure Private Link. 
