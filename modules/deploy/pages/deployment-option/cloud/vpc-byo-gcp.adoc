...
With a standard BYOC cluster, Redpanda manages the VPC lifecycle. For additional security, you can deploy the Redpanda data plane into your existing VPC and manage the VPC lifecycle yourself. With a *customer-managed* VPC:
...
. Create service accounts with necessary permissions and roles for the Redpanda agent, cluster, Redpanda Console, connectors, and the GKE cluster.
...
. In the https://cloudv2.redpanda.com[Redpanda Cloud UI^], start to xref:./create-byoc-cluster-gcp.adoc[create a BYOC cluster] so you can get the generated Redpanda ID. This is necessary to create certain required resources.
+
IMPORTANT: On the *Network* page, select the *Customer-managed* network connection type, and enter the network, service account, and storage bucket information you created. However, before running the Validation `rpk` command, note the `redpanda-id` (for example, `cisld88gfi809ee1qjcg`). You finish the cluster deployment in a later step.  
+
...
. In a previous step, you started to create a BYOC cluster in the Redpanda Cloud UI, stopping just before the Validation step. With *customer-managed* networks, you must grant yourself (the user deploying the cluster with `rpk`) the following permissions. 
+
.Expand permissions
[%collapsible]
====
...
. On the *Deploy* page, similar to standard BYOC clusters, you need to log in to Redpanda Cloud and deploy the agent.