= Configure Customer-Managed VPC on GCP
...
* Redpanda requires less permissions than standard BYOC clusters. 
...
* Redpanda creates a private Google Kubernetes Engine (GKE) cluster in the VPC. The subnet and secondary IP ranges you provide must allow public internet access. 

* As part of configuring your VPC, you need to provide reserved CIDR ranges for the subnet and GKE Pods, Services, and master IP addresses.

* Your VPC and your Redpanda cluster can be in the same GCP project or in different GCP projects. The _host_ project is where the VPC network is created, and the _service_ project is where the Redpanda cluster is created. If your host project and your service project are in different projects, you must first provision a shared VPC in Google Cloud. For details, see the https://cloud.google.com/vpc/docs/provisioning-shared-vpc[Google documentation^]. 
