...
Your VPC must have reserved CIDR ranges for Pods, Services, and Google Kubernetes Engine (GKE) and firewall rules that grant the subnet internet access.
...
* Subnet IP range should be at least /20 CIDR; for example, 10.0.0.0/24.
...
In a previous step, you created a BYOC cluster in the Redpanda Cloud UI, stopping just before validating your configuration with `rpk`. Now, you run that final step to deploy your cluster. To validate your configuration, run:

```bash
rpk byoc gcp validate
```
