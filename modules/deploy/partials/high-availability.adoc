...
The primary reason to deploy across multiple availability zones is to achieve extremely high availability, even at the expense of other considerations. Choosing this approach requires careful consideration of your system's requirements. Some of the considerations of a multi-AZ apprhach include:

* Cost: Maintaining presence across multiple availability zones may incur additional costs. You may require additional nodes to hit the minimum requirements for utilizing a multi-AZ deployment. Data sent between availability zones is often chargeable, resulting in additional cloud costs.
...
* Complexity: The RedPanda operational complexity is not appreciably increased, but the complexity of your overall cloud solution is increased. Maintaining presence across availability zones requires additional servers with corresponding maintenance, access control, and other considerations.

=== Multi-AZ infrastructure

When we talk about multiple availability zones, RedPanda requires a minimum of three zones. Deploying across only two availability zones is problematic. For example, given a cluster with three brokers spread across two availability zones, you either end up with all three brokers in one zone or a pair of brokers in one with a single broker in the other. Either way, it's possible to lose a majority of your brokers with a single availability zone outage. You lose the ability to form consensus in affected partitions, negating the high availability state you desire.

=== Multi-AZ optimization

You can configure xref:develop:consume-data/follower-fetching.adoc[follower fetching] to help ease the latency problems associated with a multi-AZ configuration. This is achieved by configuring consumers to advertise their preferred rack using the `client.rack` option within their consumer configuration. This allows consumers to read data from their closest replica rather than always reading from a (potentially non-local) partition leader. Using this capability has the additional benefit of reducing non-local network traffic and thereby minimizing any cross-AZ network charges.

=== Multi-AZ example

Creating a multi-AZ deployment in RedPanda cloud is very straightforward - just select the Multi AZ availability option when creating a cluster. RedPanda also provides an official https://github.com/redpanda-data/deployment-automation[deployment automation] project using Ansible and Teraform to help self-hosted users stand up multi-AZ deployments quickly and efficiently.

==== Configuring terraform

The first thing yu need to do is to configure the appropriate terraform script for your cloud provider. Within the deployment-automation project, locate the file for your cloud of choice and edit the availability_zones parameter. Include each availability zone you intend to use for your deployment. For example, under AWS, you will edit the `aws/main.tf` file:
...
Alternatively, you can supply the configuration at the command line as follows:
...
Once you have teraform configured for your cloud provider and chosen availability zones, you can proceed to deploying your cluster. This example steps through deploying a multi-AZ cluster and validating the rack configuration.
...
pushd deployment-automation/aws # choose one: aws|azure|gcp
terraform init

# Deploy the infrastructure (note: we assume you have cloud credentials available)
terraform apply -var=availability_zone='["us-west-2a","us-west-2b","us-west-2c"]'
...
==== Employing follower fetching

Using Redpanda's follower fetching feature helps greatly in reducing the latency and potential costs involve in a multi-AZ deployment. This example illustrates how to use the feature.

[,bash]
----
# SSH into a cluster node using appropriate credentials
ssh -i ~/.ssh/id_rsa <username>@<hostname of redpanda node>
