...
:remote-read-replicas: manage:kubernetes/remote-read-replicas.adoc
:rack-awareness: manage:kubernetes/kubernetes-rack-awareness.adoc
:tiered-storage: manage:kubernetes/storage/tiered-storage.adoc
endif::[]
...
To ensure Redpanda Pods are distributed across multiple nodes, the Redpanda Helm chart uses `podAntiAffinity` rules to discourage the scheduler from placing Redpanda Pods on the same node. This not only ensures that if a node crashes, you only lose one Redpanda broker, it's also essential because Redpanda brokers must have access to all resources on the node.
endif::[]
