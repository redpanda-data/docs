...
* If your produce latency exceeds your tolerances, you can configure producers to have `acks=1` instead of `acks=all`; however, this introduces the possibility of losing messages if the primary region becomes lost or unavailable.
...
Enabling follower fetching has the additional potential benefit of reducing broker latency. If the nearest replica is lagging, however, you may instead encounter increased data latency. Make sure to monitor your system to determine if the cost savings are worth this data latency risk.
