...
[IMPORTANT]
====
The `scope` Helm value was removed in Redpanda Operator v25.2, and cluster scope became the default. Do not pass `--set scope=Cluster` when you deploy v25.2 or later. The chart rejects it as an unknown value.

If you deploy Redpanda Operator v25.1 or earlier, cluster scope is not the default. You must add `--set scope=Cluster` to the command above.
====
