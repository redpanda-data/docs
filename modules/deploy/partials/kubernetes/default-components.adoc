...
it doesn't necessarily jeopardize the security of internal communications.
...
[.no-copy]
----
redpanda-default-cert <1>
redpanda-default-root-certificate <2>
redpanda-external-cert <3>
redpanda-external-root-certificate <4>
----
...
NOTE: While the Helm chart provides self-signed certificates for convenience, in a production environment, it's best to use certificates from a trusted Certificate Authority (CA) or integrate with your existing CA infrastructure.
