...
[NOTE]
====
Version 0.6.0 of `csi-driver-lvm` is used to avoid volume-mounting issues with newer versions. Later versions enable some XFS filesystem features that are incompatible with the default kernels used in {k8s-managed-service}, leading to failed mounts. In recent `mkfs.xfs` updates, the `-i nrext64=1` option is enabled by default, which leads to the following error on older kernels:
----
