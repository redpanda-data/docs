...
[IMPORTANT]
====
The legacy `dl.redpanda.com` repository shuts down on *{legacy-repo-shutdown-date}*. Until then, both repositories serve identical packages, so you can migrate on your own schedule. After that date, hosts still pointed at `dl.redpanda.com` cannot install new packages or upgrades from it, and `apt update` or `yum makecache` against the legacy repository fails.
====

This change has no impact on running clusters or already-installed packages. It only affects where future installs and upgrades are downloaded from. You can complete the migration at any time during the dual-publish window ({repo-dual-publish-start} - {legacy-repo-shutdown-date}) with no downtime.
