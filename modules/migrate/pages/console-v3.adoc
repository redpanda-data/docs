...
If you previously assigned roles in Redpanda Console, you must now provision those users in Redpanda and grant them the appropriate permissions.

The following examples show how to map Console roles (`viewer`, `editor`, `admin`) to Redpanda ACLs.

[,bash]
----
# Allow a viewer to read topics and consumer groups
...
# Allow an editor to produce and consume
rpk security acl create \
  --allow-principal User:alice@example.com \
...
# Grant admin access (needs separate superuser assignment for Admin API)
...
| Admin
| All of the above, plus `superuser` role using the cluster configuration
|===
