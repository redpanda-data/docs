...
<4> *Configuration*: Strimzi's `default.replication.factor` setting translates to prop:default_topic_replications[] in Redpanda, aligning Kafka cluster configurations between the two.
...
TIP: You can use the xref:connect:get-started:quickstarts/helm-chart.adoc[Redpanda Connect Helm chart], which comes with hundreds of prebuilt connectors, change data capture (CDC) capabilities, and YAML-configurable workflows.
