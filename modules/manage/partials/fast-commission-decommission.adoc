...
+
--
* prop:initial_retention_local_target_bytes_default[]: Initial local retention size target for partitions of topics with Tiered Storage enabled. The default is null.
** Use the prop:initial.retention.local.target.bytes[] topic configuration property to override on the topic level.
* prop:initial_retention_local_target_ms_default[]: Initial local retention time target for partitions of topics with Tiered Storage enabled. The default is null.
** Use the prop:initial.retention.local.target.ms[] topic configuration property to override on the topic level.
--
+
