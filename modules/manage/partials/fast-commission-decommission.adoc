...
. Make sure to xref:manage:tiered-storage.adoc#enable-tiered-storage[configure topics for Tiered Storage].
...
IMPORTANT: Because topics become more reliant on object storage to serve data, you may experience higher latency reads for data beyond the range of the configured local xref:manage:tiered-storage.adoc#set-retention-limits[retention target]. Carefully weigh the tradeoffs between faster broker commission/decommission and the increased read latency due to less data being available in local storage.
