...
Each unique combination of region and endpoint creates a separate object storage connection on the remote cluster. A cluster supports a maximum of 10 connections.

How connections are counted depends on `cloud_storage_url_style`:

- `virtual_host`: Each unique combination of bucket, region, and endpoint counts as one connection. You can create up to 10 distinct cross-region Remote Read Replica topics per cluster.
- `path`: Each unique combination of region and endpoint counts as one connection (the bucket name is not part of the key). You can create cross-region Remote Read Replica topics for an unlimited number of buckets, as long as those buckets are spread across no more than 10 distinct region/endpoint combinations.
