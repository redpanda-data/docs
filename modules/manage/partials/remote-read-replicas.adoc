...
** AWS: The remote cluster can be in the same region or a different region as the origin cluster's S3 bucket. For cross-region Remote Read Replica topics, see <<create-cross-region-rrr-topic>>.
** GCP: The remote cluster can be in the same region or a different region as the bucket/container.
...
- `virtual_host`: Each unique combination of bucket, region, and endpoint counts as one upstream. You can create up to 10 distinct cross-region Remote Read Replica topics per cluster.
- `path`: Each unique combination of region and endpoint counts as one upstream (the bucket name is not part of the key). You can create cross-region Remote Read Replica topics for an unlimited number of buckets, as long as those buckets are spread across no more than 10 distinct region/endpoint combinations.
