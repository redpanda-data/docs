...
A Remote Read Replica topic is a read-only topic that mirrors a topic on a different cluster. Remote Read Replicas work with both xref:manage:tiered-storage.adoc[Tiered Storage] and xref:manage:data-archiving.adoc[archival storage].
...
* An origin cluster with xref:manage:tiered-storage.adoc#set-up-tiered-storage[Tiered Storage] set up.
