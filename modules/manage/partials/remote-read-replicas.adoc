...
WARNING: Modifying the prop:cloud_storage_bucket[] property after writing data to a bucket could cause data loss.
...
- For `<bucket_name>`, use the bucket/container specified in the prop:cloud_storage_bucket[] or prop:cloud_storage_azure_container[] properties for the origin cluster.
...
* Do not use prop:redpanda.remote.read[] or prop:redpanda.remote.write[] with prop:redpanda.remote.readreplica[]. Redpanda ignores the values for remote read and remote write properties on read replica topics.
====
...
You must explicitly set the xref:reference:properties/object-storage-properties.adoc#cloud_storage_url_style[`cloud_storage_url_style`] cluster property to `virtual_host` or `path` on the remote cluster. The default value does not support cross-region Remote Read Replicas.
...
- `<bucket_name>`: The S3 bucket configured on the origin cluster (prop:cloud_storage_bucket[]).
...
How targets are counted depends on prop:cloud_storage_url_style[]:
...
if set, by the topic's prop:segment.bytes[] value. To reduce this lag in the data availability for the Remote Read Replica:

* You can lower the value of prop:segment.bytes[]. This lets Redpanda archive smaller log segments more frequently, at the cost of increasing I/O and file count.
* Redpanda Streaming deployments can set an idle timeout with prop:cloud_storage_segment_max_upload_interval_sec[link=true,helm-path=auto]
