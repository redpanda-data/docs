...
----
+
- `podMonitorNamespaceSelector.matchLabels.name`: The namespace in which you will deploy Redpanda. The Prometheus Operator looks for PodMonitor resources in this namespace.
...
. http://localhost:9090/graph[Open Prometheus^], and see that Prometheus is scraping metrics from your endpoints.
