...
== Connectors Issues Handling

Redpanda’s Support team monitor Managed Connectors 24/7 to ensure they meet service level agreements (SLAs). If incidents occur, teams at Redpanda trigger an incident response process to quickly mitigate them.

=== Increasing Consumer lag

Usually, the connector performs lower than expected when it is underprovisioned.

Change number of `Max Tasks` (`tasks.max`) in connector config for given number of instances and instance types. See: xref:deployment-option/cloud/managed-connectors/sizing-connectors.adoc[Sizing Connectors]

Another reasons for increasing consumer lag are:

* Connector has too low memory available
* Insufficient number of instances. Autoscaling base on the total connectors running task count

Contact Redpanda's Support team to align connector instances to your needs.

=== Connector in a failed state

The first step is a connector config and connector log check. Most connectors do fail immediately after the config change is conducted.

* Check exception details and stacktrace under `Show Error` button
* Check connector logs under `Logs` tab
* Restarting the connector using `Restart` button

The most frequent connector configuration issues causing the Failed status:
...
a|
* Check if the external system is up and running
* Check if the external system is available
* Check connector config if external system properties are correct  (url, table name, bucket name)

| *External system authentication issue*
a|
* Check if given account exists in an external system
* Check credentials defined in a connector config

| *Incorrect topic name or topic name pattern*
a|
* Check if expected topic is created
* Check if given topic name pattern matches at least one topic name

| *Out Of Memory error*
a|
* Change connector configuration, lower connector cache buffer size, decrease max records allowed in a batch
* Contact Redpanda's Support team
|===
