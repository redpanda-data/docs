...
Sink connector lag rate metric shows the difference between a topic max offset rate and a sink connector committed offsets rate.
When offsets in the topic increase faster than the sink connector is able to consume, the lag rate metric grows.
If the state persist for a longer period o time, it is recommended to react.
