...
Increase the number of `Max Tasks` (`tasks.max`) in the connector configuration for a given number of instances and instance types. For more information, see: xref:deploy:deployment-option/cloud/managed-connectors/sizing-connectors.adoc[Sizing Connectors].
