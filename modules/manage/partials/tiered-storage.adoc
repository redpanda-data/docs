...
CAUTION: Whole cluster restore is not intended to be a fully-functional disaster recovery solution. It does not provide snapshot-style consistency: some partitions in some topics will be more up to date than others. For example, restoring a consumer offsets topic could have offsets that point ahead of the restored data, although it will reflect the latest available offset in the topic data that is being restored. Committed transactions are not guaranteed to be atomic.

=== Start a target cluster

For Redpanda Enterprise, follow the steps to https://docs.redpanda.com/current/deploy/deployment-option/self-hosted/[deploy a new cluster] in a self-hosted environment.
...
Data from a failed cluster can be restored to a target cluster using an rpk command. 
...
- Topic definitions. If you have enabled tiered storage only for specific topics, topics without tiered storage enabled will not be restored 
...
. Ensure that all brokers are active:
+
[,bash]
----
...
. Select a broker and place it into maintenance mode:
+
```bash
...
Before continuing with the rolling restart, check these important metrics to make sure the cluster is healthy and working as expected.
...
After you've successfully restarted the broker:
