...
AKS supports OpenID Connect (OIDC) federation. Once a cluster is set and registered as an OIDC provider, you can configure Redpanda to use this federated identity to securely access Azure Blob Storage without needing to manage static credentials.
