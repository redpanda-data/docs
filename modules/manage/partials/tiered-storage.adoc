...
To ensure that the cluster reflects newly-restored configuration properties, you may perform a rolling restart after the restore process. 

TIP: Run the `rpk cluster config status` command to see nodes that need a restart for updated configuration properties to take effect.
