...
Starting in Redpanda 26.1, the preferred way to enable Tiered Storage for a topic is to set the `redpanda.storage.mode` topic property to `tiered`:
...
When `redpanda.storage.mode=tiered` is set, Tiered Storage is fully enabled for the topic. The `redpanda.remote.read` and `redpanda.remote.write` topic properties have no effect on a topic's storage permissions when `redpanda.storage.mode` is set to any value other than `unset`.
...
For example, to create a new `unset` topic with Tiered Storage enabled:
...
To enable Tiered Storage on an existing `unset` topic:
...
For Tiered Storage topics with `redpanda.storage.mode=unset` or `tiered`, the topic must not be a Remote Read Replica topic.
...
The following tables show outcomes for combinations of cluster-level and topic-level configurations for topics with `redpanda.storage.mode=unset`:
