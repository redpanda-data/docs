...
The `tiered` value of the `redpanda.storage.mode` topic property is an alias. When a topic is created, it resolves to the version selected by the config_ref:default_redpanda_storage_mode_tiered_impl,true,properties/object-storage-properties[] cluster property, which defaults to `tiered_v1`. To make all new `tiered` topics use Tiered Storage v2:
