...
See also: xref:manage:kubernetes/remote-read-replicas.adoc[Remote Read Replicas]
...
With tiered storage enabled, you can restore your cluster, including its metadata, in the event of failure. This is a faster, less expensive alternative to active-active replication (for example, using xref:upgrade:migrate/data-migration.adoc[MirrorMaker 2]).

You can take the following steps to restore a failed cluster:

. Start a new cluster
. Recover metadata from a failed source cluster to the new cluster
. Recover topic data from the source cluster to the new cluster
. Perform a rolling restart, if necessary
. Direct application workload to the new cluster

Use this recovery method to restore your application to the latest functional state as quickly as possible. Note that this method does not guarantee consistency at the application level. Restored transactions may not uphold atomicity. The consumer offset topic may not reflect point-in-time snapshots of topic data already consumed by applications. 

=== Start a recovery target cluster

Follow the steps to start a new cloud cluster. Cloud account credentials (such as secrets or access keys) are not included in backed up metadata, and so these must be manually configured for the new cluster.

=== Cluster metadata recovery

Metadata from a failed cluster can be restored to a target cluster using an rpk command. 
...
The following metadata is included in the restore:
- Topic definitions. If you have enabled tiered storage only for specific topics, non-tiered storage will not be recovered 
- Users and access control lists (ACLs)
- Schemas
- Consumer offsets 
- All cluster configurations, including your Redpanda license key
- Transaction metadata, up to the highest committed transaction. In-flight transactions are treated as aborted and will not be included in recovery

NOTE: Committed transactions are not guaranteed to be atomic.

The destination cluster must be empty; that is, the cluster cannot contain data other than node and cluster configuration properties. If metadata indicating usage is found in the destination (such as user-provided schemas, non-superusers, ACLs, consumer offsets, or user-created topics), the restore will fail and the rpk command will issue a warning.

If duplicate metadata is found in the destination cluster, it will be overwritten by the restore.

Metadata recovery supports only one source cluster. It is not possible to consolidate multiple clusters into the target cluster. 
...
=== Redirect workload to new cluster
