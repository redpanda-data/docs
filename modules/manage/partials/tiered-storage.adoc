...
By default, the cache directory is created in Redpanda's data directory (`<redpanda_data_directory>/cloud_storage_cache`), but it can be placed anywhere in the system. For example, you might want to put the cache directory on a dedicated drive with cheaper storage. Use the config_ref:cloud_storage_cache_directory,true,node-properties[] property on each broker to specify a different location for the cache directory. You must specify the full path.
