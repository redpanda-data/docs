...
AKS supports OpenID Connect (OIDC) federation. When a cluster is set and registered as an OIDC provider, you can configure Redpanda to use this federated identity to securely access Azure Blob Storage without needing to manage static credentials.
...
helm upgrade --install redpanda redpanda/redpanda --namespace <namespace> --create-namespace \
  ...
  --set storage.tiered.config.cloud_storage_azure_managed_identity_id=<managed-identity-client-id> \
...
+
--
- `<managed-identity-client-id>`: The access key for your Azure account.
...
cloud_storage_azure_managed_identity_id: <managed-identity-object-id>
