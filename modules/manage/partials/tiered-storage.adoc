...
See xref:deploy:redpanda/index.adoc[Deploy Redpanda] for deployment best practices.
====
...
. Copy the access key and secret key for the prop:cloud_storage_access_key[] and prop:cloud_storage_secret_key[] cluster properties.
. Run the `rpk cluster config edit` command, then edit the following required properties:
+
[,properties]
----
...
. Use a Google-managed encryption key.
. Set a https://cloud.google.com/storage/docs/migrating#defaultproj[default project^].
. Create a service user with https://cloud.google.com/storage/docs/authentication/managing-hmackeys[HMAC keys^] and copy the access key and secret key for the prop:cloud_storage_access_key[] and prop:cloud_storage_secret_key[] cluster properties.
. Run the `rpk cluster config edit` command, then edit the following required properties:
+
[,properties]
----
...
. Copy an account access key for the Azure container you want Redpanda to use and enter it in the prop:cloud_storage_azure_shared_key[] property. For information on how to view your account access keys, see the https://learn.microsoft.com/en-us/azure/storage/common/storage-account-keys-manage?toc=%2Fazure%2Fstorage%2Fblobs%2Ftoc.json&bc=%2Fazure%2Fstorage%2Fblobs%2Fbreadcrumb%2Ftoc.json&tabs=azure-portal#view-account-access-keys[Azure documentation^].
. Run the `rpk cluster config edit` command, then edit the following required properties:
+
[,properties]
----
...
. To enable Tiered Storage, set prop:cloud_storage_enabled[link=true,helm-path=auto] to `true`.
...
To enable Tiered Storage for a cluster (in addition to setting prop:cloud_storage_enabled[] to `true`), set the following cluster-level properties to `true`:

* prop:cloud_storage_enable_remote_write[link=true,helm-path=auto]
* prop:cloud_storage_enable_remote_read[link=true,helm-path=auto]
...
NOTE: prop:cloud_storage_enable_remote_write[] and prop:cloud_storage_enable_remote_read[] act as creation-time defaults only for topics with `redpanda.storage.mode=unset`. They have no effect on topics where prop:redpanda.storage.mode[] is explicitly set to `local`, `tiered`, or `cloud`. To apply a default storage mode to all new topics, use the prop:default_redpanda_storage_mode[link=true,helm-path=auto] cluster property instead.

==== Enable Tiered Storage for specific topics

Starting in Redpanda v26.1, the recommended way to enable Tiered Storage for a topic is to set the prop:redpanda.storage.mode[] topic property to `tiered`:
...
To enable Tiered Storage on an existing topic that was created in `local` mode:
...
When `redpanda.storage.mode=tiered` is set, Tiered Storage is fully enabled for the topic. The prop:redpanda.remote.read[] and prop:redpanda.remote.write[] topic properties have no effect on a topic's storage when prop:redpanda.storage.mode[] is set to any value other than `unset`.

==== Configure Tiered Storage using legacy topic properties

For topics with `redpanda.storage.mode=unset` (the default when prop:default_redpanda_storage_mode[] is not configured), Tiered Storage is controlled by the prop:redpanda.remote.read[] and prop:redpanda.remote.write[] topic properties:

* prop:redpanda.remote.write[]: Uploads data from Redpanda to object storage.
* prop:redpanda.remote.read[]: Fetches data from object storage to Redpanda.
...
To enable Tiered Storage on an existing topic where the storage mode is `unset`:
...
For newly-created unset topics, the cluster-level prop:cloud_storage_enable_remote_write[] and prop:cloud_storage_enable_remote_read[] properties dictate the topic-level properties prop:redpanda.remote.write[] and prop:redpanda.remote.read[] at topic creation time, respectively. Altering the cluster properties has no effect on existing topics, only newly-created ones. To alter the permissions for existing topics, you can set these topic properties directly. For example, `redpanda.remote.write=false` to disable uploads for a specific topic.
...
| prop:redpanda.remote.write[]
| Uploads data from Redpanda to object storage. For topics with `redpanda.storage.mode=unset`, overrides the cluster-level prop:cloud_storage_enable_remote_write[] setting. Has no effect on topics with prop:redpanda.storage.mode[] set to `local`, `tiered`, or `cloud`.

| prop:redpanda.remote.read[]
| Fetches data from object storage to Redpanda. For topics with `redpanda.storage.mode=unset`, overrides the cluster-level prop:cloud_storage_enable_remote_read[] setting. Has no effect on topics with prop:redpanda.storage.mode[] set to `local`, `tiered`, or `cloud`.

| prop:redpanda.remote.recovery[]
...
| prop:redpanda.remote.delete[]
| When set to `true`, deleting a topic also deletes its objects in object storage. Applies to both Tiered Storage topics and Cloud Topics.

For Tiered Storage topics, the topic must not be a Remote Read Replica topic.
...
|===
| Cluster-level configuration with prop:cloud_storage_enable_remote_write[] | Topic-level configuration with prop:redpanda.remote.write[] | Outcome: whether remote write is enabled or disabled on the topic
...
|===
| Cluster-level configuration with prop:cloud_storage_enable_remote_read[] | Topic-level configuration with prop:redpanda.remote.read[] | Outcome: whether remote read is enabled or disabled on the topic
...
* Tiered Storage v1 (`tiered_v1`) is the original implementation and remains the default.
* Tiered Storage v2 (`tiered_v2`) badge::[label=beta] is a new implementation that adds full compaction support, including tombstone removal, for topics stored in object storage.

IMPORTANT: Tiered Storage v2 is a beta feature. It is not supported for production deployments. Beta features are available for users to test and https://redpandacommunity.slack.com/[provide feedback].

You enable both versions the same way, by setting `redpanda.storage.mode=tiered` on a topic, and the storage mode of a topic displays as `tiered` for both. The version used by each topic is reported by the read-only prop:redpanda.storage.mode.impl[] topic property.
...
| Supported. Tombstone records are removed after the retention period configured by prop:delete.retention.ms[].
|===

==== Select a Tiered Storage version

The `tiered` value of the prop:redpanda.storage.mode[] topic property is an alias. When a topic is created, it resolves to the version selected by the prop:default_redpanda_storage_mode_tiered_impl[link=true,helm-path=auto] cluster property, which defaults to `tiered_v1`. To make all new `tiered` topics use Tiered Storage v2:
...
To select the version explicitly for a single topic, regardless of the cluster-wide default, set both the prop:redpanda.storage.mode[] and prop:redpanda.storage.mode.impl[] properties when you create the topic:
...
The two values must agree: both `tiered_v1` and `tiered_v2` correspond to the `tiered` storage mode. You can set prop:redpanda.storage.mode.impl[] on its own only when it matches the version selected by the cluster default.

To check which version an existing topic uses, run `rpk topic describe <topic_name>`. The prop:redpanda.storage.mode[] property displays `tiered` for both versions, and the prop:redpanda.storage.mode.impl[] property reports `tiered_v1` or `tiered_v2`.
...
* Upgrading a cluster never changes the version of existing topics. Changing prop:default_redpanda_storage_mode_tiered_impl[] affects only topics created after the change.
* A topic cannot be converted between Tiered Storage v1 and v2. The prop:redpanda.storage.mode.impl[] property is read-only and can be set only when the topic is created.
...
Data becomes eligible for deletion from object storage following prop:retention.ms[] or prop:retention.bytes[]. For example, if prop:retention.bytes[] is set to 10 GiB, then every partition in the topic has a limit of 10 GiB in object storage. When prop:retention.bytes[] is exceeded by data in object storage, the data in object storage is trimmed. If neither prop:retention.ms[] nor prop:retention.bytes[] is specified, then cluster-level defaults are used.
...
When you set xref:reference:properties/topic-properties.adoc#cleanup-policy[`cleanup.policy`] for a topic to `compact`, nothing gets deleted from object storage based on retention settings. When set to `compact,delete`, compacted segments are deleted from object storage based on prop:retention.ms[] and prop:retention.bytes[].

For compacted topics that use <<tiered-storage-versions,Tiered Storage v1>>, Redpanda compacts segments locally and re-uploads them to object storage. Redpanda initially uploads all uncompacted segments. It then re-uploads the segments with compaction applied. Because compaction runs on local data only, it's likely that some segments in object storage are not compacted, but the Tiered Storage read path can manage this.
...
You can use properties to control retention of topic data in local storage. With Tiered Storage enabled, data in local storage expires after the topic-level properties prop:retention.local.target.ms[] or prop:retention.local.target.bytes[]. (These settings are equivalent to prop:retention.ms[] and prop:retention.bytes[] without Tiered Storage.)

You can also use the cluster-level properties prop:retention_local_target_ms_default[] and prop:retention_local_target_bytes_default[]. Settings can depend on the size of your drive, how many partitions you have, and how much data you keep for each partition.
...
* If prop:retention.ms[] or prop:retention.bytes[] is set, Redpanda initializes the `retention.local.target.*` properties.
* If prop:retention.local.target.ms[] or prop:retention.local.target.bytes[] is set, Redpanda initializes the `min(retention.bytes, retention.local.target.bytes)` and `max(retention.ms, retention.local.target.ms)` properties.
* If properties are not specified:
** Starting in version 22.3, new topics use the default retention values of one day for local storage (prop:retention_local_target_ms_default[link=true,helm-path=auto]) and seven days for all storage, including object storage (prop:log_retention_ms[link=true,helm-path=auto]).
...
To see new log segments faster, you can edit the prop:segment.bytes[] topic-level property. Or, you can edit the xref:reference:properties/object-storage-properties.adoc#cloud_storage_segment_max_upload_interval_sec[`cloud_storage_segment_max_upload_interval_sec`] property, which sets the frequency with which new segments are generated in Tiered Storage, even if the local segment has not exceeded the prop:segment.bytes[] size or the prop:segment.ms[] age.
====
...
Use the xref:reference:properties/object-storage-properties.adoc#cloud_storage_segment_max_upload_interval_sec[`cloud_storage_segment_max_upload_interval_sec`] property to set the number of seconds for idle timeout. If this property is set to null, Redpanda uploads metadata to object storage, but the segment is not uploaded until it reaches the prop:segment.bytes[] size.
...
By default, the cache directory is created in Redpanda's data directory, but it can be placed anywhere in the system. For example, you might want to put the cache directory on a dedicated drive with cheaper storage. Use the xref:reference:properties/broker-properties.adoc#cloud_storage_cache_directory[`cloud_storage_cache_directory`] broker property to specify a different location for the cache directory. You must specify the full path.
...
Redpanda checks the cache periodically according to the interval set in prop:cloud_storage_cache_check_interval_ms[link=true,helm-path=auto]. If the size of the stored data exceeds the configured limit, the eviction process starts. This process removes segments that haven't been accessed recently until the cache size drops to the target level.

*Related properties*:

- prop:cloud_storage_cache_size_percent[link=true,helm-path=auto]
- prop:cloud_storage_cache_max_objects[link=true,helm-path=auto]
- prop:cloud_storage_cache_size[link=true,helm-path=auto]

*Recommendation*: By default, prop:cloud_storage_cache_size_percent[] is tuned for a shared disk configuration. If you are using a dedicated cache disk, consider increasing this value.
...
- prop:cloud_storage_cache_trim_threshold_percent_objects[link=true,helm-path=auto]
- prop:cloud_storage_cache_trim_threshold_percent_size[link=true,helm-path=auto]
- prop:cloud_storage_cache_trim_walk_concurrency[link=true,helm-path=auto]

*Recommendations*:

- A threshold of 70% is recommended for most use cases. This percentage helps balance optimal cache use and ensures the cache has enough free space to handle sudden spikes in data without blocking reads. For example, setting prop:cloud_storage_cache_trim_threshold_percent_size[] to 80% means that the cache trimming process starts when the cache takes up 80% of the maximum cache size.
...
To support more concurrent consumers of historical data with less local storage, Redpanda can download small chunks of remote segments to the cache directory. For example, when a client fetch request spans a subsection of a 1 GiB segment, instead of downloading the entire 1 GiB segment, Redpanda can download 16 MiB chunks that contain just enough data required to fulfill the fetch request. Use the prop:cloud_storage_cache_chunk_size[link=true,helm-path=auto] property to define the size of the chunks.
...
| prop:cloud_storage_cache_check_interval_ms[link=true,helm-path=auto]
...
| prop:cloud_storage_cache_chunk_size[link=true,helm-path=auto]
| The size of a chunk downloaded into object storage cache. Default is 16 MiB.

| prop:cloud_storage_cache_directory[link=true,helm-path=auto]
| The directory where the cache archive is stored.

| prop:cloud_storage_cache_max_objects[link=true,helm-path=auto]
| Maximum number of objects that may be held in the Tiered Storage cache. This applies simultaneously with prop:cloud_storage_cache_size[], and whichever limit is hit first will trigger trimming of the cache.

| prop:cloud_storage_cache_num_buckets[link=true,helm-path=auto]
...
If both this property and prop:cloud_storage_cache_size[] are set, Redpanda uses the minimum of the two.

| prop:cloud_storage_cache_size[link=true,helm-path=auto]
| Maximum size (in bytes) of the disk cache used by Tiered Storage. +
If both this property and prop:cloud_storage_cache_size_percent[] are set, Redpanda uses the minimum of the two.
...
| prop:cloud_storage_chunk_eviction_strategy[link=true,helm-path=auto]
| Strategy for evicting unused cache chunks, either `eager` (default), `capped`, or `predictive`.

| prop:cloud_storage_disable_chunk_reads[link=true,helm-path=auto]
| Flag to turn off chunk-based reads and enable full-segment downloads. Default is false.

| prop:cloud_storage_hydrated_chunks_per_segment_ratio[link=true,helm-path=auto]
...
| prop:cloud_storage_min_chunks_per_segment_threshold[link=true,helm-path=auto]
...
Redpanda always uses exponential backoff with cloud connections. You can configure the prop:cloud_storage_initial_backoff_ms[link=true,helm-path=auto] property to set the time used as an initial backoff interval in the exponential backoff algorithm to handle an error. Default is 100 ms.
...
| prop:cloud_storage_max_connections[link=true,helm-path=auto]
...
| prop:cloud_storage_segment_upload_timeout_ms[link=true,helm-path=auto]
| Timeout for segment upload. Redpanda retries the upload after the timeout. Default is 30000 ms.

| prop:cloud_storage_manifest_upload_timeout_ms[link=true,helm-path=auto]
| Timeout for manifest upload. Redpanda retries the upload after the timeout. Default is 10000 ms.

| prop:cloud_storage_max_connection_idle_time_ms[link=true,helm-path=auto]
...
| prop:cloud_storage_segment_max_upload_interval_sec[link=true,helm-path=auto]
| The number of seconds for idle timeout. If this property is empty, Redpanda uploads metadata to the object storage, but the segment is not uploaded until it reaches the prop:segment.bytes[] size. By default, the property is empty.

| prop:cloud_storage_trust_file[link=true,helm-path=auto]
...
To determine when the system is idle for housekeeping, Redpanda constantly calculates object storage utilization using the moving average with a sliding window algorithm. The width of the sliding window is defined by the prop:cloud_storage_idle_timeout_ms[link=true,helm-path=auto] property, which has a default of 10 seconds. If the utilization (requests per second) drops below the threshold, then object storage is considered idle, and object storage housekeeping begins. The threshold is defined by the prop:cloud_storage_idle_threshold_rps[link=true,helm-path=auto] property, which has a default of one request per second. Object storage is considered idle if, during the last 10 seconds, there were 10 or less object storage API requests.

If the object storage API becomes active after housekeeping begins, then housekeeping is paused until it becomes idle again. If object storage is not idle for prop:cloud_storage_housekeeping_interval_ms[link=true,helm-path=auto], then housekeeping is forced to run until it completes. This guarantees that all housekeeping jobs are run once for every prop:cloud_storage_housekeeping_interval_ms[].

See also: xref:manage:cluster-maintenance/disk-utilization.adoc#space-management[Space management]

=== Adjacent segment merging

By default, and as part of this object storage housekeeping, Redpanda runs adjacent segment merging on all segments in object storage that are smaller than the threshold. Two properties control the behavior of prop:cloud_storage_enable_segment_merging[link=true,helm-path=auto]:

* prop:cloud_storage_segment_size_target[link=true,helm-path=auto]: The desired segment size in object storage. The default segment size is the local segment size for the topic, which is controlled by the prop:log_segment_size[link=true,helm-path=auto] configuration property and the prop:segment.bytes[] topic property. You can set the prop:cloud_storage_segment_size_target[] property to a value larger than the default segment size, but because this triggers a lot of segment re-uploads, it's not recommended.
* prop:cloud_storage_segment_size_min[link=true,helm-path=auto]: The smallest segment size that Redpanda keeps in object storage. The default is 50% of prop:log_segment_size[link=true,helm-path=auto].

If the adjacent segment merging job finds a run of small segments, it can perform one of the following operations:

* Merge and re-upload a segment with a size up to prop:cloud_storage_segment_size_target[].
* Merge and re-upload a segment with a size less than prop:cloud_storage_segment_size_min[] if there are no other options (the run of small segments is followed by the large segment).
* Wait until new segments are added if the run is at the end of the partition.

Suppose the prop:cloud_storage_segment_max_upload_interval_sec[link=true,helm-path=auto] property is set and the partition contains a large number of small segments. For example, if `sec` is set to 10 minutes and the produce rate is 1 MiB per minute, then Redpanda uploads a new 10 MiB segment every 10 minutes. If adjacent segment merging is enabled and prop:cloud_storage_segment_size_target[] is set to 500 MiB, then every 50 segments are re-uploaded as one large 500 MiB segment. This doubles the amount of data that Redpanda uploads to object storage, but it also reduces the memory footprint of the partition, which results in better scalability because 98% less memory is needed to keep information about the uploaded segment.
...
| prop:cloud_storage_spillover_manifest_size[link=true,helm-path=auto] 
...
| prop:cloud_storage_spillover_manifest_max_segments[link=true,helm-path=auto]
| Triggers the upload of metadata to object storage when the number of segments in the spillover manifest exceeds this value. Redpanda recommends using this property only in testing scenarios, and prop:cloud_storage_spillover_manifest_size[] in production settings. (Default: null)

| prop:cloud_storage_manifest_cache_size[link=true,helm-path=auto] 
...
| prop:cloud_storage_enabled[link=true,helm-path=auto]
| Global property that enables Tiered Storage.

Set to true to enable Tiered Storage. Default is false.

| prop:cloud_storage_region[link=true,helm-path=auto]
| Object storage region.

Required for AWS and GCS.

| prop:cloud_storage_bucket[link=true,helm-path=auto]
| AWS or GCS bucket name.

Required for AWS and GCS.

| prop:cloud_storage_credentials_source[link=true,helm-path=auto]
| AWS or GCS instance metadata.

Required for AWS and GCS authentication with IAM roles.

| prop:cloud_storage_access_key[link=true,helm-path=auto]
| AWS or GCS access key.

Required for AWS and GCS authentication with access keys.

| prop:cloud_storage_secret_key[link=true,helm-path=auto]
| AWS or GCS secret key.

Required for AWS and GCS authentication with access keys.

| prop:cloud_storage_api_endpoint[link=true,helm-path=auto]
a|
...
| prop:cloud_storage_azure_container[link=true,helm-path=auto]
| Azure container name.

Required for ABS/ADLS.

| prop:cloud_storage_azure_storage_account[link=true,helm-path=auto]
| Azure account name.

Required for ABS/ADLS.

| prop:cloud_storage_azure_shared_key[link=true,helm-path=auto]
...
| prop:disk_reservation_percent[link=true,helm-path=auto]
| Amount of disk space (as a percentage) reserved for general system overhead.

Default is 20%.

| prop:retention_local_target_capacity_bytes[link=true,helm-path=auto]
| Target size (in bytes) of the log data.

Default is not set (null).

| prop:retention_local_target_capacity_percent[link=true,helm-path=auto]
| Target size (as a percentage) of the log data.

Default is the amount of remaining disk space after deducting prop:cloud_storage_cache_size_percent[] and prop:disk_reservation_percent[].

| prop:retention_local_strict[link=true,helm-path=auto]
...
| prop:retention_local_trim_interval[link=true,helm-path=auto]
...
| prop:cloud_storage_segment_max_upload_interval_sec[link=true,helm-path=auto]
| Sets the number of seconds for idle timeout. If this property is empty, Redpanda uploads metadata to the object storage, but the segment is not uploaded until it reaches the prop:segment.bytes[] size. By default, the property is empty.
...
| prop:cloud_storage_idle_timeout_ms[link=true,helm-path=auto]
...
| prop:cloud_storage_idle_threshold_rps[link=true,helm-path=auto]
...
| prop:cloud_storage_enable_segment_merging[link=true,helm-path=auto]
| Enables adjacent segment merging on all segments in object storage that are smaller than the threshold. Two properties control this behavior: prop:cloud_storage_segment_size_target[link=true,helm-path=auto] and prop:cloud_storage_segment_size_min[link=true,helm-path=auto]. Default is enabled.

| prop:cloud_storage_segment_size_target[link=true,helm-path=auto]
| The desired segment size in object storage. The default segment size is controlled by prop:log_segment_size[link=true,helm-path=auto] and the prop:segment.bytes[] topic configuration property. This property can be set to a value larger than this default segment size, but because that triggers a lot of segment reuploads, it's not recommended.

| prop:cloud_storage_segment_size_min[link=true,helm-path=auto]
...
- https://www.redpanda.com/blog/tiered-storage-architecture-deep-dive[How we built Tiered Storage to supercharge storage and data streaming^]
