...
NOTE: prop:cloud_storage_enable_remote_write[] and prop:cloud_storage_enable_remote_read[] act as creation-time defaults only for topics with `redpanda.storage.mode=unset`. They have no effect on topics where prop:redpanda.storage.mode[] is explicitly set to `local`, `tiered`, or `cloud`. To apply a default storage mode to all new topics, use the config_ref:default_redpanda_storage_mode,true,properties/cluster-properties[] cluster property instead.
...
The `tiered` value of the prop:redpanda.storage.mode[] topic property is an alias. When a topic is created, it resolves to the version selected by the config_ref:default_redpanda_storage_mode_tiered_impl,true,properties/object-storage-properties[] cluster property, which defaults to `tiered_v1`. To make all new `tiered` topics use Tiered Storage v2:
...
If the object storage API becomes active after housekeeping begins, then housekeeping is paused until it becomes idle again. If object storage is not idle for config_ref:cloud_storage_housekeeping_interval_ms,true,properties/object-storage-properties[], then housekeeping is forced to run until it completes. This guarantees that all housekeeping jobs are run once for every prop:cloud_storage_housekeeping_interval_ms[].
...
* config_ref:cloud_storage_segment_size_target,true,properties/object-storage-properties[]: The desired segment size in object storage. The default segment size is the local segment size for the topic, which is controlled by the config_ref:log_segment_size,true,cluster-properties[] configuration property and the prop:segment.bytes[] topic property. You can set the prop:cloud_storage_segment_size_target[] property to a value larger than the default segment size, but because this triggers a lot of segment re-uploads, it's not recommended.
...
| config_ref:cloud_storage_spillover_manifest_size,true,properties/object-storage-properties[`cloud_storage_spillover_manifest_size`] 
...
| config_ref:cloud_storage_spillover_manifest_max_segments,true,properties/object-storage-properties[`cloud_storage_spillover_manifest_max_segments`]
...
| config_ref:cloud_storage_manifest_cache_size,true,properties/object-storage-properties[`cloud_storage_manifest_cache_size`] 
...
| The desired segment size in object storage. The default segment size is controlled by config_ref:log_segment_size,true,cluster-properties[] and the prop:segment.bytes[] topic configuration property. This property can be set to a value larger than this default segment size, but because that triggers a lot of segment reuploads, it's not recommended.
