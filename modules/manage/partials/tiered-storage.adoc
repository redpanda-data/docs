...
:data-archiving-link: manage:kubernetes/storage/tiered-storage/k-data-archiving.adoc
endif::[]
...
[%collapsible]
====
To avoid exposing sensitive data in your configuration files, you can also use a Kubernetes Secret. For example:
...
. Update your Redpanda configuration to include the secret reference for the shared key:
+
[source,yaml]
----
storage:
...
To configure access to an Azure container with a managed identity in AKS:
...
. Update your Redpanda configuration to include the secret reference for the secret key:
+
[source,yaml]
----
storage:
...
. To enable Tiered Storage, set config_ref:cloud_storage_enabled,true,object-storage-properties[] to `true`.
...
* config_ref:cloud_storage_enable_remote_write,true,object-storage-properties[]
* config_ref:cloud_storage_enable_remote_read,true,object-storage-properties[]
...
TIP: A constant stream of data is necessary to build up the log segments and roll them into object storage. To see new log segments faster, you can edit the `segment.bytes` topic-level property. Or, you can edit the config_ref:cloud_storage_segment_max_upload_interval_sec,true,object-storage-properties[] cluster-level property, which sets the frequency with which new segments are generated in Tiered Storage, even if the local segment has not exceeded the `segment.bytes` size or the `segment.ms` age.
...
Use the config_ref:cloud_storage_segment_max_upload_interval_sec,true,object-storage-properties[] property to set the number of seconds for idle timeout. If this property is empty, Redpanda uploads metadata to object storage, but the segment is not uploaded until it reaches the `segment.bytes` size. By default, the property is null.
...
| config_ref:cloud_storage_cache_check_interval,true,object-storage-properties[]
...
| config_ref:cloud_storage_cache_chunk_size,true,object-storage-properties[]
| The size of a chunk downloaded into object storage cache. Default is 16 MiB.

| config_ref:cloud_storage_cache_size_percent,true,object-storage-properties[]
| Maximum size (as a percentage) of the disk cache used by Tiered Storage. + \nIf both this property and `cloud_storage_cache_size` are set, Redpanda honors the minimum of the two. 

| config_ref:cloud_storage_cache_size,true,object-storage-properties[]
| Maximum size (in bytes) of the disk cache used by Tiered Storage. + \nIf both this property and `cloud_storage_cache_size_percent` are set, Redpanda honors the minimum of the two.

| config_ref:cloud_storage_chunk_eviction_strategy,true,object-storage-properties[]
| Strategy for evicting unused cache chunks, either `eager` (default), `capped`, or `predictive`.

| config_ref:cloud_storage_disable_chunk_reads,true,object-storage-properties[]
| Flag to turn off chunk-based reads and enable full-segment downloads.

| config_ref:cloud_storage_hydrated_chunks_per_segment_ratio,true,object-storage-properties[]
...
| config_ref:cloud_storage_min_chunks_per_segment_threshold,true,object-storage-properties[]
...
Redpanda always uses exponential backoff with cloud connections. You can configure the config_ref:cloud_storage_initial_backoff_ms,true,object-storage-properties[] property to set the time used as an initial backoff interval in the exponential backoff algorithm to handle an error. Default is 100 ms.
...
| config_ref:cloud_storage_max_connections,true,object-storage-properties[]
...
| config_ref:cloud_storage_segment_upload_timeout_ms,true,object-storage-properties[]
| Timeout for segment upload. Redpanda retries the upload after the timeout. Default is 30000 ms.

| config_ref:cloud_storage_manifest_upload_timeout_ms,true,object-storage-properties[]
| Timeout for manifest upload. Redpanda retries the upload after the timeout. Default is 10000 ms.

| config_ref:cloud_storage_max_connection_idle_time_ms,true,object-storage-properties[]
...
| config_ref:cloud_storage_segment_max_upload_interval_sec,true,object-storage-properties[]
...
| config_ref:cloud_storage_trust_file,true,object-storage-properties[]
...
To determine when the system is idle for housekeeping, Redpanda constantly calculates object storage utilization using the moving average with a sliding window algorithm. The width of the sliding window is defined by the config_ref:cloud_storage_idle_timeout_ms,true,object-storage-properties[] property, which has a default of 10 seconds. If the utilization (requests per second) drops below the threshold, then object storage is considered idle, and object storage housekeeping begins. The threshold is defined by the config_ref:cloud_storage_idle_threshold_rps,true,object-storage-properties[] property, which has a default of one request per second. Object storage is considered idle if, during the last 10 seconds, there were 10 or less object storage API requests.

If the object storage API becomes active after housekeeping begins, then housekeeping is paused until it becomes idle again. If object storage is not idle for config_ref:cloud_storage_housekeeping_interval_ms,true,object-storage-properties[], then housekeeping is forced to run until it completes. This guarantees that all housekeeping jobs are run once for every `cloud_storage_housekeeping_interval_ms`.

See also: xref:manage:cluster-maintenance/disk-utilization.adoc#space-management[Space management]

=== Adjacent segment merging

By default, and as part of this object storage housekeeping, Redpanda runs adjacent segment merging on all segments in object storage that are smaller than the threshold. Two properties control the behavior of config_ref:cloud_storage_enable_segment_merging,true,object-storage-properties[]:

* config_ref:cloud_storage_segment_size_target,true,object-storage-properties[]: The desired segment size in object storage. The default segment size is the local segment size for the topic, which is controlled by the config_ref:log_segment_size,true,tunable-properties[] configuration property and the `segment.bytes` topic property. You can set the `cloud_storage_segment_size_target` property to a value larger than the default segment size, but because this triggers a lot of segment re-uploads, it's not recommended.
* config_ref:cloud_storage_segment_size_min,true,object-storage-properties[]: The smallest segment size that Redpanda keeps in object storage. The default is 50% of config_ref:log_segment_size,true,tunable-properties[].
...
Suppose the config_ref:cloud_storage_segment_max_upload_interval_sec,true,object-storage-properties[] property is set and the partition contains a large number of small segments. For example, if `cloud_storage_segment_max_upload_interval_ms` is set to 10 minutes and the produce rate is 1 MiB per minute, then Redpanda uploads a new 10 MiB segment every 10 minutes. If adjacent segment merging is enabled and `cloud_storage_segment_size_target` is set to 500 MiB, then every 50 segments are re-uploaded as one large 500 MiB segment. This doubles the amount of data that Redpanda uploads to object storage, but it also reduces the memory footprint of the partition, which results in better scalability because 98% less memory is needed to keep information about the uploaded segment.
...
| config_ref:cloud_storage_enabled,true,object-storage-properties[]
| Global property that enables Tiered Storage.

Set to true to enable Tiered Storage. Default is false.

| config_ref:cloud_storage_region,true,object-storage-properties[]
| Object storage region.

Required for AWS and GCS.

| config_ref:cloud_storage_bucket,true,object-storage-properties[]
| AWS or GCS bucket name.

Required for AWS and GCS.

| config_ref:cloud_storage_credentials_source,true,object-storage-properties[]
| AWS or GCS instance metadata.

Required for AWS and GCS authentication with IAM roles.

| config_ref:cloud_storage_access_key,true,object-storage-properties[]
| AWS or GCS access key.

Required for AWS and GCS authentication with access keys.

| config_ref:cloud_storage_secret_key,true,object-storage-properties[]
| AWS or GCS secret key.

Required for AWS and GCS authentication with access keys.

| config_ref:cloud_storage_api_endpoint,true,object-storage-properties[]
a|
...
| config_ref:cloud_storage_azure_container,true,object-storage-properties[]
| Azure container name.

Required for ABS/ADLS.

| config_ref:cloud_storage_azure_storage_account,true,object-storage-properties[]
| Azure account name.

Required for ABS/ADLS.

| config_ref:cloud_storage_azure_shared_key,true,object-storage-properties[]
| Azure shared key.
...
| config_ref:cloud_storage_initial_backoff_ms,true,object-storage-properties[]
...
| config_ref:cloud_storage_idle_timeout_ms,true,object-storage-properties[]
...
| config_ref:cloud_storage_idle_threshold_rps,true,object-storage-properties[]
...
| config_ref:cloud_storage_enable_segment_merging,true,object-storage-properties[]
| Enables adjacent segment merging on all segments in object storage that are smaller than the threshold. Two properties control this behavior: config_ref:cloud_storage_segment_size_target,true,object-storage-properties[] and config_ref:cloud_storage_segment_size_min,true,object-storage-properties[]. Default is enabled.

| config_ref:cloud_storage_segment_size_target,true,object-storage-properties[]
...
| config_ref:cloud_storage_segment_size_min,true,object-storage-properties[]
...
| config_ref:cloud_storage_upload_ctrl_update_interval_ms,true,object-storage-properties[]
| The recompute interval for the upload controller. Default is 60000 ms.

| config_ref:cloud_storage_upload_ctrl_p_coeff,true,object-storage-properties[]
| The proportional coefficient for the upload controller. Default is -2.

| config_ref:cloud_storage_upload_ctrl_d_coeff,true,object-storage-properties[]
| The derivative coefficient for the upload controller. Default is 0.

| config_ref:cloud_storage_upload_ctrl_min_shares,true,object-storage-properties[]
| The minimum number of I/O and CPU shares that the remote write process can use. Default is 100.

| config_ref:cloud_storage_upload_ctrl_max_shares,true,object-storage-properties[]
| The maximum number of I/O and CPU shares that the remote write process can use. Default is 1000.

| config_ref:cloud_storage_disable_tls,true,object-storage-properties[]
| Disables TLS encryption. Set to true if TLS termination is done by the proxy, such as HAProxy. Default is false.

| config_ref:cloud_storage_api_endpoint_port,true,object-storage-properties[]
| Overrides the default API endpoint port. Default is 443.

| config_ref:cloud_storage_trust_file,true,object-storage-properties[]
| The public certificate used to validate the TLS connection to object storage. If this is empty, Redpanda uses your operating system's CA cert pool.

| config_ref:cloud_storage_reconciliation_interval_ms,true,object-storage-properties[]
| Deprecated.
