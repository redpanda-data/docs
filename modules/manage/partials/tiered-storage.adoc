...
With tiered storage enabled, you can restore your cluster, including its metadata, in the event of failure. 

=== Cluster metadata recovery

CLuster metadata can be restored to a target cluster using an rpk command. Metadata includes topic definitions, users, ACLS, schemas, consumer offsets, and all cluster configuration, including license key.
