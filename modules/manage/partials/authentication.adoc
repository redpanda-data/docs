...
IMPORTANT: Enabling authorization without a superuser can lock you out of the cluster, as you would not have the necessary permissions to manage the cluster's settings or users.
...
ifdef::env-kubernetes[]
To enable mTLS authentication, set `authenticationMethod` to `mtls_identity` for a listener that has mTLS enabled (`requireClientAuth: true`). For example, to enable mTLS authentication for the internal Kafka API listener:
[source,yaml]
----
listeners:
  kafka:
...
# Schema Registry listener configuration
listeners:
  schemaRegistry:
    enabled: true
    port: 8081
...
# HTTP Proxy (PandaProxy) listener configuration
listeners:
  http:
    enabled: true
    port: 8082
