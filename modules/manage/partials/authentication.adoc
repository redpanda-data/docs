...
- A superuser is a privileged principal who has unrestricted access to xref:manage:security/authorization/index.adoc#operations[all operations] in a Redpanda cluster.
...
When you enable authentication with `auth.sasl.enabled: true`, a default bootstrap user is created automatically at cluster formation time. This ensures your cluster is "secure by default" with authentication enforced from the first startup.

To deploy a cluster with your own bootstrap user credentials instead of the default, configure `auth.sasl.bootstrapUser`. This creates an initial superuser with your specified username and password before the cluster accepts client connections. The user is automatically added to the `superusers` list and can be used to create additional users and configure ACLs.

==== When to customize bootstrap user credentials

Customize the bootstrap user configuration when:

- You need to control the initial superuser credentials for security compliance
- You're automating cluster deployments with infrastructure-as-code
- You want to use your organization's credential management system
- You need specific SASL mechanism requirements (SCRAM-SHA-256 vs SCRAM-SHA-512)
...
----
+
The chart requires this Secret when `secretRef` is specified, even when `users` is empty. The name `redpanda-superusers` is used in these examples because it matches the value in `auth.sasl.secretRef`.
  ...
  clusterSpec:
    auth:
      sasl:
        ...
        users: []
        bootstrapUser:
          name: "admin-bootstrap"  <1>
          secretKeyRef:
            name: "bootstrap-user-secret"
            key: "password"
          mechanism: "SCRAM-SHA-512"  <2>
----
<1> Optional. The username for the bootstrap superuser. Defaults to `kubernetes-controller` if omitted.
...
helm upgrade --install redpanda redpanda/redpanda --namespace <namespace> --create-namespace \
  --set auth.sasl.enabled=true \
  --set auth.sasl.secretRef=redpanda-superusers \
  --set auth.sasl.bootstrapUser.name=admin-bootstrap \
...
======
+
NOTE: The configuration uses two different Secret references: `auth.sasl.secretRef` points to the Secret containing SASL user credentials (typically `redpanda-superusers`), while `auth.sasl.bootstrapUser.secretKeyRef` points to the Secret containing the bootstrap user's password.
...
==== Combine with additional security settings

For complete "secure by default" configuration beyond authentication, configure additional security settings like `admin_api_require_auth: true` using the `config.cluster` section in your Helm values. For the bare-metal equivalent of bootstrap configuration using `.bootstrap.yaml` files, see xref:deploy:redpanda/manual/production/production-deployment.adoc#configure-a-bootstrap-file[Configure a bootstrap file].

The Helm chart automatically manages the `RP_BOOTSTRAP_USER` environment variable based on your `auth.sasl.bootstrapUser` configuration. Redpanda reads this environment variable only on first startup to create the initial user.
...
* *prop:enable_sasl[]*: A cluster-configuration property (set with `rpk cluster config set enable_sasl true`). A legacy (not deprecated) approach maintained for backwards compatibility.
...
|===
|Use case |Recommended authentication method |Reasons to use

|When creating new clusters
|Use per-listener configuration
|Lets you configure authentication per listener, which is useful for mixed environments where some listeners are internal (no authentication required) and others are external (authentication required).

|When you have existing clusters already using prop:enable_sasl[]
|Use prop:enable_sasl[]
|You can continue using prop:enable_sasl[] if it meets your needs. You only need to migrate to per-listener configuration if you need per-listener authentication control.
...
Using both prop:enable_sasl[] and prop:kafka_enable_authorization[] + `authentication_method` together can lead to configuration inconsistencies and initialization failures. Choose one approach and use it consistently.
====
...
Using the prop:enable_sasl[] option applies SASL authentication to all Kafka API listeners with a single command.

Use this option when:

* All Kafka listeners use SASL authentication.
...
To use the prop:enable_sasl[] option, run:
...
* Enables SASL authentication on _all_ Kafka API listeners
* Implicitly enables authorization (prop:kafka_enable_authorization[] is set to `true`)

[WARNING]
====
When using this method, do *not* additionally set `kafka_enable_authorization: true` or add `authentication_method: sasl` to `redpanda.yaml`. The prop:enable_sasl[] setting handles both automatically.
====
...
Use the per-listener authentication option when:
...
2. Configure the authentication method for each listener in `redpanda.yaml`:
+
[,yaml]
----
redpanda:
...
[WARNING]
====
When using this method, do _not_ set `enable_sasl: true`. The per-listener configuration and explicit prop:kafka_enable_authorization[] handle authentication and authorization separately.
====
...
*If you used prop:enable_sasl[]*: SASL is already enabled on all Kafka listeners automatically. Skip this section.
...
----
+
When HTTP Proxy API listeners use xref:reference:properties/broker-properties.adoc#http_proxy_auth_method[`authentication_method`]: `none`, the HTTP Proxy client uses these credentials to authenticate with the Kafka API (required starting in Redpanda 25.2). The user specified in prop:scram_username[] must have appropriate permissions to access the required Kafka resources.
+
...
To enable SASL/PLAIN authentication for the Kafka API, set the xref:reference:properties/broker-properties.adoc#kafka_api_auth_method[`authentication_method`] broker property of the Kafka listeners to `sasl`.
...
NOTE: OpenID Connect (OIDC) authentication requires an xref:get-started:licensing/index.adoc[enterprise license].
...
* `aud`:  Audience. Must match the configuration specified in prop:oidc_token_audience[]. Cannot contain the value `none`.
* `kid`: Key identifier. Must match _any_ of the public JWK listed in the `jwks_uri` endpoint.
* `exp`: Expiration. The timestamp listed is greater than current time. Must validate within acceptable bounds of the value specified in prop:oidc_clock_skew_tolerance[]. A clock skew tolerance period may be configured by an Admin to account for clock drift between Redpanda and the OIDC Identity Provider (IdP).
* `iss`: Issuer. Must exactly match the `issuer` property of the JSON returned from the URL specified in prop:oidc_discovery_url[].
...
* You have an access token from your IdP whose claims satisfy prop:oidc_token_audience[], prop:oidc_discovery_url[], and prop:oidc_principal_mapping[]. For the claims that Redpanda validates, see <<oidc-credentials-flow-and-access-token-validation, OIDC credentials flow and access token validation>>.
...
* The `aud` claim matches prop:oidc_token_audience[]
* The `iss` claim matches the issuer at prop:oidc_discovery_url[]
* `exp` is in the future within prop:oidc_clock_skew_tolerance[] 
...
NOTE: Kerberos authentication requires an xref:get-started:licensing/index.adoc[enterprise license]. To upgrade, contact https://redpanda.com/try-redpanda?section=enterprise-trial[Redpanda sales^].
...
. Set prop:sasl_kerberos_principal_mapping[]. This maps Kerberos user principal names (UPNs) onto Redpanda principals used in the ACLs. For example:
+
[,bash]
----
...
----
+
To add support for Kerberos, append the prop:sasl_mechanisms[] property with the value `GSSAPI`:
+
[,bash]
----
...
To enable mTLS authentication, set xref:reference:properties/broker-properties.adoc#kafka_api_auth_method[`authentication_method`] broker property for a listener to `mtls_identity`. For example, to enable mTLS authentication for the internal Kafka API listener, in `redpanda.yaml`, enter:
...
By default, Redpanda matches the entire DN. To override the default, specify prop:kafka_mtls_principal_mapping_rules[]. This is a list of rules that provide a mapping from DN to principal.
...
To update the prop:kafka_mtls_principal_mapping_rules[] property:
...
Before enabling authentication for the HTTP APIs, you must <<sasl,enable SASL authentication for the Kafka API>>. This creates the credential store that HTTP authentication will use.
...
The HTTP Proxy API and the Schema Registry API use the same credential store as the Kafka API, so you can use the same credentials for all three APIs.
The Admin API uses the same credential store as the Kafka API, but it requires superuser credentials to access it.
...
For all available endpoints, see the link:/api/doc/schema-registry/[Schema Registry API].
...
To enable OIDC for the HTTP API listeners as well as basic authentication, include OIDC in the prop:http_authentication[] cluster property list:
...
To enable OIDC for HTTP API listeners, set xref:reference:properties/broker-properties.adoc#schema_registry_auth_method[`authentication_method`] to `http_basic` to require authentication on those listeners. For example, in `redpanda.yaml`, enter:
...
* What authentication methods are enabled in the xref:reference:properties/cluster-properties.adoc#http_authentication[`http_authentication`] cluster property 
...
When prop:schema_registry_enable_authorization[] is enabled during cluster instability (controller election, restart, heavy load), ACL creation for the internal role may fail partway through, leaving an incomplete role that persists across restarts.
...
* Only enable prop:schema_registry_enable_authorization[] when the cluster is stable.
