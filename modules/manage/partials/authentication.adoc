...
ifndef::env-kubernetes[]
When you <<enable-authentication, enable authentication>> with `enable_sasl=true`, the Schema Registry API and the HTTP Proxy API are configured with basic authentication by default.
...
To enable basic authentication for a specific listener, set `authentication_method` to `http_basic`. For example, in `redpanda.yaml`, enter:
