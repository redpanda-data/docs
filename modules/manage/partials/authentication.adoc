...
If you're configuring authentication for the first time, you may need to
configure xref:manage:security/authorization.adoc#acls[ACLs] before users can access Redpanda resources.
...
+
For Kerberos authentication, Redpanda requires that SASL/SCRAM be enabled for connectivity by `rpk`, Redpanda Console, and other Redpanda products. Operating with Kerberos only is not a supported configuration.
