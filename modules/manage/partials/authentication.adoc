...
Starting with `rpk` v26.1.7 (also available in v25.3.x and v25.2.x patches), `rpk` supports the `OAUTHBEARER` SASL mechanism. After you enable OIDC on a Kafka listener, you can authenticate `rpk` against the Kafka API, Admin API, and Schema Registry with an OIDC access token issued by your IdP, instead of a SASL/SCRAM username and password.
...
* You have an access token from your IdP whose claims satisfy `oidc_token_audience`, `oidc_discovery_url`, and `oidc_principal_mapping`. For the claims that Redpanda validates, see <<oidc, OIDC credentials flow and access token validation>>.
...
You can also pass these values as `-X` options, which is the form used by `rpk` profiles:

* `-X sasl.mechanism=OAUTHBEARER`
* `-X pass="token:$OIDC_TOKEN"`

For details, see xref:reference:rpk/rpk-x-options.adoc#sasl-mechanism[`sasl.mechanism`] and xref:reference:rpk/rpk-x-options.adoc#pass[`pass`].
