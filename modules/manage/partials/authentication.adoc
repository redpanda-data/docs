...
When enabling authentication, you must create at least one superuser. A superuser is a special user that has xref:security/authorization/index.adoc#operations[all permissions] on the cluster. They can grant permissions to other users through access control lists (ACLs).
...
Before enabling authorization in Redpanda, which can happen implicitly when you enable authentication, you must create a superuser. A superuser is a special user that has xref:security/authorization/index.adoc#operations[all permissions] on the cluster. They can grant permissions to other users through access control lists (ACLs).
...
. Use the xref:reference:rpk/rpk-acl/rpk-acl-create.adoc[`rpk security acl create`] command to grant xref:manage:security/authorization.adoc[`create` and `describe` permissions] to `myuser` in the cluster:
+
```bash
...
See also: xref:security/authorization/index.adoc#user-create[User create].
...
. xref:security/authorization/index.adoc[Create appropriate ACLs] for the Schema Registry and HTTP Proxy users to define and restrict their access rights within the Redpanda cluster.
...
. Configure xref:security/authorization/index.adoc#acls[ACLs] for your users so they can access Redpanda resources.
