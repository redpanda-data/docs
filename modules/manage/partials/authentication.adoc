...
credentials, SASL/PLAIN stores plaintext passwords, but requires TLS for external encryption.

If you have existing PLAIN Kafka clients and applications, you can migrate to Redpanda without updating
your application by creating local Redpanda SCRAM accounts and enabling PLAIN as a authentication mechanism.
...
[,bash]
----
rpk cluster config set sasl_mechanisms '["PLAIN"]'
----
...
redpanda:
  kafka_api:
    - address: 0.0.0.0
      ...
      authentication_method: sasl_plain
----
