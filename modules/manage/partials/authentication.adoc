...
ifndef::env-kubernetes[If you <<enable-authentication, enabled authentication with `enable_sasl=true`>>, Redpanda implicitly sets `authentication_method` to `sasl` for the Kafka listeners.]

ifndef::env-kubernetes[If you <<enable-authentication, enabled authentication with `kafka_enable_authorization=true`>>, you must enable SASL for the Kafka API.]
...
SASL/SCRAM is enabled by default.

To check if SASL/SCRAM is enabled:
...
If SASL/SCRAM is not enabled, enable it:
...
This registration process involves creating a new application entry in the IdP's management console.
...
+
NOTE: If you enabled OIDC authentication for the Admin API, you must also <<create-superusers,create a SCRAM superuser>> so that you can use `rpk` to create ACLs. `rpk` supports only basic authentication for the Admin API. See <<Authentication for the HTTP APIs>>.
...
To enable basic authentication for the HTTP Proxy or Schema Registry, set `authentication_method` to `http_basic`. For example, in `redpanda.yaml`, enter:
