...
When you enable mTLS, the Helm chart generates a TLS Secret resource called `redpanda-client` that contains the client's `tls.crt` and `tls.key` files. If you want to use your own TLS files, you can instead provide the Helm chart with your own TLS Secret to use.
