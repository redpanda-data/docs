...
To generate a debug bundle, you can use `rpk` or Redpanda Console.
...
+
```bash
kubectl cp redpanda/redpanda-0:/var/lib/redpanda/<bundle-name> debug-bundle/<bundle-name>.zip
```
endif::[]
...
See xref:console:ui/generate-debug-bundle.adoc[].
