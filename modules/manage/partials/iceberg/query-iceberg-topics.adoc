...
ifdef::env-cloud[]
Redpanda generates an Iceberg table that has the same name as the topic name. Depending on the processing engine and your Iceberg catalog implementation, you may also need to create a table to point your data lakehouse to the table location in the catalog.
