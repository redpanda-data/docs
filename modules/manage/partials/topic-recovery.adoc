...
- xref:{link-tiered-storage}#remote-read[Remote read] (prop:redpanda.remote.read[]) enabled on the topic you want to recover.

== Limitations

- Remote recovery is only safe when no other clusters are writing to the same bucket or container.
- If you disable prop:redpanda.remote.read[] after remote recovery, previously downloaded data will not be used to serve requests.
...
To create a new topic using remote recovery, while also disabling the prop:redpanda.remote.write[] property:
