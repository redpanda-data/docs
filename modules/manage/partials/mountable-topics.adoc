...
ifdef::env-cloud[]
. xref:manage:api/cloud-api-authentication.adoc[Authenticate] to the Cloud API.
. You'll make requests against Data Plane APIs. Make sure you xref:manage:api/cloud-dataplane-api.adoc#get-data-plane-api-url[retrieve the Data Plane API URL] and use this URL to perform a mount or unmount operation.
endif::[]
