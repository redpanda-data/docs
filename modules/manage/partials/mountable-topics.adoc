...
ifdef::env-cloud[]
. xref:manage:rpk/rpk-install.adoc[Install `rpk`] to run cluster mount and unmount operations on the command line or
. xref:manage:api/cloud-api-authentication.adoc[Authenticate] to the Cloud API and make requests against the Data Plane APIs. xref:manage:api/cloud-dataplane-api.adoc#get-data-plane-api-url[Retrieve the Data Plane API URL] and use it to perform a mount or unmount operation.
endif::[]
