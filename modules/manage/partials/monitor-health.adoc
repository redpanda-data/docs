...
- <<dedicated-gauges, Dedicated gauges>>: Redpanda can internally calculate consumer group lag and expose it as two dedicated gauges. These gauges simplify monitoring by providing direct lag metrics without the need for complex calculations. This method is recommended for environments where observability platforms may not support complex queries.
- <<offset-based-calculation, Offset-based calculation>>: You can use your observability platform to calculate consumer group lag by combining two offset metrics: `redpanda_kafka_max_offset` and `redpanda_kafka_consumer_group_committed_offset`. This method requires complex queries to calculate lag and may not be supported in all observability platforms.
...
To have these dedicated gauges available, you must enable consumer group metrics in your cluster properties. Add the following settings to your Redpanda configuration:
