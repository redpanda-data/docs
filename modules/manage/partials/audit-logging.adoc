...
|User connect and disconnect time |No |Connection and disconnection timestamps are logged only when explicit connection events are requested; disconnection time may be inferred from session timeout or absence of activity.
...
|VLAN ID |No |Not included
|MAC address |No |Not included
|===
