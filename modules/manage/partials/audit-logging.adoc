...
IMPORTANT: You must configure `auditLogging.partitions` before enabling audit logging because this settings impacts the creation of the `_redpanda.audit_log` topic itself. The Kafka API allows you to add partitions after enabling audit logging, but Redpanda prevents you from altering this configuration directly.
endif::[]
...
TIP: If you want to manage the audit topic using a xref:manage:kubernetes/Topic resource, create the Topic resource first, and make sure to set `overwriteTopicName` to `_redpanda.audit_log`.
...
--
+
For details, see xref:manage:kubernetes/security/authentication/sasl-kubernetes[].

- `auditLogging.enabled`: This is a boolean value to enable audit logging. When you set this to `true`, Redpanda checks for an existing topic named `_redpanda.audit_log`. If the topic is not found, Redpanda automatically creates one for you. Default: `false`.
