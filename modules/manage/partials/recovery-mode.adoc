...
ifndef::env-kubernetes[]
. Run the xref:reference:rpk/rpk-redpanda/rpk-redpanda-mode.adoc[`rpk redpanda mode recovery`] command to set the prop:recovery_mode_enabled[] broker configuration property to `true`.
+
[,bash]
----
...
Problems that prevent normal cluster startup may be isolated to certain partitions or topics. You can use rpk or the link:/api/doc/admin/group/endpoint-partitions[Admin API] to disable these partitions at the topic level, or individual partition level. A disabled partition or topic returns a `Replica Not Available` error code for Kafka API requests.
