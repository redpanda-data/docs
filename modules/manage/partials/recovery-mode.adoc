...
Problems that prevent normal cluster startup may be isolated to certain partitions or topics. You can use rpk or the link:/api/doc/admin/operation/operation-post_cluster_partitions_topic_partition[Admin API] to disable these partitions at the topic level, or individual partition level. A disabled partition or topic returns a `Replica Not Available` error code for Kafka API requests.
