...
. For *Authentication*, select *HTTP Basic* and enter the *Username* and *Password*. For Confluent Cloud, these are the Schema Registry API key and secret.
. Turn on *Enable TLS* if the source registry uses HTTPS. To trust a private certificate authority, upload a CA certificate. To authenticate with mTLS instead of HTTP basic authentication, provide a client certificate and private key.
...
* Turning off *Redpanda* mode does not remove a `_schemas` shadow topic that the link already added. To stop shadowing that topic, fail over or delete the shadow topic after you save.
...
| Authentication
| The authentication *Type* and *Username*. The *Password* shows as `Set` or `Not set`; Redpanda Console never displays the stored value.
