...
Schema replication settings live in the shadow link configuration. Two cluster properties, prop:schema_registry_sync_memory_bytes[] and prop:schema_registry_sync_parallelism[], tune how much memory and concurrency the shadow cluster uses while importing schemas. The defaults suit most deployments.
====
...
ifndef::env-cloud[]
* To configure schema replication in xref:console:index.adoc[Redpanda Console] rather than with `rpk`, Redpanda Console v3.9.0 or later. The Schema Registry fields appear only when the shadow cluster reports Redpanda 26.2 or later. On an earlier cluster, Redpanda Console falls back to a single toggle that enables `_schemas` topic replication, and API mode is unavailable.
endif::[]
...
image::console:shadow-link-schema-registry.png[alt="The Shadow Schema Registry card in Redpanda Console, with the Other mode tab selected and the source connection, scope, and sync behavior sections visible"]

. From the navigation menu, select *Shadow Links*, then click *Create shadow link*.
. On the *Connection* step, complete the connection details for the source Kafka cluster. See xref:manage:disaster-recovery/shadowing/setup.adoc#create-a-shadow-link[Configure Shadowing].
. On the *Configuration* step, in the *Shadow Schema Registry* card, select the *Other* mode tab. *Other* is API mode. *Redpanda* shadows the `_schemas` topic instead, and *None* leaves the shadow cluster's Schema Registry independent.
...
| `auth_options.basic`, with `username` and `password`, or `auth_options` omitted
