...
+
--
. In your source cluster, create a secret to store the authentication credential that your shadow cluster will use (`"scram_configuration": "password"` in the example configuration below). Your secret must be scoped to "Redpanda Cluster".
+
...
. To create a shadow link with the source cluster using the Control Plane API, make a `POST /shadow-links` request from the shadow cluster:
+
[,bash]
----
