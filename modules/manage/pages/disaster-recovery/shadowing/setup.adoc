...
ifndef::env-cloud[]
// tag::rpk-tab-generate-config-create-shadow-link[]
[TIP]
====
...
ifdef::env-cloud[]
# Generate a sample configuration file with placeholder values
rpk shadow config generate --from-cloud -o shadow-config.yaml

# Or generate a template with detailed field documentation
rpk shadow config generate --from-cloud --print-template -o shadow-config-template.yaml
endif::[]
----
...
cloud_options:
  ...
  resource_group_id: <resource-group-id> # Resource group ID containing the shadow link
endif::[]
client_options:
...
// end::rpk-tab-generate-config-create-shadow-link[]
endif::[]
...
+
--
include::manage:disaster-recovery/shadowing/setup.adoc[tag=rpk-tab-generate-config-create-shadow-link]
--
      ...
      "client_options": {
        ...
        "authentication_configuration": {
          "scram_configuration": {
            ...
            "password": "${secrets.<sasl-password-secret-name>}",
...
* `<resource-group-id>`: Resource group ID for this shadow link. 
...
* `<sasl-password-secret-name>`: The name of the secret containing the SASL/SCRAM password.
...
For the full API reference, see link:/api/doc/cloud-controlplane/v1/operation/operation-shadowlinkservice_createshadowlink[Control Plane API reference].
--
======
endif::[]
...
For the full API reference, see link:/api/doc/cloud-controlplane/v1/operation/operation-shadowlinkservice_updateshadowlink[Control Plane API reference].
--
======
endif::[]
// end::single-source[]
