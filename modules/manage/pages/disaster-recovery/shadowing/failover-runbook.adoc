...
Use xref:reference:rpk/rpk-shadow/rpk-shadow-status.adoc[`rpk shadow status`] or the link:/api/doc/cloud-dataplane/v1/operation/operation-[Data Plane API] to check lag, which shows the message count difference between source and shadow partitions:
...
The `DELETE /v1/shadow-links/<shadow-link-id>` endpoint of the Control Plane API force deletes the shadow link by default in Redpanda Cloud.
--
======
endif::[]
