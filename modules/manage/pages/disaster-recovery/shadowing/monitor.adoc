...
ifndef::env-cloud[]
* **Role sync counts**: For links that replicate RBAC roles, the Roles Migrator task's `Reason` field reports the counts from the most recent sync cycle (roles created, updated, deleted, and failures). See xref:manage:disaster-recovery/shadowing/setup.adoc#role-filtering[Role filtering].
endif::[]
