...
* Different rate limits apply to different apps used by the same user. For example, `alice`pass:['s] `payment-processor` gets 10 MB/s, but `alice`pass:['s] `analytics-consumer` gets 50 MB/s. See <<Quota precedence and tracking>> for examples.
...
Unauthenticated connections have an empty user principal (`user=""`) and is not treated as `user=<default>`.
...
Within each user, all client ID values share that user's bucket. `alice` connecting with client ID `app-1` and `alice` connecting with client ID `app-2` share the same 10 MB/s produce limit, while `bob`pass:['s] connections have a separate 10 MB/s limit.
