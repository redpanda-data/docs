...
* Cache storage (`cloud_storage_cache_size_percent`, or `cloud_storage_cache_size` which overrides the former) is the maximum size of the xref:../tiered-storage.adoc#caching[disk cache] used by Tiered Storage. As the cache reaches its limit, new data added to the cache removes old data from the cache.
