...
It is important to manage the local disk space in Redpanda to ensure the stability of a cluster. If a node's disk space reaches a critically low level then Redpanda will block clients from writing new data, and furthermore, the Redpanda process will terminate if a node runs out of disk space altogether. This will obviously have a significant impact on performance as client connections and topic data concentrates on fewer nodes.

Redpanda provides several ways to manage disk space, with varying degrees of flexibility and control over which data is removed from local disk and when.

* Starting with space management, which when used in conjunction with xref:../tiered-storage.adoc[Tiered Storage], treats the local disks as a cache.
...
* Configurable storage thresholds allow you to set up alerts to notify you when disk space is running out.
* xref:./continuous-data-balancing.adoc[Continuous Data Balancing] can be used to ensure well balanced disk usage across the cluster.
* A ballast file can be created to allow fast recovery from a full disk.
...
NOTE: Space management will only work when xref:../tiered-storage.adoc[Tiered-Storage] is enabled on all topics.

The goal of space management is to utilize the local disk space as a cache. It works alongside xref:../tiered-storage.adoc[Tiered-Storage] to provide faster access to recent data, while making sure that local disk utilization is managed in accordance with retention policies and other processes, such as xref:./continuous-data-balancing.adoc[Continuous Data Balancing] and xref:./decommission-brokers.adoc[Decommissioning]. 
...
** As the disk space used by the other areas grows to their target sizes, this provides a buffer space to avoid free disk space alerts.
** SSDs that run near capacity can experience performance degradation, so this buffer space prevents the disks running at capacity.
* Cache storage (`cloud_storage_cache_size_percent` or `cloud_storage_cache_size`) is the maximum size of the xref:../tiered-storage.adoc#caching[disk cache] used by Tiered Storage. As the cache reaches its limit, new data added to the cache removes old data from the cache.
* Log storage (`retention_local_target_capacity_percent` or `retention_local_target_capacity_bytes`) is the area of disk space used for topic data.
** Typically 70-80% of total disk space.

image::shared:disk_storage.png[Redpanda disk storage categories]

Log segment eviction occurs in each of the following phases, and as soon as log storage usage falls below the target, the eviction process ends.
...
* When `retention_local_strict` is true, the housekeeping process uses topic-level retention settings to select what data to remove.
+
...
This phase removes partition data that has exceed the effective local retention policy. Including the explicit retention settings applied to a topic, as well as the cluster-level defaults, which are assigned to any topic that does not have explicit topic-level overrides.
...
* When `retention_local_strict` is true, the housekeeping process removes data fairly across all topics until each topic has reached its local retention target.

After this phase has completed, all partitions will be operating at a size that reflects their effective local retention target. The next phase starts to override the local retention settings to remove more data.
...
The final _catch-all_ trims all topics down to their last active segment. Data in the active log segment cannot be removed, and will not be available for reclaim until it is rolled, which occurs when it reaches `segment.bytes` or when `segment.ms` time expires.
