...
NOTE: Retention policy applies on each partition in a topic individually. Within a partition, only closed segments are subject to deletion or compaction. When a time-based policy is set, all messages within the segment must exceed the set limit. For example, assume the retention.ms for a topic is 300,000 ms (5 minutes). If the segment.ms is 1,800,000ms (30 minutes)  then messages will remain for a minimum of 30 minutes until the segment containing them expires.
