...
NOTE: Space management only works when xref:../tiered-storage.adoc[Tiered Storage] is enabled on all topics.
...
* Log storage (`retention_local_target_capacity_percent` or `retention_local_target_capacity_bytes`) is the area of disk space used for topic data. This is typically 70-80% of total disk space.
...
The final phase trims all topics down to their last active segment. Data in the active log segment cannot be removed, and is not available for reclaim until it is rolled, which occurs when it reaches `segment.bytes` or when `segment.ms` time expires.
