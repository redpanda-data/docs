...
[NOTE]
====
Make sure that all of the produced data sent to `redpanda` pre-validation checks is correct with respect to having properly set `log_message_timestamp_before_max_ms` and `log_message_timestamp_after_max_ms` in each record batch. If these timestamps are missing or incorrect:
