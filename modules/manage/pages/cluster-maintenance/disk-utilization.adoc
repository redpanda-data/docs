...
By default, all topics on self-hosted Redpanda clusters retain 24 hours of data on local disk. Redpanda Cloud retains 6 hours of data and uses our space management . If data is written fast enough to consume all disk space within the local retention time period, it's possible to exhaust local disk space, even when using Tiered Storage. You can adjust the default local storage size or time period, if required.
