...
When a disk exceeds the configured threshold, Redpanda updates the xref:reference:public-metrics-reference.adoc#redpanda_storage_disk_free_space_alert[`redpanda_storage_disk_free_space_alert`] metric appropriately and writes an error level `storage space alert` message to the service log. The message looks like the following:

[,bash]
----
ERROR 2023-12-08 15:07:45,716 [shard  0] cluster - storage space alert: free space at 25.574% on /var/lib/redpanda/data: 96.732GiB total, 24.739GiB free, min. free 0.000bytes. Please adjust retention policies as needed to avoid running out of space
----
