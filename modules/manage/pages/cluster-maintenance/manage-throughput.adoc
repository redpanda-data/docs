...
You can view current throughput quotas set through the Kafka API by running the xref:reference:rpk/rpk-cluster/rpk-cluster-quotas-describe.adoc[`rpk cluster quotas describe`] command.
...
//TODO: replace with cluster quotas import reference when available 
To more easily manage multiple quotas, you can use the `cluster quotas describe` and xref:reference:rpk/rpk-cluster/rpk-cluster-quotas.adoc[`cluster quotas import`] commands to do a bulk export and update.
...
You may also save the JSON or YAML output to a file and pass the file path in the `--from` flag.
