...
:learning-objective-1: Configure user-based throughput quotas
...
Client-based quotas apply to all users using a specific `client_id`. These quotas do not require authentication. Because `client_id` is self-declared, client-based quotas are not suitable for guaranteeing isolation between tenants.
