...
[NOTE]
====
By default, both prop:kafka_throughput_limit_node_in_bps[] and prop:kafka_throughput_limit_node_out_bps[] are disabled, and no throughput limits are applied. You must manually set them to enable throughput throttling.
====
...
ifdef::env-cloud[]
To enable them, set the prop:kafka_per_entity_quota_metrics[] cluster property. See xref:manage:cluster-maintenance/config-cluster.adoc[].
endif::[]
