...
NOTE: The client group that matches on `client-id-prefix` does not denote a Kafka consumer group. Instead, it matches all requests with a `client_id` field that has the configured prefix. The `client_id` field is typically a configurable parameter in kafka libraries when the client is constructed.
