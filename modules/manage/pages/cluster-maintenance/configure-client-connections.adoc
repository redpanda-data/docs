...
* xref:reference:properties/cluster-properties.adoc#kafka_connections_max_overrides[`kafka_connections_max_overrides`]: A list of IP addresses for which prop:kafka_connections_max_per_ip[] is overridden and doesn't apply.
ifndef::env-cloud[]
...
* xref:reference:properties/cluster-properties.adoc#kafka_connection_rate_limit_overrides[`kafka_connection_rate_limit_overrides`]: A list of IP addresses for which prop:kafka_connection_rate_limit[] is overridden and doesn't apply.
endif::[]
...
Configure the prop:kafka_connections_max_per_ip[] property to limit the number of connections from each client IP address. 
...
To configure prop:kafka_connections_max_per_ip[] safely without disrupting legitimate clients, follow these steps:
...
. Set the prop:kafka_connections_max_per_ip[] value based on your analysis. Use the upper bound of normal connections observed, or use a lower value if you know how many connections per client IP are being opened. 
...
NOTE: If you find a high load of unexpected connections from multiple IP addresses, prop:kafka_connections_max_per_ip[] alone may be insufficient. If offending IPs outnumber legitimate client IPs, you may need to set prop:kafka_connections_max_per_ip[] so low that it affects legitimate clients. If this is the case, use prop:kafka_connections_max_overrides[] to exempt known legitimate client IPs from the connection limit.
...
* All clients behind the shared IP are collectively subject to the single prop:kafka_connections_max_per_ip[] limit.
* Connection rejections occur randomly among clients when the limit is reached. For example, suppose prop:kafka_connections_max_per_ip[] is set to 100, but clients behind a NAT gateway collectively need 150 connections. When the limit is reached, clients can make only some of the connections while others get rejected, leaving the client in a not-working state.
