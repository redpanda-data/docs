...
Continuous intra-broker partition balancing (prop:core_balancing_continuous[]) requires an xref:get-started:licensing/index.adoc#redpanda-enterprise-edition[enterprise license].
...
| Redpanda continuously monitors the cluster for broker failures and high disk usage and automatically redistributes partitions to maintain optimal performance and availability. It also monitors rack availability after failures, and for a given partition, it tries to move excess replicas from racks that have more than one replica to racks where there are none. See xref:./continuous-data-balancing.adoc[Configure Continuous Data Balancing]. +
 +
This is the default setting for clusters with an enterprise license. An enterprise license is required.
...
. Set the prop:partition_autobalancing_mode[] property to `off`. If Redpanda partition balancing is enabled, Redpanda may change partition assignments regardless of what you do with `rpk`.
+
[,bash]
----
