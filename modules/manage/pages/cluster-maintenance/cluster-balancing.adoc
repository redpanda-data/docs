...
* Kafka doesn't support shard-level partition assignments, but Redpanda does. For example, to specify that all replicas of partition `1` be assigned on shard `0` on each broker, and all replicas of partition `2` be assigned on shard `1` on each broker, run: `rpk cluster partitions move test -p 1:3-0,1-0,2-0 -p 2:2-1,1-1,3-1`. For more information, run: `rpk cluster partitions move --help`.
