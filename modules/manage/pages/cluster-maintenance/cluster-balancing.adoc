...
| Topic-aware balancing that moves partition replicas. This helps avoid topic replica hotspots on one or a few specific brokers in your cluster. 
 +
 +
Redpanda prioritizes balancing a topic's partition count on each broker while it's balancing the cluster's overall partition count. Because different topics in a cluster can have vastly different load profiles, this better distributes the workload evenly across brokers.
 +
 +
...
Different topics in a cluster can have vastly different load profiles, but partitions of a single topic are often similar. To distribute the workload of high-demand topics, Redpanda prioritizes an equal count of a topic's partitions on each broker while it aims for an equal overall partition count on each broker. 
