...
| When a node is unavailable for at least this timeout duration, Redpanda automatically and permanently decommissions the node. Unlike `partition_autobalancing_node_availability_timeout_sec`, which moves partitions while keeping the node in the cluster, this property removes the node from the cluster entirely. A decommissioned node cannot rejoin the cluster. +
 +
