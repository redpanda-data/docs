...
In many instances, cluster administrators wish to lock certain properties in a way that individual topic settings cannot override them. If a property is locked then any attempts to edit that property at a topic level fail. For example, if you lock `internal_topic_replication_factor` at a value of 3, then any attempt to set `replication.factor` for a topic will yield an error.
...
NOTE: If you previously set a lock on the property, you will see the `;locked` attribute present in the output if this command.
