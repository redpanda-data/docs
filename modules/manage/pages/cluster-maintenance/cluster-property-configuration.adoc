...
== Lock Properties at the Cluster Level

In many instances, cluster administrators may wish to lock certain properties in a way that individual topic settings cannot override them. If a property is locked, then any attempts to edit that property at a topic level will fail. For example, if you lock `internal_topic_replication_factor` at a value of 3, then any attempt to set or edit `replication.factor` for a topic will yield an error.

NOTE: No properties are shipped with a locked setting. Property locking is a strictly opt-in feature.

Enabling a property lock on a topic uses a straightforward syntax - you append `;locked` to the property value. For example, if you run `rpk cluster config edit` you might set `log_compression_type=producer;locked`. When viewing the property, you will see the appended keyword giving a quick identification of the presence of the lock. Any YAML generated via `rpk cluster config export` also includes the presence of the locked attribute.

NOTE: You can only set a property to locked when providing a new or existing value for the property. You cannot set a property to locked with a default value unless you explicitly specify that value along with the locked attribute.

Removing a property lock is just as straightfoward - you remove the `;locked` attribute from the property definition. Following the previous example, changing `log_compression_type=producer` will remove the lock on this property and the corresponding topic property can then be set at will.
...
TIP: If you previously set a lock on the property, you will see the `;locked` attribute present in the output if this command.
