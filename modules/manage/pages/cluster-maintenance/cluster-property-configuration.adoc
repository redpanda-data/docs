...
Redpanda includes cluster properties, broker properties, and topic properties. Some properties can be configured at either the cluster level or the topic level, such as prop:retention_bytes[] (cluster-level) and prop:retention.bytes[] (topic-level). Topic-level properties override cluster-level properties. Most Redpanda configuration properties are cluster properties.
...
* xref:reference:node-properties.adoc[Broker configuration properties] and xref:deploy:redpanda/manual/node-property-configuration.adoc[Configure broker properties]
...
To change any property settings, edit the configuration from the command line using your default text editor. As you make changes, the Redpanda Admin API verifies that the new value is valid. For example, if you change prop:fetch_max_bytes[] from the default of `57671680` to `5o` (using the letter "`o`" by mistake), the system displays the following message:
...
CAUTION: Redpanda does not support importing cluster-specific identification (such as prop:cluster_id[]) with this command.
