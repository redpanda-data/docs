...
The replication factor appears as REPLICAS in the SUMMARY section, not as replication.factor in the CONFIGS list. However, you need to use the prop:replication.factor[] key when modifying the value with `rpk topic alter-config`.
...
For example, to create a topic with the prop:cleanup.policy[] property set to `compact`:
...
For example, to modify a topic's prop:retention.ms[] property:
...
* xref:develop:manage-topics/config-topics.adoc[Manage Topics] - Create and manage topics
