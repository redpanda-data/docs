...
Log compaction in Redpanda removes duplicate keys to reduce log size and improve read efficiency. Compaction is triggered during scheduled scans. A log is eligible for compaction based on its *dirty ratio* and the applicable threshold.

=== Dirty ratio-based compaction

The *dirty ratio* determines whether a log should be compacted. It is defined as:
...
| Cluster
| Minimum dirty ratio a log must exceed to be eligible for compaction.
...
| Cluster
| Interval between log compaction scans.
|===
