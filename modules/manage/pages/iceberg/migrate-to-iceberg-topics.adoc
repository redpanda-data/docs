...
// tag::single-source[]
ifdef::env-cloud[:rpcn-doc: develop:connect/about.adoc]
ifndef::env-cloud[:rpcn-doc: connect:home:index.adoc]
...
NOTE: This page focuses on migrating from Kafka Connect Iceberg Sink. The migration patterns and SQL examples can be adapted for other Iceberg sources such as Apache Flink or Spark.
...
. Set the prop:iceberg_enabled[] configuration property on your cluster to `true`:
ifdef::env-cloud[]
+
[tabs]
=====
...
. Configure the prop:redpanda.iceberg.mode[] property for the topic:
+
[,bash]
----
...
. Use a stream processor (xref:{rpcn-doc}[Redpanda Connect] or Flink) to split records.
...
For a complete list of Iceberg metrics, see the xref:reference:public-metrics-reference.adoc#iceberg-metrics[Iceberg metrics reference].
...
If the count is greater than zero, inspect the failed records. See xref:manage:iceberg/iceberg-troubleshooting.adoc[] for steps to inspect and reprocess DLQ records.
...
If any issues arise during cutover, see <<Plan for rollback>>.
