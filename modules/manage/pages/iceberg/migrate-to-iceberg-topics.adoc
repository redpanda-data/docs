...
NOTE: This guide focuses on migrating from Kafka Connect Iceberg Sink, the most common external integration. The migration patterns and SQL examples can be adapted for other Iceberg sources such as Apache Flink or Spark.
...
----
-- Source table
SELECT COUNT(*) AS source_count FROM <catalog>.<source_table>;
...
If your existing integration routes records to multiple Iceberg tables based on a field value (for example, Kafka Connect's `iceberg.tables.route-field` property), you need to implement equivalent routing logic.

With Iceberg Topics, the pattern is:
...
SELECT DISTINCT partition FROM <catalog>.<source_table>
EXCEPT
...
[,sql]
----
SELECT MAX(timestamp) FROM <catalog>.<source_table>;
...
[,sql]
----
DROP TABLE <catalog>.<source_table>;
----
