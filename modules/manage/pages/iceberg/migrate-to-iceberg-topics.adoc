...
:learning-objective-1: Compare Kafka Connect Iceberg Sink with Iceberg Topics architectures
...
To migrate from an existing Kafka Connect Iceberg Sink setup to Iceberg Topics, you must have:
...
* Knowledge of your Kafka Connect configuration, especially if using `iceberg.tables.route-field` for multi-table routing.
* If migrating multi-table fan-out patterns, xref:develop:data-transforms/deploy.adoc[data transforms] enabled on your cluster.
...
. Disable Kafka Connect Iceberg Sink.
...
----
+
Choose the mode based on your Kafka Connect converter configuration:
+
[cols="2,3"]
|===
...
----
+
IMPORTANT: If using the `value_schema_id_prefix` mode, schema subjects must use the `<topic-name>-value` xref:manage:schema-reg/schema-id-validation.adoc#set-subject-strategy-per-topic[naming convention] (TopicNameStrategy).
+
...
----
-- Kafka Connect table
SELECT COUNT(*) AS kafka_connect_count FROM <catalog>.<kafka_connect_table>;

-- Iceberg Topics table
SELECT COUNT(*) AS iceberg_topics_count FROM <catalog>.<iceberg_topics_table>;
----
...
If your Kafka Connect setup used the `iceberg.tables.route-field` property to fan out records to multiple Iceberg tables based on a field value, for example `iceberg.tables.route-field=table_name`, you need to implement equivalent routing logic.

With Iceberg Topics, the equivalent pattern is:

. Create separate Iceberg-enabled topics (one per destination table).
. Use xref:develop:data-transforms/build.adoc#multi-topic-fanout[Data Transforms] to route records based on the routing field.
. Redpanda automatically creates the Iceberg table for each topic.

==== Option 1: Wasm transforms with separate topics (recommended)

This approach provides clean separation and built-in Iceberg integration per topic.
...
----
┌──────────────┐
│ Input Topic  │  Single topic with batched updates
│  (events)    │
└──────┬───────┘
       │
       ▼
┌──────────────┐
│  Transform   │  Reads routing field, writes to output topics
│   (Wasm)     │  Validates with registered schemas
└──────┬───────┘
       │
       ├─────────────┬─────────────┐
       ▼             ▼             ▼
 ┌─────────┐   ┌──────────┐  ┌───────────┐
 │ orders  │   │inventory │  │ customers │  Each topic → Iceberg table
 │(Iceberg)│   │(Iceberg) │  │ (Iceberg) │
 └─────────┘   └──────────┘  └───────────┘
----

Your cluster must have data transforms enabled (`rpk cluster config set data_transforms_enabled true`).

. Register schemas for each output topic following the `<topic-name>-value` naming convention.

. Create output topics and enable Iceberg:
+
[,bash]
----
rpk topic create orders inventory customers
rpk topic alter-config orders --set redpanda.iceberg.mode=value_schema_id_prefix
rpk topic alter-config inventory --set redpanda.iceberg.mode=value_schema_id_prefix
rpk topic alter-config customers --set redpanda.iceberg.mode=value_schema_id_prefix
----

. Implement a transform function that:
.. Reads the routing field from each input message.
.. If validating with a schema, ensures the message conforms to the registered schema.
.. Writes to the appropriate output topic based on routing field.
...
rpk transform deploy \
  ...
  --input-topic events \
  --output-topic orders \
  --output-topic inventory \
  --output-topic customers
----

. Validate the fanout by checking that each output topic receives the correct records.

For complete implementation with Schema Registry wire format encoding, see xref:develop:data-transforms/build.adoc#multi-topic-fanout[Multi-topic fan-out with Schema Registry]. The example includes working Go and Rust code that handles routing, wire format encoding, and multiple output topics.
...
FROM <catalog>.<kafka_connect_table>
UNION ALL
SELECT 'Target', COUNT(*)
FROM <catalog>.<iceberg_topics_table>;
----
...
SELECT DISTINCT partition FROM <catalog>.<kafka_connect_table>
EXCEPT
SELECT DISTINCT partition FROM <catalog>.<iceberg_topics_table>;
...
* Wrong xref:manage:iceberg/specify-iceberg-schema.adoc[Iceberg mode].
* Schema validation failures. For example, using the wrong schema subject name or not using the wire format when required.
...
If the count is greater than zero, inspect the failed records.

==== Multi-table fanout transform issues

The transform may not be processing messages due to:

* Specified output topics don't exist or aren't enabled with Iceberg.
* Schema IDs in transform code don't match registered schemas.
...
Verify that the routing field exists in input messages and that schema IDs match between transform code and Schema Registry.
...
. Point consumers back to Kafka Connect tables.
. Investigate Iceberg Topics issues using troubleshooting section.
. Fix issues and re-validate.
. Attempt cutover again when ready.

==== Rollback after Kafka Connect disabled

WARNING: Rollback after stopping Kafka Connect may result in data loss or gaps.

If you must rollback after disabling Kafka Connect:

. Restart Kafka Connect Iceberg Sink connector immediately.
...
WHERE timestamp > '<kafka_connect_stop_time>';
----

. Write exported data to Kafka Connect input topics or directly to Kafka Connect tables.
. Verify data completeness across both systems.
. Resume operations on Kafka Connect.
...
Before disabling Kafka Connect, ensure you have completed all validation steps:
...
. Monitor Iceberg Topics to ensure data continues flowing. Use the queries from <<Monitor during parallel operation>>.

. Verify that no new records are being written to Kafka Connect tables:
+
[,sql]
----
SELECT MAX(timestamp) FROM <catalog>.<kafka_connect_table>;
-- Should not change after connector is stopped
----
...
. If migrating to a unified table of historical plus new data, optionally delete old Kafka Connect tables after an extended validation period (for example, at least seven days):
+
...
[,sql]
----
DROP TABLE <catalog>.<kafka_connect_table>;
----

. Decommission Kafka Connect infrastructure after an extended safety period (30+ days, for example).
