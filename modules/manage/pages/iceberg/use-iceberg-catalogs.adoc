...
|===
| | Databricks Unity Catalog | Snowflake Open Catalog | AWS Glue Data Catalog | GCP Lakehouse

|AWS   |Supported |Beta |Beta |N/A
|GCP   |Supported |Beta |N/A  |Supported
...
The Iceberg integration for Redpanda Streaming supports multiple Iceberg catalogs, with progressive levels of release maturity. Each catalog integration is tested and released independently.
...
* prop:iceberg_catalog_type[link=true,helm-path=auto]: `rest`
* prop:iceberg_rest_catalog_endpoint[link=true,helm-path=auto]: The endpoint URL for your Iceberg catalog. You either manage this directly, or you have this managed by an external catalog service.

NOTE: You must set prop:iceberg_rest_catalog_endpoint[] at the same time that you set prop:iceberg_catalog_type[] to `rest`.
...
By default, Redpanda creates Iceberg tables in a namespace called `redpanda`. To use a unique namespace, configure the prop:iceberg_default_catalog_namespace[link=true,helm-path=auto] cluster property. You must set this property before enabling the Iceberg integration or at the same time. After you have enabled Iceberg, do not change this property value.

==== Configure authentication

To authenticate with the REST catalog, set the following cluster properties:

* prop:iceberg_rest_catalog_authentication_mode[link=true,helm-path=auto]: The authentication mode to use for the REST catalog. Choose from `oauth2`, `aws_sigv4`, `bearer`, or `none` (default). You must use `aws_sigv4` for xref:manage:iceberg/iceberg-topics-aws-glue.adoc[AWS Glue Data Catalog].
ifdef::env-cloud[]
+
...
+
--
* prop:iceberg_rest_catalog_oauth2_server_uri[link=true,helm-path=auto]: The OAuth endpoint URI used to retrieve tokens for REST catalog authentication. If left unset, the deprecated catalog endpoint `/v1/oauth/tokens` is used as the token endpoint instead. 
* prop:iceberg_rest_catalog_client_id[link=true,helm-path=auto]: The ID used to query the OAuth token endpoint for REST catalog authentication.
* prop:iceberg_rest_catalog_client_secret[link=true,helm-path=auto]:  The secret used with the client ID to query the OAuth token endpoint for REST catalog authentication.
--
** For `bearer`, configure the prop:iceberg_rest_catalog_token[link=true,helm-path=auto] property with your bearer token.
+
...
* prop:iceberg_rest_catalog_trust[link=true,helm-path=auto]: The contents of a certificate chain to trust for the REST catalog. 
ifndef::env-cloud[]
** Or, use prop:iceberg_rest_catalog_trust_file[link=true,helm-path=auto] to specify the path to the certificate chain file.
endif::[]
* prop:iceberg_rest_catalog_crl[link=true,helm-path=auto]: The contents of a certificate revocation list for prop:iceberg_rest_catalog_trust[].
ifndef::env-cloud[]
** Or, use prop:iceberg_rest_catalog_crl_file[link=true,helm-path=auto] to specify the path to the certificate revocation list file.
endif::[]
...
* prop:iceberg_rest_catalog_client_secret[]
* prop:iceberg_rest_catalog_crl[]
* prop:iceberg_rest_catalog_token[]
* prop:iceberg_rest_catalog_trust[]
...
For example, to use a secret for the prop:iceberg_rest_catalog_client_secret[] property, run:
...
If you configured a different namespace using prop:iceberg_default_catalog_namespace[link=true,helm-path=auto], replace `redpanda` with your configured namespace.
...
By default, Iceberg topics use the filesystem-based catalog (prop:iceberg_catalog_type[link=true,helm-path=auto] cluster property set to `object_storage`). Redpanda stores the table metadata in https://iceberg.apache.org/docs/latest/java-api-quickstart/#using-a-hadoop-catalog[HadoopCatalog^] format in the same object storage bucket or container as the data files.
...
ifndef::env-cloud[]
The prop:iceberg_catalog_base_location[link=true,helm-path=auto] property stores the base path for the filesystem-based catalog if using the `object_storage` catalog type. The default value is `redpanda-iceberg-catalog`. 

CAUTION: Do not change the prop:iceberg_catalog_base_location[] value after you have enabled Iceberg integration for a topic.
endif::[]
...
* xref:manage:iceberg/iceberg-topics-aws-glue.adoc[]
* xref:manage:iceberg/iceberg-topics-databricks-unity.adoc[]
