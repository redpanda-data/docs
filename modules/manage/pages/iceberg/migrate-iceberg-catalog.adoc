...
----
+
While Iceberg translation is paused in the next step, the topic's retention anchor on the log is released. Without infinite retention, untranslated data could be deleted before the migration completes.
...
----
+
Setting the mode to `disabled` stops new translation while letting already-translated data finish being committed to the old catalog. For more about Iceberg modes, see xref:manage:iceberg/specify-iceberg-schema.adoc[].
+
...
+
This metric reports the number of offsets pending a commit to the Iceberg catalog. While it is non-zero, Redpanda is still flushing translated data to the old catalog. The translation lag metric, `redpanda_iceberg_pending_translation_lag`, can remain non-zero; this reflects new incoming data the cluster has not yet translated, which is expected because translation is paused.
+
[TIP]
====
...
. Apply the new catalog configuration. Update the catalog cluster properties — for example, to switch from `object_storage` to a REST catalog:
+
[,bash]
----
...
----
+
For full property guidance for each catalog type, see xref:manage:iceberg/use-iceberg-catalogs.adoc#rest[Connect to a REST catalog].
...
. After the cluster comes up, confirm it is connected to the new catalog by checking broker logs for successful catalog requests and the absence of authentication errors.
...
* Check broker logs for any commit failures referencing the new catalog. Repeated failures often indicate a schema or partition spec mismatch — see <<troubleshooting,Troubleshooting>>.
...
* Translation does not resume after restoring `redpanda.iceberg.mode`: Check that `redpanda_iceberg_pending_translation_lag` is increasing as new records are produced. If it remains `0`, no new records are being translated. Verify that your producer is still writing to the topic and that the topic's mode value is one of `key_value`, `value_schema_id_prefix`, or `value_schema_latest`.
