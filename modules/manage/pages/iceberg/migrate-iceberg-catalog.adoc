...
[IMPORTANT]
====
Do not change prop:iceberg_catalog_type[link=true,helm-path=auto] or any other catalog cluster property in place without following this procedure. In-flight commits and untranslated data can be lost or stuck if the catalog changes mid-translation.
====
...
. Save the current prop:retention.ms[] and prop:retention.bytes[] values for every Iceberg topic, then set both to `-1` (infinite retention):
+
[,bash]
----
...
. Pause Iceberg translation on every Iceberg topic by setting prop:redpanda.iceberg.mode[] to `disabled`. Save each topic's previous mode value so you can restore it later.
+
[,bash]
----
...
+
NOTE: Do not change prop:iceberg_enabled[link=true,helm-path=auto] at the cluster level. The Iceberg integration must remain enabled at the cluster level so that pending commits can drain to the old catalog.
...
. Resume Iceberg translation by restoring prop:redpanda.iceberg.mode[] on every Iceberg topic to its previous value:
+
[,bash]
----
...
. Restore prop:retention.ms[] and prop:retention.bytes[] on every Iceberg topic to the values you saved before starting the migration.
...
* Authentication errors against the new REST catalog: Verify that the credential cluster properties (for example, prop:iceberg_rest_catalog_client_id[], prop:iceberg_rest_catalog_client_secret[], prop:iceberg_rest_catalog_token[]) match what the new catalog expects. For OAuth, also check prop:iceberg_rest_catalog_oauth2_server_uri[].
* Translation does not resume after restoring prop:redpanda.iceberg.mode[]: Check that `redpanda_iceberg_pending_translation_lag` is increasing as new records are produced. If it remains `0`, the cluster is not translating new records. Verify that your producer is still writing to the topic and that the topic's mode value is one of `key_value`, `value_schema_id_prefix`, or `value_schema_latest`.
