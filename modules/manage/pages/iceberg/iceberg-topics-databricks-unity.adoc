...
ifndef::env-cloud[:storage: Tiered Storage]
ifdef::env-cloud[:storage: object storage]
...
* Workspace admin privileges to complete the steps to create a Unity Catalog storage credential and external location that connects your cluster's {storage} bucket to Databricks.
...
The external location stores the Unity Catalog-managed Iceberg metadata, and the Iceberg data written by Redpanda. You must use the same bucket configured for {storage} for your Redpanda cluster.
...
* By default, Redpanda creates tables in a schema named `redpanda`. If you want to use a different schema, set prop:iceberg_default_catalog_namespace[link=true,helm-path=auto] before enabling Iceberg, then manually create that schema in the catalog.
* Set the schema's managed storage location to the same S3 bucket used for {storage}, using the external location you created in the previous step.
...
You should see the topic as a table with data in Unity Catalog. The data may take some time to become visible, depending on your prop:iceberg_target_lag_ms[link=true,helm-path=auto] setting.

. In Catalog Explorer, open your catalog. You should see a `redpanda` schema (or the namespace you configured with prop:iceberg_default_catalog_namespace[link=true,helm-path=auto]), in addition to `default` and `information_schema`.
