...
If you are using the recommended new-catalog setup, you can skip Option 2. Redpanda creates the default `redpanda` schema for you.
...
If you need to integrate Redpanda with an existing Unity Catalog catalog object, manually create the schema that Redpanda will use before Redpanda creates any tables in that catalog.

Unity Catalog resolves managed storage locations through a hierarchy of metastore > catalog > schema. If you assign the schema its own managed storage location, Redpanda can use the existing catalog while the schema stores its managed Iceberg data in the schema-specific location. See the Databricks documentation on https://docs.databricks.com/aws/en/data-governance/unity-catalog/#managed-storage-location-hierarchy[managed storage location hierarchy^] and https://docs.databricks.com/aws/en/schemas/create-schema[creating schemas^].

When you use this approach:

* By default, Redpanda creates tables in a schema named `redpanda`.
* If you want to use a different schema, set config_ref:iceberg_default_catalog_namespace,true,properties/cluster-properties[`iceberg_default_catalog_namespace`] before enabling Iceberg, then manually create that schema in the catalog.
* When you create the schema, set its managed storage location to the same S3 bucket used for Redpanda Tiered Storage, through the external location you created in the previous step.

For example:

* Your existing Unity Catalog catalog stores managed data in `s3://<catalog-bucket-name>`.
ifdef::env-cloud[]
* You manually create a `redpanda` schema in that catalog and override its managed storage location, through the external location, to `s3://redpanda-cloud-storage-<cluster-id>`, which matches the S3 bucket that Redpanda uses for Tiered Storage.
endif::[]
ifndef::env-cloud[]
...
After you choose one of these catalog options, the remaining authorization and Redpanda configuration steps are the same.
