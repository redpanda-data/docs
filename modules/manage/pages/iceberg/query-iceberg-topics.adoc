...
----
+
Iceberg topics require that the schema subject follow `TopicNameStrategy` so Redpanda can use the schema to decode records and translate them into the Iceberg table format, where the table name is the name of the topic. For more information on subject name strategies, see xref:manage:schema-reg/schema-id-validation.adoc[].
...
. The following is an example of a query that returns the semi-structured data in the `ClickEvent_key_value` table. The table consists of two columns: one named `redpanda`, containing the record key and other metadata, and another binary column named `value` for the record's value:
+
[,sql]
----
SELECT *
