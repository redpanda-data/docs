...
NOTE: The query examples on this page use `redpanda` as the Iceberg namespace, which is the default. If you configured a different namespace using prop:iceberg_default_catalog_namespace[link=true,helm-path=auto], replace `redpanda` with your configured namespace.
...
Assume that you have created the `ClickEvent` topic, set prop:redpanda.iceberg.mode[] to `value_schema_id_prefix`, and are connecting to a REST-based Iceberg catalog. The following is an Avro schema for `ClickEvent`:
...
In this example, assume that you have created the `ClickEvent_key_value` topic, and set prop:redpanda.iceberg.mode[] to `key_value`.
