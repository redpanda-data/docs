...
ifdef::env-cloud[]
* `rpk` or familiarity with the Redpanda Cloud API to use secrets in your cluster configuration. For `rpk`, see xref:manage:rpk/rpk-install.adoc[]. For the Cloud API, you must link:/api/doc/cloud-controlplane/authentication[authenticate] using a service account.
endif::[]
ifndef::env-cloud[]
...
* The Data Plane API to xref:api:ROOT:cloud-dataplane-api.adoc#post-/v1/topics[create a new topic] or link:/api/doc/cloud-dataplane/operation/operation-topicservice_updatetopicconfigurations[update a property for an existing topic]. Specify the key-value pair for `redpanda.iceberg.mode` in the request body.
endif::[]
+
