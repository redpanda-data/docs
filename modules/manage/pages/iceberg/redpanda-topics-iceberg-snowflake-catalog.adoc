...
ifndef::env-cloud[]
. Edit your cluster configuration to set the prop:iceberg_enabled[] property to `true`, and set the catalog integration properties listed in the example below. You must restart your cluster if you change this configuration for a running cluster. You can run `rpk cluster config edit` to update these properties:
+
[,bash]
----
iceberg_enabled: true 
...
. Under your catalog, you will see the `redpanda` namespace (or the namespace you configured with prop:iceberg_default_catalog_namespace[link=true,helm-path=auto]), and a table with the name of your topic. The namespace and the table are automatically added for you.
