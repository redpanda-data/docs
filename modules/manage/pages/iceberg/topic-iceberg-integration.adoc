...
Redpanda supports schema evolution for Avro and Protobuf schemas. When you update the schema in the Schema Registry, Redpanda automatically updates the Iceberg table schema to match the new schema.
...
If you update the schema to add a new field `ts`, and produce records with the updated schema:
