...
When you enable Iceberg for any substantial workload, you may see most of your cluster's CPU utilization increase as a result of translation of topic data to the Iceberg format. If this process overloads the brokers, Redpanda automatically applies backpressure to clients to prevent runaway lag of the Iceberg table behind the topic log.
...
rpk topic create <new-topic-name> -p5 -r3 -c "redpanda.iceberg.partition.spec=(<partition-key1>, <partition-key2>, ...)"
----

Valid `<partition-key>` values include a source column name or a transformation of a column. The columns referenced can be Redpanda-defined (such as `redpanda.timestamp`) or user-defined based on a schema that you register for the topic. The Iceberg table stores records that shares different partition key values in separate files based on this specification. 
...
[TIP]
====
* It is recommended to partition by columns that you frequently use in queries. Columns with relatively few unique values, also known as low cardinality, are also good candidates for partitioning.
...
A high column count or schema field count results in more overhead when translating topics to the Iceberg table format. Small message sizes can also increase CPU utilization. Redpanda recommends keeping to a low column count and large message size for Iceberg topics to minimize the performance impact on your cluster.

=== Size your cluster for Iceberg topics

Iceberg workloads can be CPU-intensive and decrease throughput. To ensure that your cluster is sized appropriately for Iceberg topics, contact the Redpanda Customer Success team. 
