...
* If you must partition based on columns with high cardinality, for example timestamps, use Iceberg's available transforms such as extracting the year, month, or day to avoid creating too many partitions. Too many partitions can offset the benefits of partitioning because more files need to be scanned and managed.
====

=== Avoid high column count

A high column count results in more overhead when translating topics to the Iceberg table format. Redpanda recommends keeping the column count low for Iceberg topics to minimize the performance impact on your cluster.
