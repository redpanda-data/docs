...
This guide walks you through querying Redpanda topics as Iceberg tables stored in Google Cloud Storage, using a REST catalog integration with https://docs.cloud.google.com/lakehouse/docs/introduction[Google Lakehouse for Apache Iceberg^] (formerly BigLake). In this guide, you do the following:
...
- Create a catalog in Lakehouse
- Configure the Lakehouse integration for your Redpanda cluster
...
ifndef::env-cloud[]
* Redpanda version {rp_version} or later. Your Redpanda cluster must be deployed on GCP VMs.
endif::[]
ifdef::env-cloud[]
...
ifdef::env-cloud[]
For BYOC clusters, the required Lakehouse IAM permissions (`roles/biglake.editor`, `roles/serviceusage.serviceUsageConsumer`, `roles/storage.objectAdmin`) are automatically provisioned and attached to the cluster's service account when Iceberg is enabled with a Lakehouse endpoint. You don't need to manually create a service account or grant permissions.

For BYOVPC clusters, you must ensure that the service account used by the cluster has the <<grant-required-permissions,required permissions>> for Lakehouse. You must also enable the `biglake.googleapis.com` and `bigquery.googleapis.com` APIs in your GCP project.
endif::[]
...
Create a Lakehouse Iceberg REST catalog using the `gcloud` CLI:

NOTE: This command is currently pre-GA and may change. Check the https://docs.cloud.google.com/sdk/gcloud/reference/alpha/biglake/iceberg/catalogs/create[gcloud reference^] for the latest information.
...
rpk cluster config set \
  ...
  iceberg_rest_catalog_warehouse=gs://<bucket-name>/ \
  iceberg_rest_catalog_gcp_user_project=<gcp-project-id>
// TODO: iceberg_dlq_table_suffix is not in the cloud proto. Lakehouse rejects the default ~dlq (tildes not allowed). Needs engineering follow-up to confirm if cloud sets this automatically or if the property needs to be added to the cloud API.
----
+
--
...
It may take a few moments for the Iceberg data to become available in Lakehouse.
