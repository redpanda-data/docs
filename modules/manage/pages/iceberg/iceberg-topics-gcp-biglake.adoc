...
For BYOVPC clusters, you must ensure that the service account used by the cluster has the following IAM roles: `roles/storage.objectAdmin`, `roles/biglake.editor`, and `roles/serviceusage.serviceUsageConsumer`. You must also enable the `biglake.googleapis.com` and `bigquery.googleapis.com` APIs in your GCP project.
endif::[]
