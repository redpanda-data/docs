...
ifdef::env-cloud[]
** BigLake must be in the same GCP project as the cluster. Cross-project BigLake is not supported.
endif::[]
...
ifdef::env-cloud[]
For BYOC clusters, the required BigLake IAM permissions (`roles/biglake.editor`, `roles/serviceusage.serviceUsageConsumer`, `roles/storage.objectAdmin`) are automatically provisioned and attached to the cluster's service account when Iceberg is enabled with a BigLake endpoint. You don't need to manually create a service account or grant permissions.

For BYOVPC clusters, you must ensure that the service account used by the cluster has the <<grant-required-permissions,required permissions>> for BigLake. You must also enable the `biglake.googleapis.com` and `bigquery.googleapis.com` APIs in your GCP project.
endif::[]
...
rpk cluster config set \
  ...
  iceberg_rest_catalog_warehouse=gs://<bucket-name>/ \
  iceberg_rest_catalog_gcp_user_project=<gcp-project-id>
// TODO: iceberg_dlq_table_suffix is not in the cloud proto. BigLake rejects the default ~dlq (tildes not allowed). Needs engineering follow-up to confirm if cloud sets this automatically or if the property needs to be added to the cloud API.
----
+
--
