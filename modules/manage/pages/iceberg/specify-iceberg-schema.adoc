...
:learning-objective-3: Apply independent translation for record keys, values, and headers
...
The latest schema is cached periodically. The cache period is defined by the cluster property prop:iceberg_latest_schema_cache_ttl_ms[] (default: 5 minutes).

[[disabled]]
=== disabled

Default for prop:redpanda.iceberg.mode[]. Disables writing to an Iceberg table for the topic.
...
NOTE: For `schema_id_prefix` and `schema_latest`, every field in the decoded key struct is optional and an absent field is stored as null, so the table can tolerate schema evolution.

[[value-type-by-value-mode]]
...
NOTE: Only header values are affected by `value_type`. Header keys are always stored as strings.
...
If you use xref:manage:schema-reg/schema-reg-contexts.adoc[Schema Registry contexts] to isolate schemas (for example, by environment or tenant), set the prop:redpanda.schema.registry.context[] topic property to bind the topic to that context. Redpanda then resolves the schemas referenced by records in the topic against the configured context instead of the default context (`.`). This applies to all schema-decoding modes, for both keys and values: the `value_schema_id_prefix` and `value_schema_latest` shorthand modes, and any `key` or `value` section that uses `mode=schema_id_prefix` or `mode=schema_latest`.
...
If Redpanda cannot resolve a record's schema within the configured context, it doesn't translate the record and instead writes it to a dead-letter queue (DLQ) table. See xref:manage:iceberg/iceberg-troubleshooting.adoc[Troubleshoot Iceberg Topics].

[IMPORTANT]
====
Redpanda resolves schemas using the topic's current prop:redpanda.schema.registry.context[] value, not the context that was active when a record was ingested. Changing the context on a topic that has pending, uncommitted Iceberg translation entries can cause resolution errors or unexpected DLQ routing for records still being translated.
...
. Change prop:redpanda.schema.registry.context[].
