...
The `key` section decodes the same way, though decoded key fields always land in the single `redpanda.key` field — there's no `layout` option for keys. For example, given a key schema with fields `user_id` (int) and `region` (string):
