...
- You must use a JSON Schema that constrains JSON documents to a strict type in order for Redpanda to translate to Iceberg.
...
| array 
| list 
| The keywords `items` and `additionalItems` are used to define element types.
...
* Boolean JSON Schema combinations (`allOf`, `anyOf`, o`neOf` keywords)
* Dynamic object members (`patternProperties`, `additionalProperties` (except when it is set to `false`) keywords)
--
