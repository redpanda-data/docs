...
For example, you could create a separate IAM policy that manages access to AWS Glue, and attach it to the IAM role that Redpanda also uses to access S3. It is recommended to add all AWS Glue actions in the policy (`"glue:*"`) on the following resources:
...
== Authorize AWS Glue access to S3

You also need to allow AWS Glue service to access the S3 bucket where your Iceberg data is stored.

Follow the steps in the AWS Glue documentation to:

. https://docs.aws.amazon.com/glue/latest/dg/create-service-policy.html[Create an IAM policy for the Glue service^] to access your Iceberg data in S3.
. https://docs.aws.amazon.com/glue/latest/dg/create-an-iam-role.html[Create an IAM role for the Glue service^] and attach the policy you created in the previous step.

You must grant AWS Glue the minimum read and write permissions to the `iceberg_rest_catalog_base_location` defined in the <<update-cluster-configuration,cluster configuration>>. This is the path where Redpanda stores Iceberg metadata and data files. As a security best practice, Redpanda Data recommends that you restrict access to only this path to prevent unauthorized changes to the rest of your Tiered Storage data or other resources in S3.
...
+
--
- `<aws-region>`: The AWS region where your Data Catalog is located. The region in the AWS Glue endpoint must match the region specified in either your `cloud_storage_region` or `iceberg_rest_catalog_aws_region` property.
- `<bucket-name>` and `<warehouse-path>`: AWS Glue requires you to specify the base location where Redpanda stores Iceberg data and metadata. You must use an S3 URI like `s3://<bucket-name>/<warehouse-path>`.
