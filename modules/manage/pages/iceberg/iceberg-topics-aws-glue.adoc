...
Redpanda's role must also have the minimum read and write permissions to the `iceberg_rest_catalog_base_location` defined in the <<update-cluster-configuration,cluster configuration>>, assuming that you manage S3 access with a different policy. This is the path where Redpanda stores Iceberg metadata and data files.

For security best practices, refer to the AWS documentation on https://docs.aws.amazon.com/glue/latest/dg/configure-iam-for-glue.html[configuring IAM permissions for Glue^].
