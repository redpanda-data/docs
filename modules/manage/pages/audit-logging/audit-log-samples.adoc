...
[%collapsible]
====
This example shows an API Activity (6003) where the authorization decision matched an ALLOW ACL on a `Group:` principal. The `actor.user.groups` field includes the matched group with type `idp_group`, and the `authorization_metadata` shows the group ACL that granted access.
ifndef::env-cloud[]
See xref:{gbac-doc}[Group-Based Access Control].
endif::[]
