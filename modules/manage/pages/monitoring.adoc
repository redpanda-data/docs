...
NOTE: Redpanda recommends creating monitoring dashboards with `/public_metrics`.
...
====
. Edit the `prometheus.yml` file in the Prometheus root folder to add the Redpanda configuration under `scrape_configs`. Customize `targets` for the names and number of running nodes.
+
[,yaml]
----
 scrape_configs:
...
----
  rpk generate grafana-dashboard \
  --datasource prometheus \
  --metrics-endpoint <node-addr>:9644/metrics> redpanda-dashboard.json
----
