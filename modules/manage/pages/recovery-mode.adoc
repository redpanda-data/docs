...
You may find that problems preventing normal cluster startup are isolated to certain partitions or topics. You can use rpk or the xref:api:ROOT:admin-api.adoc#tag/Partitions[Admin API] to disable these partitions even outside of recovery mode. You can disable partitions at the topic level, or individual partition level. 
