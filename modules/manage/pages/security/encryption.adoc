...
You can also specify a certificate revocation list (`ca.crl`) to block connections from entities using certifications that have expired or been revoked by a certificate authority (CA). 

Each broker has its own `broker.key` and `broker.crt`, but all brokers can have the same `ca.crt` and `ca.crl`. The `ca.crl` must contain a certificate revocation list (CRL) for each issuing certificate in the truststore file concatenated into a single list. 
...
redpanda:
  ...
  - name: internal
    ...
    crl_file: <path-to-PEM-formatted-CRL-file> # Optional
----
...
If you're using `rpk` to interact with the Kafka API using mTLS identity (for example, to manage topics or messages), pass the `--tls-key`, `--tls-cert`, and `--tls-truststore` flags to authenticate. If you have specified a certificate revocation list (`crl_file`) in your configuration, you can also use --crl-key`.

To interact with the Admin API (for example, to manage users), pass the `--admin-api-tls-key`, `--admin-api-tls-cert`, `--admin-api-tls-truststore` and `--admin-api-tls-crl` flags.
...
--tls-crl <path-to-PEM-formatted-CA-file>\
----
...
Alternatively, you can choose to <<configure-tls,specify a certificate revocation list>> to block connections from entities using certifications that have expired or been revoked by a certificate authority (CA). 
