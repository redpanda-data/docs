...
NOTE: Self-signed certificates are useful if you want to generate multiple certificates all signed by the same root. For example, if you want to use mTLS but issue different certificates to multiple Redpanda brokers and clients.

=== Option 1: Simple self-signed certificate for a single broker

Use this method if you need a quick, standalone self-signed certificate for a single broker.
...
- `broker.key`: The private key.
- `broker.crt`: The self-signed certificate.

[IMPORTANT]
====
Ensure that `subjectAltName` includes the appropriate DNS names and IP addresses for your broker.
====

=== Option 2: Create a local CA for multiple brokers or clients

Use this method to set up a local Certificate Authority (CA) for signing multiple broker or client certificates.

. Create a file named `ca.cnf` and include your organization's details:
+
[source,ini]
----
[ ca ]
...
+
- `ca.crt`: The public certificate for the CA.
