...
You can use IAM roles with any Redpanda feature that makes use of cloud storage, such as xref::tiered-storage.adoc[Tiered Storage] or xref::remote-read-replicas.adoc[Remote Read Replicas].
...
. Ensure that https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-IMDS-existing-instances.html[IMDSv2 is optional] for your EC2 instance.

Redpanda supports only IMDSv1. Now that all EC2 instances default to IMDSv2, you must configure IMDSv2 as optional so that Redpanda can use IMDSv1. For details see the https://aws.amazon.com/about-aws/whats-new/2023/06/imds-packet-analyzer-simplifies-migration-imdsv2/[AWS announcement].
