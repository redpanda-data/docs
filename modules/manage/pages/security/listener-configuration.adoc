...
Redpanda supports running multiple authentication schemes concurrently. Each listener can specify its xref:reference:properties/broker-properties.adoc#authentication_method[`authentication_method`], and must define a `name` property.
