...
Administrators may remove a role from a principal without deleting the role. A principal is composed of two parts: a type and a name. Redpanda currently supports the `User` type only for principals. All principals, therefore, have the format: `User:[name]`
...
Administrators can modify an existing role by adding additional ACLs to it.
