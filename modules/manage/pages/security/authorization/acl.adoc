...
Access-control lists (ACLs) are one of the primary mechanisms Redpanda provides to manage user permissions. Redpanda stores ACLs internally, replicated with https://raft.github.io/[Raft^] to provide the same consensus guarantees as your data. You can manage ACLs with xref:reference:rpk/rpk-acl.adoc[`rpk acl`].
...
In flag form to set up a general producing/consuming client, you can invoke xref:reference:rpk/rpk-acl/rpk-acl-create.adoc[`rpk acl create`] up to three times with the following (including your `--allow-principal`):
...
The xref:reference:rpk/rpk-acl.adoc[`rpk acl`] command manages your ACLs as well as your SASL/SCRAM users. (If you're on Kubernetes, you can use `kubectl exec` to run rpk commands.)
...
To get more information, see xref:reference:rpk/rpk-acl.adoc[`rpk acl`].

=== Global flags

Every xref:reference:rpk/rpk-acl.adoc[`rpk acl`] command can use these flags:
