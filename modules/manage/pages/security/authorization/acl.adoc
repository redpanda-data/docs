...
Starting in version 25.2, Redpanda also supports the following resources for Schema Registry:

- `registry-global` (for "top level" operations, such as `GET :8081/mode`, `GET :8081/config`.)
- `registry-subject` (for "subject-level" operations, such as `GET :8081/subjects`, `GET :8081/config/{subject}`))
