...
Access control lists (ACLs) provide a way to configure fine-grained access to Redpanda resources. ACLs are permission rules that determine what actions users or roles can perform on clusters, topics, consumer groups, transactional IDs, and Schema Registry subjects and operations. Redpanda stores ACLs internally, replicated with glossterm:Raft[] to provide the same consensus guarantees as your data. 
