...
Access control lists (ACLs) provide a way to configure fine-grained access to Redpanda resources. ACLs are assigned principals, either user or role. 

In {ui}, you create ACLs from the Security tab on either the ACLs page or the Roles page. The ACLs page defaults to the `user` principal. The Roles page defaults to the `Redpanda role` principal. But you can edit to specify different principals as needed. You then select the host, either all hosts or specific IP addresses, and then you select which Redpanda resources that ACL can access: clusters, topics, consumer groups, transactional IDs, subjects and Schema Registry.

ACLs work with SASL/SCRAM and with mTLS with principal mapping for authentication.

== ACLs

Use access control lists (ACLs) to manage user and role permissions. ACLs are assigned principals (user or role), which then access resources within Redpanda. 
