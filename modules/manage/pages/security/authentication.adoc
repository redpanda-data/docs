...
NOTE: OpenID Connect (OIDC) authentication requires an xref:get-started:licenses.adoc[Enterprise license]. To upgrade, contact https://redpanda.com/try-redpanda?section=enterprise-trial[Redpanda sales^].

https://openid.net/developers/how-connect-works/[OIDC^] is a protocol that simplifies authentication of users so that Redpanda can delegate the authentication process to an external identity provider (IdP) such as Okta or Azure AD. Using OIDC provides platform-wide authentication that allows users to interact with Redpanda features using a single set of credentials managed externally by your existing trusted IdP. OIDC benefits include:

* *Single sign-on (SSO) experience*: Users can authenticate once with their IdP and then access multiple applications, including Redpanda, without having to re-enter their credentials.
* *Compliance and Auditing*: For organizations with strict compliance requirements OIDC allows for detailed audit trails and compliance reporting.
* *Enhanced Security*: OIDC relies on industry-standard security practices and protocols, reducing the risk of security vulnerabilities and unauthorized access to the Redpanda platform.
...
* If you wish to use OIDC with clients, then you will need to register the client credentials.
