...
* Catalog: Contains the current metadata pointer for the table. Clients reading and writing data to the table see the same version of the current state of the table. You'll <<integrate-into-iceberg-catalog,configure your Iceberg catalog>> to point to your object storage bucket or container where the Redpanda data in Iceberg format is located. Redpanda uses the https://iceberg.apache.org/concepts/catalog/#catalog-implementations[Iceberg REST catalog^] endpoint to update your catalog when there are changes to the Iceberg data and metadata.
...
. `DISABLED` (default): Disables the creation of an Iceberg table for this topic.
+
[,bash]
----
...
== Integrate into Iceberg catalog
