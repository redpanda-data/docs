...
+
This option is not recommended for production use cases.
...
Using a REST catalog enables Spark to automatically discover the topic's Iceberg table. If using the object storage catalog type, you may have to explicitly create a new table and load the Iceberg data, depending on the processing engine you're using. 
...
In either mode, you do not need to rely on complex ETL jobs or pipelines to consume real-time data from Redpanda.
