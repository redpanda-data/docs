...
----
+
If you select `KEY_VALUE` for the topic Iceberg mode, Redpanda uses a simple schema consisting of a column that stores the record’s metadata, key, and value.
...
== Schema mapping and support

The `redpanda.iceberg.mode` property determines how Redpanda maps the topic data to the Iceberg table structure. 
...
The `KEY_VALUE` mode writes to the table format:
...
Consider this schemaless approach if you are using the JSON Schema format, or if you can use the data in a semi-structured format.
