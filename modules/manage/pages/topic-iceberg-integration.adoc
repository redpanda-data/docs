...
+
If you don't configure a schema for the topic, Redpanda by default uses a simple schema consisting of the record’s key, value, and the original message timestamp. If you don't have a need for a strict schema and can use the data in a more semi-structured format, consider the schemaless approach.
+
[,bash]
----
...
As you produce messages to the topic, the data also becomes available to consume from object storage by Iceberg-compatible clients.
