...
In this example each broker has 8 cores available. If you plan to scale in to five broker, then you would have 40 cores available, which means that your cluster is limited by core count to 40K partitions, which exceeds the current 3018 partitions.
...
You can repeat this procedure to scale in to 5 brokers.
