...
Here's an example that configures two certificates: `public-ca-cert` and `private-ca-cert`. The `public-ca-cert` certificate is configured with an <<use-a-ca-certificate, Issuer managed by cert-manager>>, while the `private-ca-cert` certificate is configured with a <<use-secrets-to-manage-tls-certificates-without-cert-manager, generic Secret>> containing the `tls.crt`, `tls.key`, and `ca.crt` files.
