...
* *Redpanda Operator*: Ensure you have at least version v2.2.2-24.2.4 of the xref:deploy:redpanda/kubernetes/k-production-deployment.adoc[Redpanda Operator].
...
- *Consider roles for shared permissions*: When multiple users need the same set of permissions, consider using xref:manage:kubernetes/security/authorization/k-role-controller.adoc[RedpandaRole resources] instead of duplicating ACLs across individual User resources.
