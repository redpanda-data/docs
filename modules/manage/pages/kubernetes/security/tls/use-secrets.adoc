...
Learn how configure Redpanda listeners with TLS using TLS certificates in your own Secret resources.
...
If your certificates are issued by a private CA, create an Opaque Secret that includes the `ca.crt` file:
...
You can configure Redpanda to use your TLS certificates for all internal or external listeners. All internal listeners use the TLS certificates defined in `tls.certs.default`. All external listeners use the TLS certificates defined in `tls.certs.external`.
...
By default, certificates will be used to encrypt traffic between clients and all external listeners. You can also select specific certificates for each external listener. See xref:manage:kubernetes/networking/configure-listeners.adoc#tls[].
