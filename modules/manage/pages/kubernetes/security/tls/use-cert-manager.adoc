...
:description: Enable TLS encryption in your Redpanda cluster and use cert-manager to simplify the process of obtaining, renewing, and using certificates.

Learn how to enable TLS encryption in your Redpanda cluster and use glossterm:cert-manager[^] to simplify the process of obtaining, renewing, and using certificates.

When using cert-manager for TLS certificate management, you have the option to use a self-signed certificate or a certificate signed by a trusted Certificate Authority (CA). This topic provides instructions for each option.
...
The Issuer or ClusterIssuer specifies the CA that will be used when generating certificates. If you select an ACME server such as Let's Encrypt as the CA, cert-manager will automatically handle the required HTTP01 or DNS01 ACME challenges to issue certificates.
...
======
+
By default, this certificate will be used to encrypt traffic between clients and all external listeners. You can select specific certificates for each external listener. See xref:manage:kubernetes/networking/configure-listeners.adoc#tls[].
...
include::manage:partial$kubernetes/mtls.adoc[leveloffset=+1]
