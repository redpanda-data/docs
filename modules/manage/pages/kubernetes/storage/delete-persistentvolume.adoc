...
:tags: ["Kubernetes"]
:page-aliases: manage:kubernetes/delete-persistentvolume.adoc
...
. xref:manage:kubernetes/decommission-brokers.adoc[Decommission the broker] that has a PersistentVolumeClaim (PVC) bound to the PV.
+
Decommissioning ensures no data loss by gracefully moving the broker's topic partitions and replicas to other brokers in the cluster.
