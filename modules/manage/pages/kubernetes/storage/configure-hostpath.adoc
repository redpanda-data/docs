...
Both the Redpanda Helm chart and the Redpanda custom resource definition provide an interface for configuring `hostPath` volumes.
...
- `storage.persistentVolume.enabled`: Determines if a PersistentVolumeClaim (PVC) should be created for the Redpanda data directory. When set to `false`, a PVC is not created.
