...
- `private-ca-cert`. This certificate is configured with an xref:manage:kubernetes/security/tls/use-secrets.adoc[Opaque Secret] containing the `tls.crt`, `tls.key`, and `ca.crt` files.
