...
Because data isn't acknowledged until uploaded to object storage, produce latency is higher than with standard topics. The end-to-end latency experienced when using Cloud Topics can range from 500 ms to as high as a few seconds with different object stores. Lower latencies may be achievable in certain environments, but Cloud Topics is optimized for throughput rather than low latency or tightly constrained tail latency. You should generally expect end-to-end latencies of 1-2 seconds with public cloud stores.
