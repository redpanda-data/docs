...
* xref:manage:kubernetes/cloud-topics/k-cloud-topics.adoc[Cloud Topics enabled] on the source cluster, with cluster metadata uploads enabled (prop:enable_cluster_metadata_upload_loop[] is enabled by default).
...
----
+
The output includes prop:redpanda.storage.mode[] with the value `cloud`:
+
----
redpanda.storage.mode    cloud    DYNAMIC_TOPIC_CONFIG
----
+
If the topic inherited cloud storage mode from the cluster default (prop:default_redpanda_storage_mode[]), the source column shows `DEFAULT_CONFIG` instead of `DYNAMIC_TOPIC_CONFIG`.
