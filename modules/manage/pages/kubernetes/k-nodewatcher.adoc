...
NOTE: Make sure to use the `--skip-liveness-check` flag when decommissioning the broker with xref:reference:rpk/rpk-cluster/rpk-cluster-brokers-decommission.adoc[`rpk cluster brokers decommission`]. This flag is required when the broker is no longer running.
