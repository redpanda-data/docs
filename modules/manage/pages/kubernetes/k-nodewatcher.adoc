...
NOTE: Make sure to use the `--skip-liveness-check` flag when decommissioning the broker with `rpk cluster brokers decommission`. This flag is required when the broker is no longer running.
