...
:description: Learn how to scale a Redpanda cluster vertically to increase its resources and horizontally to add a remove brokers from a cluster.
...
- `rbac.createAdditionalControllerCRs=true`: Creates the requires RBAC rules for the Redpanda Operator to monitor the Node resources and update PVCs and PVs.
...
- `rbac.enabled`: Creates the requires RBAC rules for the controller to monitor the Node resources and update PVCs and PVs.
