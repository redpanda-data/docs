...
To change any property settings, edit the default configuration using the `--values` or `--set` flags in the `helm upgrade --install` command. For example, to set the config_ref:cloud_storage_access_key,true,object-storage-properties[] cluster configuration:
