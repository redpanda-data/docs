...
CAUTION: The Nodewatcher controller is an emergency backstop to keep your Redpanda cluster running in case of unexpected node failures. *Never use this controller as a routine method for removing brokers.* If you need to remove a broker, always follow the xref:manage:kubernetes/k-decommission-brokers.adoc[decommission process] to properly scale in your cluster.
...
After the Nodewatcher controller has finished, you have two options to maintain the stability of your Redpanda cluster:

  - *Option 1:* xref:manage:kubernetes/k-decommission-brokers.adoc[Decommission the broker] that was removed from the node. This is necessary to prevent quorum loss and ensure cluster stability.

  - *Option 2:* Recover the node. The StatefulSet will automatically reschedule the broker on the node, maintaining the desired number of brokers in the cluster. This option helps to keep your cluster running at full capacity, but you should still monitor the situation closely and consider decommissioning brokers if node failures persist.
