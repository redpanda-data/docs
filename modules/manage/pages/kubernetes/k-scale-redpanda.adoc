...
TIP: If you use the Redpanda Operator, you can use the xref:manage:kubernetes/k-node-pools.adoc[NodePool CRD] to move brokers to new worker nodes through automated blue/green migrations instead of manual decommission and re-provision steps.
...
. Ensure that you have one additional worker node for each Redpanda broker that you want to add. Each Redpanda broker requires its own dedicated worker node so that it has access to all resources. For more details, see xref:deploy:redpanda/kubernetes/k-requirements.adoc[].
...
* xref:manage:kubernetes/k-node-pools.adoc[Node Pools] -- Manage groups of brokers independently for blue/green migrations and vertical scaling.
