...
- <<qos, Enable the `Guaranteed` QoS class for your Pods that run Redpanda>>. This setup ensures that the CPU and memory allocated to Redpanda are not subject to throttling or other contention issues, providing a stable and predictable performance environment.
...
+
To enable CPU pinning in Kubernetes, you must enable the https://kubernetes.io/docs/tasks/administer-cluster/cpu-management-policies/#static-policy[`static` CPU Manager policy], which bypasses the default CPU allocation behavior managed by the CFS.
+
...
- *Overprovisioning disabled (default)*:
+
When overprovisioning is disabled in Redpanda, each thread runs on a dedicated core for maximum performance.
