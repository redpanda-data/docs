...
. Ensure your Kubernetes cluster is using the  https://kubernetes.io/docs/tasks/administer-cluster/cpu-management-policies/#static-policy[`static` CPU Manager]. With this policy enabled, a request for an integral CPU amount in a `Guaranteed` Pod will result in that container being allocated exclusive access to that number of CPUs through [cgroup's cpuset](https://www.kernel.org/doc/Documentation/cgroup-v1/cpusets.txt).
+
[source,bash]
----
