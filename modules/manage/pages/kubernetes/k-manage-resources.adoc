...
. Ensure the https://kubernetes.io/docs/tasks/administer-cluster/cpu-management-policies/#static-policy[`static` policy] is enabled for the CPU Manager on your Kubernetes cluster. With this policy enabled, a request for an integral CPU amount in a `Guaranteed` Pod will result in that container being allocated whole CPUs with a guarantee that no other container will run on them.
+
[source,bash]
----
...
CAUTION: When `resources.cpu.cores` is set to a fractional value, such as `1100m`, Redpanda may not use all the allocated CPU due to CFS quotas, leading to potential performance bottlenecks.
