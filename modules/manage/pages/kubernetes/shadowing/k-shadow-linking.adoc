...
NOTE: While the link is active, the destination contexts that the link replicates into are read-only on the shadow cluster. Schema replication in `api` mode requires Redpanda 26.2 or later, and the feature activates only after all brokers complete the upgrade.
...
Set `enabled: false` to turn role replication off. Role name filters use the same pattern and filter types as topic filters.
--
