...
If you don't want to use the <<Automated, Decommission controller>>, follow this workflow to manually decommission a broker before reducing the number of StatefulSet replicas:

[mermaid]
....
flowchart TB
    A[Start Manual Scale-In] --> B["Identify Broker to Remove<br/>(Highest Pod Ordinal)"]
    B --> C[Decommission Broker Running on Pod with Highest Ordinal]
    C --> D[Monitor Decommission Status]
    D --> E{Is Broker Removed?}
    E -- No --> D
    E -- Yes --> F[Decrease StatefulSet Replicas by 1]
    F --> G[Wait for Rolling Update and Cluster Health]
    G --> H{More Brokers to Remove?}
    H -- Yes --> B
    H -- No --> I[Done]
....
