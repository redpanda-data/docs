...
To enable the BrokerDecommissioner:
...
<3> `statefulset.sideCars.brokerDecommissioner.decommissionAfter`: The sidecar waits 60 seconds after detecting a failed broker before triggering decommissioning.
<4> `statefulset.sideCars.brokerDecommissioner.decommissionRequeueTimeout`: Interval at which the sidecar checks the cluster for invalid brokers.
