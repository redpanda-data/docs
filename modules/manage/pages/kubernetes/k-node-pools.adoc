...
[WARNING]
====
Never create and delete NodePools simultaneously. Due to the eventual consistency model of Kubernetes, simultaneous creation and deletion of NodePools is inherently race-prone. If decommissioning and reconciliation overlap, the cluster could be recreated (blown away) rather than migrated. Always ensure the new NodePool is fully stable before removing the old one.
====
