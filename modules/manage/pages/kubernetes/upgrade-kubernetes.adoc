...
For example, starting from Kubernetes 1.25+, the `v1beta1` version of the PodDisruptionBudget resource was deprecated. If you perform a Helm upgrade after updating Kubernetes, Helm carries out a 3-way merge between the desired state, the previous state, and the current state of your cluster. If your cluster's previous state still references the `v1beta1` resource version, Helm will fail to upgrade as this version isn't available in Kubernetes 1.25+.
