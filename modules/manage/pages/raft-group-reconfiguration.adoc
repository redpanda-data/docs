...
Raft uses the joint consensus algorithm to maintain consistency during reconfiguration. It involves both the old member set and the proposed new member set reaching an agreement between them, either on accepting writes, or electing a new leader. The new leader can be elected from either the old or new member set. Partitions are unavailable to accept writes until the old and new sets reach joint consensus.
