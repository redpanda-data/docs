...
Raft uses joint consensus to maintain consistency during reconfiguration. Joint consensus involves both the old member set and the proposed new member set reaching an agreement between them, either on accepting writes, or electing a new leader. The new leader can be elected from either the old or new member set. 
...
+
Only after learners are up to date with the leader will the group elect a new leader through joint consensus. Members that are not in the new configuration are demoted to learners before removal. The new leader then replicates the desired final configuration.
...
A Raft group that can tolerate _F_ broker failures needs to have at least 2__F__ + 1 members. The number of members is typically equal to the replication factor. In our example, the group can tolerate single-broker failure. The two remaining healthy group members can still reach quorum.
