= Raft Group Reconfiguration in Redpanda
:description: Learn how the Redpanda Raft group protocol provides consistency and availability during reconfiguration.

Redpanda and Kafka with Raft both use the Raft consensus algorithm to manage cluster metadata. This metadata is stored as messages in an internal topic in a separate Raft cluster instance, called a Raft group. The messages in this topic contain the current set of brokers that are members of a Raft group. In a Raft group, one member is the leader and the others are followers. 

Additionally, Redpanda uses Raft as its replication protocol for the actual data (that is, the application data). Each topic partition also forms a Raft group comprised of its leader as well as follower brokers. See xref:get-started:architecture.adoc#raft-consensus-algorithm[How Redpanda Works] for a further explanation of how Raft manages data replication in Redpanda.
...
Raft uses the joint consensus approach to maintain consistency during reconfiguration. It involves both the old member set and the proposed new member set electing a new group leader and agreeing on the latest committed offset. The new leader can be elected from either the old or new member set. Partitions are unavailable to accept writes until the group reaches joint consensus.

Redpanda’s implementation of Raft includes changes to the reconfiguration protocol to ensure high availability even while group membership changes.
...
- In addition to leader, follower, and voter, a broker can also have learner status. Learners are followers that cannot vote for a new leader, so they do not count towards quorum. Learners do not trigger leader elections and do not accept writes. New Raft group members start as learners. Brokers can be promoted or demoted between learner and voter. 
+
This means that when new members are added to the Raft group member set, the group continues to operate without interruption since members that can form quorum still accept writes, even if the majority has technically changed due to the additional members. As soon as a learner is synced with the current leader, the learner is promoted to voter. Learners are promoted one at a time. 

- Because all new nodes joining a Raft group start in learner status, the group does not yet have to enter the joint consensus phase as soon as new members join.
...
+
Only after learners are up to date with the leader will the group elect a new leader through joint consensus. Members that are not needed in the new configuration are demoted to learners before removal. The new leader then replicates the desired final configuration.

The following diagrams illustrate the reconfiguration steps in Redpanda. In this example, we replace one member in a three-broker member set of a Raft group. The box labeled "new" represents the new or current configuration, and "old" represents the old or previous configuration. Each configuration includes the member brokers as indicated by their IDs. Brokers in green are voters, whereas brokers in yellow are learners. If the old configuration is grayed out, it means that only the new configuration is active.


. We start with a configuration containing brokers 1, 2, and 3:
+
image::shared:raft_reconfiguration_initial_state.png[]

. We want to replace broker 1 with a new member broker 4. Broker 4 is added to the current configuration as a learner:
+
...
A Raft group that can tolerate _F_ broker failures needs to have at least 2__F__ + 1 members. The number of members is typically equal to the replication factor. In our example, the group can tolerate single-broker failure. Quorum can still be reached with the two remaining healthy group members.
