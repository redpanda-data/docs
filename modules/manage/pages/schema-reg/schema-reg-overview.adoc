...
**Normalization**: Normalization is the process of converting a schema into its simplest form. When a schema is normalized, it can be compared and considered equivalent to another schema even if they contain minor syntactic differences. Schema normalization allows you to more easily manage schema versions and compatibility by prioritizing meaningful logical changes. Normalization during both schema registration and lookup for a subject is supported for Avro and Protobuf.
