...
For optimal performance, use schemas that are 128KB or less in size. For Protobuf and Avro schemas, Redpanda recommends using schema xref:manage:schema-reg/schema-reg-api.adoc#reference-a-schema[references] to break up large schemas into smaller constituent parts.
