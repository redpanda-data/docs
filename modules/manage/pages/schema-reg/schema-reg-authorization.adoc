...
You can use Schema Registry Authorization to control user access to specific Redpanda Schema Registry resources through Access Control Lists (ACLs).
...
Schema Registry Authorization allows you to control which users and applications can perform specific operations within the Redpanda Schema Registry. This ensures that only authorized entities can read, write, modify, or delete schemas.

=== What changed in release 25.2

Before release 25.2, Schema Registry supported authentication, but once a user was authenticated, they had full access to all Schema Registry operations, including reading, modifying, and deleting schemas and configuration both per-subject and globally.

Starting in release 25.2, Schema Registry Authorization provides fine-grained access control through ACLs. You can now restrict access to specific subjects and operations.
...
Schema Registry Authorization introduces two new ACL resource types in addition to the standard Kafka ACL resources (`topic`, `group`, `cluster`, and `transactionalID`):

* **`registry-global`**: Controls access to global, or top-level Schema Registry operations.
* **`registry-subject`**: Controls access for specific Schema Registry subjects.
...
[NOTE]
====
Not all Kafka endpoints are supported when using Redpanda Schema Registry ACLs.
====
...
To enable Schema Registry Authorization for your cluster:
...
You can combine Schema Registry ACLs with xref:manage:security/authorization/rbac[Role-Based Access Control (RBAC)] to create reusable roles. This approach simplifies permission management when you need to assign the same set of permissions to multiple users.

This example creates a role called `SoftwareEng` and assigns it ACLs for both topic and Schema Registry access:

[NOTE]
====
Redpanda recommends using the topic naming strategy for Schema Registry subjects, where subjects follow the pattern `<topicName>-key` or `<topicName>-value`. For details, see xref:manage:schema-reg/schema-id-validation#set-subject-name-strategy-per-topic.adoc[Set subject name strategy per topic].
====
...
* The Schema Registry subject `bar-value` doesn't exist yet
...
In this case, the error `Not found` indicates that the Schema Registry subject `bar-value` does not exist. You must first create the subject using the xref:api:ROOT:schema-registry-api.adoc#post-/subjects[POST /subjects] endpoint, or by using the xref:reference:rpk/rpk-schema-registry/rpk-schema-registry-create-subject.adoc[`rpk schema-registry create subject`] command.
