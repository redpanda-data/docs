...
You can use Schema Registry Authorization to set up and manage ACL definitions for operations that allow users access to specific Schema Registry resources.
...
Starting in release 25.2, Schema Registry Authorization allows you to control access to Schema Registry resources by:

- Using `rpk` to define fine-grained ACLs (Access Control Lists). More specifically, use the xref:reference:rpk/rpk-security/rpk-security-acl-create.adoc[`rpk security create acl`] command as you would to create other ACLs.
...
Not all Kafka endpoints and operations are supported for Redpanda Schema Registry ACLs.

The following endpoints and operations are supported for Redpanda Schema Registry ACLs:
...
=== Create an ACL for a topic and a subject

To create an ACL for both a topic and a Schema Registry subject, use the `rpk security acl create` command with the appropriate resource types. In this case, the principal `panda` is allowed to read from the topic `bar` and the subject `bar-value`:
...
PRINCIPAL   HOST  RESOURCE-TYPE  RESOURCE-NAME  RESOURCE-PATTERN-TYPE  OPERATION  PERMISSION  ERROR
User:panda  *     SUBJECT        bar-value      LITERAL                READ       ALLOW       Not found
User:panda  *     TOPIC          bar            LITERAL                READ       ALLOW
----

In this case, the topic `bar` is created, but the subject `bar-value` was not created. Such an error can occur for a variety of reasons:
...
[,bash]
----
rpk security acl create --allow-principal panda --operation read --topic bar --registry-subject bar-value -v
