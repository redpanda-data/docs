...
While Schema Registry supported authentication prior to release 25.2, after a user was authenticated they had full access to read, modify, and delete schemas and configuration per-subject as well as globally.

Starting in release 25.2, Schema Registry Authorization allows you to control access to Redpanda Schema Registry resources by setting Access Control Lists (ACLs) on specific subjects and schemas. You can manage Schema Registry Authorization:

- Using `rpk` to define fine-grained ACLs. More specifically, use the xref:reference:rpk/rpk-security/rpk-security-acl-create.adoc[`rpk security acl create`] command as you would to create other ACLs.
...
* `registry-global`: Specifies whether or not to grant ACLs for the Schema Registry. This is for "top level" or global operations.
* `registry-subject`: Specifies Schema Registry subjects to grant ACLs to. This is for "subject-level" operations.
...
This section describes how to create and manage ACLs for the Redpanda Schema Registry.

=== Create an ACL for a topic and a Schema Registry subject

To create an ACL for both a topic and a registry-subject resources, use the `rpk security acl create` command with the appropriate resource types. In this case, the principal `panda` is allowed to read from the topic `bar` and the schema subject `bar-value`:
...
== Create an ACL that provides global access to your Schema Registry

To grant the user `jane` global `read`/`write access to the Schema Registry and to the topic `private` (essentially, two ACLs):
...
* The wrong URL is specified when the request is sent to the Schema Registry.
