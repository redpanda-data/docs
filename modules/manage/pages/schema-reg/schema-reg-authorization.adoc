...
To enable Schema Registry Authorization for the registry ACL admin:
...
In this case, the user has received a `Not found` error after attempting to create two ACLs, one for the subject and one for the topic:
...
This next error occured while the user was trying to create two ACLs, one for a topic and one for a registry-subject:
