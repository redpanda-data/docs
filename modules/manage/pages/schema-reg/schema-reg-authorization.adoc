...
When migrating schemas between clusters, you need **different ACLs for source and target clusters**.
...
[IMPORTANT]
====
**Schema Registry ACLs are only for Schema Registry operations.** For complete data migration, you also need Kafka ACLs:
...
1. **Bootstrap superusers** - Configure superusers via `.bootstrap.yaml` before enabling authentication
...
Pattern types:
* **`prefixed`** - Matches subjects starting with the specified string (e.g., `orders-` matches `orders-value`, `orders-key`)
