...
ifndef::env-cloud[]
For example, use `rpk` to set the value of prop:enable_schema_id_validation[] to `redpanda` through the Admin API:
...
ifdef::env-cloud[]
* Set prop:redpanda.key.schema.id.validation[] to `true` to enable key schema ID validation for the topic, and set prop:redpanda.key.subject.name.strategy[] to the desired subject name strategy for keys of the topic (default: `TopicNameStrategy`).
* Set prop:redpanda.value.schema.id.validation[] to `true` to enable value schema ID validation for the topic, and set prop:redpanda.value.subject.name.strategy[] to the desired subject name strategy for values of the topic (default: `TopicNameStrategy`).
...
| prop:redpanda.key.schema.id.validation[]
| prop:confluent.key.schema.validation[]

| prop:redpanda.key.subject.name.strategy[]
| prop:confluent.key.subject.name.strategy[]

| prop:redpanda.value.schema.id.validation[]
| prop:confluent.value.schema.validation[]

| prop:redpanda.value.subject.name.strategy[]
| prop:confluent.value.subject.name.strategy[]
|===
====
endif::[]
