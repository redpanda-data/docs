...
[NOTE]
====
The Schema Registry is built into Redpanda, and you can use it with the API or the UI. This section describes operations available in the link:/api/doc/schema-registry/[Schema Registry API].
====
...
For example, to link:/api/doc/schema-registry/group/endpoint-security[create ACLs] that allow users with the `admin` role read-only access to all registered schemas, run:
...
ifndef::env-cloud[]
In Redpanda v26.2 and later, contexts are enabled by default. In earlier versions, first set the prop:schema_registry_enable_qualified_subjects[] cluster configuration property to `true`, then restart the brokers. See xref:manage:schema-reg/schema-reg-contexts.adoc[] for setup instructions.
endif::env-cloud[]
ifdef::env-cloud[]
