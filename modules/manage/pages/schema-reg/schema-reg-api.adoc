...
[NOTE]
====
The Schema Registry is built into Redpanda, and you can use it with the API or the Redpanda UI. This section describes operations available in the xref:api:ROOT:pandaproxy-schema-registry.adoc[Schema Registry API]. 
====
...
* xref:manage:console/protobuf.adoc#schema-registry[Deserialization]
