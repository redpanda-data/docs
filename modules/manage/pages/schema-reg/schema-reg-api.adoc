...
. You need a running Redpanda cluster. If you don't have one, see the xref:get-started:quick-start.adoc[].
+
...
----
+
You can also use xref:get-started:intro-to-rpk.adoc[`rpk`] to interact with the Schema Registry. The xref:reference:rpk/rpk-registry/[`rpk registry`] set of commands call the different API endpoints as shown in the curl and Python examples. 
...
```bash
rpk registry schema create sensor-value ~/code/tmp/sensor_sample.avro
```
--
====
...
[NOTE]
====
`rpk` returns only the subject and not the schema.
...
--
```bash
rpk registry schema create import --schema import_schema.proto --references simple:simple:1
```
--
====
...
--
```bash
rpk registry schema delete simple --schema-version 1
```
--
====
...
--
```bash
rpk registry schema references simple --schema-version 1
```
--
====
...
```bash
rpk registry subject list --deleted true
```
