...
[NOTE]
====
This section describes managing Schema Registry operations in Redpanda Console (or Redpanda Cloud), but you also xref:manage:schema-registry.adoc[manage Schema Registry operations with the API].
====
...
To register a new schema, select the topic, the key or value, and the unique subject name you want to register. You can include a schema reference, which lets the schema refer to other schemas.

After you register a schema, you can add a new version to it, delete its subject, or change its compatibility.
