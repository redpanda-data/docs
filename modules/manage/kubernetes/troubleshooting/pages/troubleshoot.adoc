...
If you're unsure what might be wrong, generate a diagnostics bundle. It contains a wide range of data to help debug and diagnose issues with a Redpanda cluster, a broker, or the machines on which the brokers are running.

See xref::diagnostics-bundle.adoc[Diagnostics Bundles in Kubernetes].

== View Helm chart configuration

To inspect the values set in the Redpanda Helm chart:
