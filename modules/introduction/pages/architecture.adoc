...
This page provides details about how Redpanda works. For a high-level overview, see xref:introduction:intro-to-events.adoc[Introduction to Redpanda].

== Tiered Storage

Redpanda xref:data-management:tiered-storage.adoc[Tiered Storage] is a multi-tiered remote storage solution that provides the ability to offload log segments to cloud storage in near real time. Tiered Storage can be combined with local storage to provide infinite data retention and disaster recovery on a per-topic basis.
...
xref:quickstart:index.adoc[Try out Redpanda], or learn about xref:introduction:licenses.adoc[Redpanda Licensing].
