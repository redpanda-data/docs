...
- Deploy your own data pipeline with Redpanda Connect.
...
docker exec -it redpanda-0 rpk topic create chat-room \
  --replicas 3 \ <1>
  -c redpanda.remote.read=true \ <2>
  -c redpanda.remote.write=true \ <3>
  -X user=superuser \
...
== Deploy a data streaming pipeline with Redpanda Connect
