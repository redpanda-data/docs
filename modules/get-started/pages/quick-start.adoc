...
Redpanda Self-Managed is a modern streaming platform, compatible with Kafka APIs, designed for speed, simplicity, and efficiency. In this quickstart, you'll:
...
All new Redpanda clusters include a built-in 30-day enterprise license, allowing you to evaluate Redpanda Enterprise Edition, including its advanced features.

Some features highlighted in this quickstart require an enterprise license:
...
By default, your trial license also includes additional enterprise features.
...
Redpanda Console is a developer-friendly web UI for managing and debugging your Redpanda cluster and your applications. In this section, you'll explore practical examples and scenarios to help you understand how to leverage Redpanda Console for different use cases, including data observability, Redpanda management, access control, and connectivity.

Redpanda Console was deployed as part of the `docker-compose.yml` file and is running locally on port 8080.

=== Log into Redpanda Console
...
To test these permissions, you can use `rpk` to connect to Redpanda as the user Sasha.
...
Data transforms let you run common data streaming tasks, like filtering, scrubbing, and transcoding, on the Redpanda broker. For example, you may have consumers that require you to redact credit card numbers or convert JSON to Avro.
...
You should see your transform. You can use Redpanda Console to manage and monitor your transforms.
// (step {"action":"wait"})
...
For simplicity, this quickstart uses the bootstrapped superuser (`superuser`) to perform all operations. The superuser has full administrative privileges, making it convenient for setup and exploration. However, in a production environment, it's recommended to create specific users with restricted permissions for each task to enhance security.
...
NOTE: For this quickstart, Redpanda uses MinIO as the object storage provider. MinIO is not officially supported for production deployments. For production, see xref:deploy:deployment-option/self-hosted/manual/production/requirements.adoc#object-storage-providers-for-tiered-storage[Object storage providers for Tiered Storage].
...
To configure the Redpanda services with xref:reference:node-properties.adoc[broker configuration properties], you can pass configuration properties using the `--set` option in the `redpanda start` command within your Docker Compose file.
...
Cluster-level configurations are defined in the `bootstrap.yml` file. This file is included in your Docker Compose setup and contains essential settings that apply to the entire Redpanda cluster.
...
Redpanda Console's configuration is managed through the environment variable `CONSOLE_CONFIG_FILE`. You can specify the path to your configuration file within the Docker Compose file.
...
To create new data pipelines, use the `rpk connect run` command, specifying the path to your own YAML configuration file.
