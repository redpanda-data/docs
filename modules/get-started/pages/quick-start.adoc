...
[NOTE]
====
This quickstart uses Docker to run Redpanda, which is only for development and testing purposes. For production deployments, see the xref:deploy:redpanda/manual/index.adoc[Linux deployment guides] or the xref:deploy:redpanda/kubernetes/index.adoc[Kubernetes deployment guides]. To download the Redpanda binary, see https://github.com/redpanda-data/redpanda/releases/latest[GitHub^].
