...
Redpanda Self-Managed is a modern streaming platform, compatible with Kafka APIs, designed for speed, simplicity, and efficiency. The Enterprise Edition adds advanced capabilities that make it easier to scale, manage, and optimize your cluster. In this quickstart, you'll:

- Deploy Redpanda Self-Managed
- Learn the basics of streaming with the Redpanda CLI (`rpk`).
- Explore how to use Redpanda Console.
- Connect to the cluster externally from your local machine.
- Leverage Tiered Storage to keep performance stable as data scales.
...
curl -sSL https://docs.redpanda.com/redpanda-quickstart | tar xzf - && \ <2>
...
To start building a basic streaming application, you can use the Redpanda CLI (`rpk`) to create a topic, produce messages to it, and consume messages from it.

Each Redpanda broker comes preinstalled with `rpk`, so you can use it inside one of the Redpanda broker's Docker containers.
...
. Enter the following credentials, which are defined in the `console` service of the `docker-compose.yml` file:
+
--
- Username: `jane`
...
NOTE: The pipeline configuration is in the `docker-compose/transform/transform.go` file that you downloaded. This file is commented to explain how it works.
...
== View offloaded data to Tiered Storage
...
To create new data pipelines, use the `rpk connect run` command, specifying the path to your configuration file.
