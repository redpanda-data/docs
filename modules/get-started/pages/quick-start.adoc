= Quickstart for Redpanda Self-Managed Data Platform
...
// (test start {"testId": "quickstart", "description": "Redpanda quickstart"})

{description}

Redpanda Data Platform is a modern streaming platform, compatible with Kafka APIs, designed for speed, simplicity, and efficiency. In this quickstart, you:

- Deploy a three-broker Redpanda Streaming cluster.
...
[NOTE]
====
This quickstart uses Docker to run Redpanda, which is only for development and testing purposes. For production deployments, see the xref:deploy:redpanda/manual/production/index.adoc[Linux deployment guides] or the xref:deploy:redpanda/kubernetes/index.adoc[Kubernetes deployment guides]. To download the Redpanda binary, see https://github.com/redpanda-data/redpanda/releases/latest[GitHub^].
...
The Enterprise Edition of Redpanda adds advanced capabilities to help scale, manage, optimize, and secure your cluster.
...
To download, extract, and start Redpanda in Docker, run:
...
// (step {"find": {"elementTestId": "auth-username-input", "timeout": 10000, "click": true, "type": "superuser"}})
// (step {"find": {"selector": "input", "elementTestId": "auth-password-input", "timeout": 10000, "click": true, "type": "secretpassword"}})
// (step {"find": {"selector": "button", "elementTestId": "auth-submit", "elementText": "Log in", "timeout": 10000, "click": true}})
// (step {"checkLink": "http://localhost:8080/overview", "statusCodes": 200})
// (step {"wait": 5000})
// (step {"screenshot": {"path": "../../modules/console/images/overview.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})

To view details about a specific broker, click *View* at the end of the row in the *Broker Details* table.

// (step {"find": {"selector": "button", "elementText": "View", "timeout": 30000, "click": true}})
// (step {"screenshot": {"path": "../../modules/console/images/broker-overview.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
...
// (step {"screenshot": {"path": "../../modules/console/images/topic.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
// (step {"find": {"selector": "[data-testid='add-topic-filter']", "elementText": "Add filter", "click": true, "timeout": 10000}})
// (step {"find": {"selector": "[data-testid='add-topic-filter-javascript']", "elementText": "JavaScript Filter", "click": true, "timeout": 20000}})
...
// (step {"screenshot": {"path": "../../modules/console/images/js-filter.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
+
...
// (step {"goTo": "http://localhost:8080/topics/transactions"})
// (step {"find": {"selector": "[role='tablist'] button", "elementText": "Documentation", "click": true, "timeout": 15000}})
// (step {"screenshot": {"path": "../../modules/console/images/topic-documentation.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
...
// (step {"find": {"selector": "[data-testid='create-user-name']", "click": true, "type": "Sasha", "timeout": 15000}})
. Enter "sashapassword" in the password field.
// (step {"find": {"selector": "[data-testid='create-user-password']", "click": true, "type": "sashapassword", "timeout": 15000}})
// (step {"screenshot": {"path": "../../modules/console/images/user.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
// Close the dialog so it doesn't block clicks on the navigation menu in later steps.
// (step {"find": {"selector": "button[aria-label='Close']", "click": true, "timeout": 10000}})
. Click *Create*.
...
// (step {"find": { "selector": "[data-testid='show-internal-topics-checkbox']", "click": true, "timeout": 15000}})
. Click the *_redpanda.audit_log* topic in the table.
// (step {"goTo": "http://localhost:8080/topics/_redpanda.audit_log"})
// (step {"screenshot": {"path": "../../modules/console/images/audit-logs.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
...
// (step {"goTo": "http://localhost:8080/overview"})
// (step {"wait": 5000})
// (step {"screenshot": {"path": "../../modules/console/images/degraded-cluster.png", "maxVariation": 0.10, "overwrite": "aboveVariation"}})
...
- xref:get-started:licensing/overview.adoc[Learn more about enterprise features for Redpanda]
