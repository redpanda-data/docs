...
Redpanda now exposes dedicated consumer lag gauges that brokers compute internally. These new metrics eliminate the need for complex calculations based on offset metrics and work with observability platforms that lack advanced query support. The new consumer lag gauges include:
