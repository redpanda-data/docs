...
Redpanda now includes `rpk` and Redpanda Console support for managing xref:manage:cluster-maintenance/manage-throughput.adoc#client-throughput-limits[client throughput limits] with the AlterClientQuotas and DescribeClientQuotas Kafka APIs. You can set client throughput quotas using `rpk` for individual client based on client ID, as well as a group of clients based on ID prefix matching. New xref:manage:cluster-maintenance/manage-throughput.adoc#monitor-client-throughput[metrics] are available for insight on client throughput utilization.

Client throughput quotas, previously applied on a per-shard basis, are also adjusted to apply on a per-broker basis. Cluster configuration properties for managing client quotas are deprecated, including `target_quota_byte_rate` which is disabled by default with the value `0`.
