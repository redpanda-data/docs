...
A new cluster configuration property, xref:reference:properties/cluster-properties.adoc#fetch_read_strategy[`fetch_read_strategy`], sets new clusters with a non-polling strategy to fulfill fetch requests. The backend is signaled when a partition has new data, so it doesn't need to repeatedly read from every partition in the fetch. This improves fetch latency when there is extra CPU space. 
