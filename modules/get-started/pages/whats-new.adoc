...
Redpanda now includes support for setting xref:manage:cluster-maintenance/manage-throughput.adoc#client-throughput-limits[client throughput limits] with the AlterClientQuotas and DescribeClientQuotas Kafka APIs. You can set client throughput quotas using `rpk` for individual client based on client ID, as well as a group of clients based on ID prefix matching. New xref:manage:cluster-maintenance/manage-throughput.adoc#monitor-client-throughput[metrics] are available for insight on client throughput utilization.
