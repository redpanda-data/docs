...
* *`rpk redpanda admin` commands moved to `rpk cluster`*: The broker maintenance commands now live under xref:reference:rpk/rpk-cluster/rpk-cluster-brokers.adoc[`rpk cluster brokers`], broker listing under xref:reference:rpk/rpk-cluster/rpk-cluster-info.adoc[`rpk cluster info -b --detailed`], partition listing under xref:reference:rpk/rpk-cluster/rpk-cluster-partitions.adoc[`rpk cluster partitions list`], node configuration under `rpk cluster config list --node-id`, and logger management under `rpk cluster loggers`. The old `rpk redpanda admin` commands keep working in this release as deprecated aliases, and each prints its replacement when you run it.
...
* *New `check` managed plugin*: Install with xref:reference:rpk/rpk-check/rpk-check.adoc[`rpk check install`].
