...
New metrics report the estimated capacity of each broker's host, including vCPUs, memory, disk size and IOPS, and network throughput for the detected cloud instance type, as well as actual disk activity from the host OS, enabled with the `enable_host_metrics` cluster property. The new *Redpanda - Load Factor* Grafana dashboard combines these with utilization metrics to compute a load factor for each resource, showing how close each broker is to its first bottleneck so you can plan scaling before saturation.
