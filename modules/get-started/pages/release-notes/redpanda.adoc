...
Cloud Topics no longer require a separate cluster property. When cloud storage is enabled for your cluster (xref:reference:properties/object-storage-properties.adoc#cloud_storage_enabled[`cloud_storage_enabled`] set to `true`), you can create Cloud Topics immediately by setting the storage mode during topic creation. The prop:cloud_topics_enabled[] cluster property is deprecated.

See xref:develop:manage-topics/cloud-topics.adoc[Manage Cloud Topics].

== Schema Registry context for Iceberg translation

Bind a topic's Iceberg translation to a specific xref:manage:schema-reg/schema-reg-contexts.adoc[Schema Registry context] using the new prop:redpanda.schema.registry.context[] topic property, instead of always using the default context. This is useful for multi-tenant or per-environment schema isolation.
...
Schema Registry contexts are also enabled by default in this release: prop:schema_registry_enable_qualified_subjects[] now defaults to `true`. This applies to existing clusters when they upgrade, unless the property was explicitly set to `false` before the upgrade. If any of your subject names start with `:.`, review the xref:manage:schema-reg/schema-reg-contexts.adoc#upgrade-considerations[upgrade considerations] before upgrading.
...
Independently control how Redpanda translates a record's key, value, and headers into the Iceberg table with a new section-based syntax for the prop:redpanda.iceberg.mode[] topic property. You can decode keys and values using a schema registered in the Schema Registry, store keys or values as UTF-8 strings, or decode header values from raw bytes to strings. The `key_value`, `value_schema_id_prefix`, and `value_schema_latest` modes continue to work as shorthand for common combinations.
...
* prop:cloud_topics_enabled[]: Cloud Topics no longer require a separate enablement property and can be created whenever object storage is enabled for the cluster.
* prop:space_management_enable_override[]: now formally deprecated. This property has been ignored since v23.3.
