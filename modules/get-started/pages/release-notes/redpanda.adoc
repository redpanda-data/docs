...
HTTP Proxy previously used automatically-generated ephemeral credentials to authenticate with the Kafka API when the HTTP Proxy listeners specified xref:reference:properties/broker-properties.adoc#authentication_method[`authentication_method`]: `none`. To improve security and simplify the authentication model, ephemeral credentials are no longer available.
