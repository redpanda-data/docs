...
Support for deploying multiple instances of the Redpanda Operator in a single Kubernetes cluster is **deprecated** starting in Redpanda Operator v25.1.x. This feature will be removed in v25.2.x. For more information, see xref:deploy:deployment-option/self-hosted/kubernetes/k-production-deployment.adoc#deploy-multiple-redpanda-clusters[Deploy multiple Redpanda clusters].
...
TIP: For a supported and scalable Kafka Connect alternative, consider using Redpanda Connect. For more information, see xref:connect:get-started:index.adoc[].
