...
To try Redpanda Console v3 in Kubernetes, you can deploy Redpanda using the Redpanda Helm chart instead of the Redpanda Operator. Redpanda Console v3 introduces support for unified authentication and authorization with Redpanda, including user impersonation.
...
If you require Kafka Connect:

- You must install the `connectors` Helm chart separately after deploying Redpanda.
- This change affects both Helm and Operator-based Redpanda deployments on Kubernetes.

link:<link-to-knowledge-base-article>[See the migration guide^] for instructions.

To check if you're currently using Redpanda's managed Kafka Connect:
