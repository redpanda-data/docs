...
To check the status of your license in the Redpanda Console UI, see xref:console:ui/add-license.adoc[].
...
If you deployed a Redpanda cluster in Kubernetes using the Redpanda Operator, you can monitor the Enterprise Edition license status from the Redpanda custom resource. The operator integrates license monitoring into the resource's `status.conditions` and `status.license` fields, allowing you to get the license state with `kubectl`.
