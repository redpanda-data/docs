...
|
Set the cluster config prop:audit_enabled[] to `false`:
...
|
Set the cluster config prop:partition_autobalancing_mode[] to `node_add`:
...
(prop:core_balancing_continuous[])
|
Set the cluster config prop:core_balancing_continuous[] to `false`:
...
|
Set the node config prop:fips_mode[] to `disabled`:
...
|
Remove `GSSAPI` from the cluster config prop:sasl_mechanisms[]:
...
|
Set prop:default_leaders_preference[] to `none`:
...
|
Remove `OIDC` from the cluster config prop:sasl_mechanisms[] and prop:http_authentication[]:
[,bash]
----
rpk cluster config set sasl_mechanisms <other-mechanisms>
...
|
Set the cluster config prop:cloud_storage_enable_remote_read[] to `false`:
...
|
Set the cluster config prop:enable_schema_id_validation[] to `false`:
...
|
Set the cluster config prop:cloud_storage_enabled[] to `false`:
