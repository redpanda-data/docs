...
To get a trial license key or extend your trial period, https://redpanda.com/try-enterprise[generate a new trial license key]. Or, to purchase a license, contact https://redpanda.com/upgrade[Redpanda Sales^].
...
The following table lists the available Enterprise features for Redpanda, along with how Redpanda behaves when each enterprise feature is enabled and the license expires.

[IMPORTANT]
====
*Upon license expiration*:

- The cluster continues to operate without data loss, but further use of enterprise features is restricted (see the <<redpanda-enterprise-features, table>>).
- Configuration of enterprise features remains unchanged, allowing you to apply a new license and resume the use of enterprise features as they were before expiration. For instructions, see xref:get-started:licensing/add-license-redpanda/index.adoc[]

*After license expiration*: You cannot enable any enterprise features without a valid license.
====
...
|Enables authentication for Redpanda Console, allowing secure access for users.
