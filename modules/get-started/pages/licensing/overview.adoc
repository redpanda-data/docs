...
All new Redpanda clusters automatically receive a trial license valid for 30 days. To extend this trial for 30 days, use the xref:reference:rpk/rpk-generate/rpk-generate-license.adoc[`rpk generate license`] command or request one at [https://redpanda.com/try-enterprise[redpanda.com^]](https://redpanda.com/try-enterprise%5Bredpanda.com%5E%5D). This extended evaluation period begins after you apply the trial license to your cluster using the `--apply` flag. After this period expires, inactive enterprise features are disabled, and active features enter a restricted state.
