...
In Redpanda Connect, you must https://redpanda.com/try-enterprise[generate] and xref:redpanda-connect:get-started:licensing.adoc#apply-a-license-key-to-redpanda-connect[apply a trial license key] to evaluate enterprise features. The 30-day evaluation period begins when you run your first data pipeline. After this period expires, you are blocked from using enterprise connectors unless you upgrade to an https://www.redpanda.com/upgrade[Enterprise Edition license^].
