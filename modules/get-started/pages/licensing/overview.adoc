...
| xref:manage:kubernetes/k-connect-pipelines.adoc[Pipelines in Kubernetes] (Pipeline resource)
| Run Redpanda Connect pipelines declaratively with the Redpanda Operator. The operator's Connect controller requires a valid license, configured through the operator's `enterprise.licenseSecretRef` Helm chart values.
