...
Redpanda beta versions provide users the opportunity to test and share feedback on new features before they're finalized in general availability. Beta versions, like `{full-version}`, are published to `redpanda-unstable` as release candidate (RC) builds. RC builds are not recommended for production use.
...
----
docker pull docker.redpanda.com/redpandadata/redpanda-unstable:{full-version}
----
...
services:
  redpanda-0:
    ...
    image: docker.redpanda.com/redpandadata/redpanda-unstable:{full-version}
...
helm install redpanda redpanda/redpanda \
  --namespace <namespace> \
  --create-namespace --set image.repository=docker.redpanda.com/redpandadata/redpanda-unstable --set image.tag={full-version} --set console.enabled=false
----
