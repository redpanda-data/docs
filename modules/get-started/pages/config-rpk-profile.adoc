...
A profile saves rpk-specific command properties. For details, see xref::intro-to-rpk.adoc#specifying-command-properties[Specifying command properties].
...
:::note
You should always use and set the `--description` flag to describe your profiles. The description is printed in the output of xref:reference:rpk:rpk-profile:rpk-profile-list.adoc[`rpk profile list`].
:::
...
`
username@hostname`+++<font color="red">+++[dev]+++</font>+++
...
See the xref:reference:rpk.adoc[rpk commands reference] for commands that support profiles.
...
For details about all commands for rpk profiles, see the xref:reference:rpk:rpk-profile.adoc[`rpk profile`] reference page and its sub-pages.
