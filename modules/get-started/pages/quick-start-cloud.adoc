...
Spin up a Dedicated cluster with Redpanda Cloud, create a basic streaming application using the `rpk` command-line tool, and explore your cluster in Redpanda Console.
...
To deploy a Dedicated cluster in Redapanda Cloud, create a namespace, then follow the steps for creating a Dedicated cluster.
...
+
- **Cluster name**: `redpandaquickstart`
+
- Use default values for the remaining cluster settings. 
+
. Click **Next**. 
