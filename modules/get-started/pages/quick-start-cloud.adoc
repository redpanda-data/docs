...
TIP: The fastest and easiest way to start data streaming in the cloud is with a Serverless cluster. In less than one minute, you have a free Serverless welcome cluster that includes a `hello-world` topic in AWS. You can interact with your Serverless cluster the same way you'd interact with a Dedicated or BYOC cluster. See xref:deploy:deployment-option/cloud/serverless.adoc[].
