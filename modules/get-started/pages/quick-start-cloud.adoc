...
TIP: The fastest and easiest way to start data streaming in the cloud is with a Serverless cluster. In less than one minute, you have a Serverless `welcome` cluster that includes a `hello-world` topic with messages. You interact with Serverless clusters the same way you'd interact with Dedicated or BYOC clusters. To learn more or to sign up for a free trial, see xref:deploy:deployment-option/cloud/serverless.adoc[].
