...
- *User-impersonation*: When you log in using OIDC or basic authentication, a JWT is generated. This token is used for API calls to the Kafka API, Admin API, and Schema Registry, ensuring that actions are performed under your identity and are properly audited in Redpanda.
...
When a user logs in successfully through OIDC or basic authentication, Redpanda Console creates a secure session by generating a JSON Web Token (JWT). This JWT is signed with a secret defined by `jwtSigningKey` and, if configured, stored as a secure cookie. The token encapsulates the user's credentials, which is either an ID token from an OIDC provider or the username/password provided by basic authentication, and serves as the basis for authenticating API requests.
