= Authentication
:description: Redpanda Console supports login authentication using OIDC (OAuth 2.0) and basic authentication—empowered by user impersonation for secure, unified API access.
...
This topic describes how to enable authentication in Redpanda Console and explains how it interacts with the Redpanda APIs. Authentication allows users to log in to Redpanda Console and provides an option for Redpanda Console to reuse those login credentials to authenticate to the connected Redpanda cluster, ensuring that API requests are executed under the user's identity.
...
- *User-impersonation*: When you log in using OIDC or basic authentication, a JSON Web Token (JWT) is generated. This token is used for API calls to the Kafka API, Admin API, and Schema Registry, ensuring that actions are performed under your identity and are properly audited in Redpanda.
