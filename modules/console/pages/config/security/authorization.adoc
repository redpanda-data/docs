...
:page-aliases: console:single-sign-on/authorization.adoc, manage:security/console/authorization.adoc
...
All configurations are applied through YAML files which Redpanda Console loads at startup.
This page describes the concepts, the available roles, and how you can bind roles to your users.

NOTE: This page describes the RBAC implementation for Redpanda Console which only manages access to the Console and its functions. This does not apply to clients that interact with a system through other methods such as the Kafka API. To manage client access, you must use either xref:manage:security/authorization/acl.adoc[access control lists] or xref:manage:security/authorization/rbac.adoc[Redpanda role-based access control].
