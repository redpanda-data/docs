...
In {ui}, the *Schema Registry* menu item lists registered and verified schemas, including their serialization format and versions. Select an individual schema to see which topics it applies to.
...
You must add a valid `schemaRegistry` configuration in Redpanda Console. For help configuring Redpanda Console to connect to Schema Registry, see xref:console:config/record-deserialization.adoc[].
endif::[]
...
* xref:console:config/record-deserialization.adoc[]
