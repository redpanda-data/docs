...
```
// (step {"action":"runShell", "command": "docker compose up -d --wait", "workingDirectory": "../test-resources", "timeout": 20000, "exitCodes": [0,1]})
...
// (step {"action":"wait"})
// (step {"action":"saveScreenshot", "path": "overview.png", "directory": "../images", "overwrite": "byVariation", "maxVariation": 0.5})
