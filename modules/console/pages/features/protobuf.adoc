...
:description: Configuring Protobuf deserialization to decode encoded Kafka records in Redpanda Console.
...
xref:features/programmable-push-filters.adoc[programmable push filters] like a JavaScript object.
...
xref:features/schema-registry.adoc[Schema registry] for information about how to set up the schema registry in Redpanda Console.
...
Redpanda Console searches for all files with the .proto extension, up to a directory depth of five levels.
Next, enable `git` in the `config.yaml` file, as shown in the following example.
