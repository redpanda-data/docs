...
* *Consumer group insights*: Track the activity and performance of your consumer groups, manage offsets, and identify potential bottlenecks. Consumers that manage offsets outside of Kafka consumer groups, such as Apache Spark Structured Streaming, do not appear here. Flink consumers appear only when Flink is configured to commit offsets on checkpoints. For details, see xref:develop:consume-data/consumer-offsets.adoc[Consumer Offsets].
