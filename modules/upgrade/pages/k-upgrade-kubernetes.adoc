...
flowchart TB
  ...
  A((Start)):::systemAction --> B["Verify PDB is at least 1 & replication factor is at least 3"]:::userAction
...
. Decommission the broker on the target node to be upgraded. This moves its data and replicas to other brokers. See xref:manage:kubernetes/k-decommission-brokers.adoc[Decommission Brokers].
...
----
+
This step ensures that the Pod is rescheduled on a different node.

. Check that this new Pod is running on a different node:
+
[source,bash]
----
