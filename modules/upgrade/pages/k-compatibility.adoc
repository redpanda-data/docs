...
Starting from version v25.1-k8s, the Redpanda Operator and Redpanda Helm chart follow a new versioning scheme aligned with Redpanda core releases:
...
NOTE: Beta versions are available only for testing and feedback. They are not supported by Redpanda and must not be used in production environments. To give feedback on beta releases, reach out to the Redpanda team in https://redpanda.com/slack[Redpanda Community Slack^].
