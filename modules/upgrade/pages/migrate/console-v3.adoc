...
NOTE: OIDC-based group authorization is no longer available in Redpanda Console. With the move to unified authentication, RBAC is now managed directly in Redpanda, which does not support OIDC groups. As a result, GitHub team synchronization or group-based authorization through OIDC groups is no longer supported.
...
NOTE: OIDC-based group authorization is no longer available in Redpanda Console. With the move to unified authentication, RBAC is now managed directly in Redpanda, which does not support OIDC groups. As a result, Keycloak group synchronization or group-based authorization through OIDC groups is no longer supported.
