...
kubectl --namespace <namespace> patch <namespace>-helmchart <cluster-name> -p '{"metadata":{"finalizers":null}}' --type=merge
kubectl --namespace <namespace> patch <namespace>-helmrepository <cluster-name> -p '{"metadata":{"finalizers":null}}' --type=merge
```
