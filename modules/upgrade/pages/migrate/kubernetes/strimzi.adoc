...
Redpanda includes a built-in HTTP proxy on each broker, enabling direct HTTP-based interactions without the need for a separate bridge component. For details, see xref:develop:http-proxy.adoc[]. For the complete API reference, see the link:/api/doc/http-proxy/[HTTP Proxy API reference].
