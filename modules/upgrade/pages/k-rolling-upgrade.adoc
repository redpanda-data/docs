...
+
When upgrading across this version boundary, Redpanda Console is automatically upgraded from v2 to v3. The operator attempts to migrate your Console configuration automatically. If any configuration settings cannot be migrated, warnings are added to the Console custom resource status.
+
[,bash]
----
kubectl get console <console-name> --namespace <namespace> -o jsonpath='{.status.warnings}'
----
+
