...
=======
ID    HOST                                         PORT
0     <pod-name>.redpanda.test.svc.cluster.local.  9093
1*    redpanda-1.redpanda.test.svc.cluster.local.  9093
2     redpanda-2.redpanda.test.svc.cluster.local.  9093
----
====
+
Here, `<pod-name>` is running a broker with the ID `0`. In this example, the ordinal of the StatefulSet replica (`0` in `<pod-name>`) is the same as the broker's ID. However, this is not always the case.

. Select a broker that has not been upgraded yet and place it into maintenance mode.
+
In this example, the command is executed on a Pod called `<pod-name>`.
+
