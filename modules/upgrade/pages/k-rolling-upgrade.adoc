...
When a new version of the Redpanda Helm chart is released, you must assess its compatibility with the current Redpanda Operator. Changes in the Redpanda Helm chart, especially modifications to the values file, may introduce new features or configuration options that the Operator should be aware of.

. Review the release notes of the Redpanda Helm chart to understand the nature of the changes and their potential impact on your Redpanda clusters.
. Determine if the current version of the Redpanda Operator is compatible with the new Helm chart version. Not all changes in the Helm chart require an upgrade of the Redpanda Operator.
...
* Ensure you are using the latest version of the Redpanda Operator that supports the new Redpanda Helm chart version. See xref:k-compatibility.adoc[].
...
----
+
This step ensures that the Operator can correctly interpret and manage the configurations defined in the Helm chart.
