...
. Before upgrading to v25.3, disable Iceberg on all Iceberg topics by setting the prop:redpanda.iceberg.mode[] topic property to `disabled`. This step ensures that no additional Parquet files are written by Iceberg topics.
+
NOTE: Don't set the prop:iceberg_enabled[] cluster property to `false`. Disabling Iceberg at the cluster level would prevent pending Iceberg commits from being finalized post-upgrade.
